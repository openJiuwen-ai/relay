...
export class IndexBuilder implements IIndexBuilder {
        ...
        if (this.messageListFn) {
          try {
            ...
            if (messages.length > 0) {
              const turns = messages.map((m) => `[${m.agentId ?? 'user'}] ${m.content}`);
   ...
   * Scans dataDir/threads/{threadId}/{agentId}/sessions/{sessionId}/digest.extractive.json
   */
  private discoverSessionDigests(excludedThreadIds?: Set<string>): EvidenceItem[] {
    ...
    for (const threadId of threadIds) {
      ...
      let agentIds: string[];
      try {
        agentIds = readdirSync(threadPath).filter((e) => {
          try {
      ...
      for (const agentId of agentIds) {
        const sessionsPath = join(threadPath, agentId, 'sessions');
        ...
        for (const sessionId of sessionIds) {
          ...
          try {
            ...
            const summary = [
              `Session ${digest.seq} by ${digest.agentId}`,
            ...
            results.push({
              ...
              kind: 'session',
              status: 'active',
              title: `Session ${digest.seq} — ${digest.agentId} @ ${threadId.slice(0, 12)}`,
              summary,
              keywords: toolNames,
              sourcePath: `transcripts/threads/${threadId}/${agentId}/sessions/${sessionId}`,
              sourceHash,
