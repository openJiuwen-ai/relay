...
function resolveToolAgentId(): string | undefined {
...
export async function handleSessionSearch(input: {
  ...
  if (input.agents) params.set('agents', input.agents);
  if (input.limit != null) params.set('limit', String(input.limit));
...
export const sessionChainTools = [
  {
    ...
    description:
      'List session chain for a thread. Shows session IDs, sequence numbers, status, and context health for each agent. ' +
