...
export interface RichAudioBlock extends RichBlockBase {
  kind: 'audio';
  url: string;
  /** F34-b: Voice message text (what the agent "said"). Present = voice message style. */
  text?: string;
  /** F085-P3: Override voice — use this agent's voice instead of the message sender's.
   *  Enables multi-agent voice in a single message (e.g. three cats taking turns). */
  speaker?: string;
  title?: string;
  durationSec?: number;
  mimeType?: string;
