...
/** Per-agent override for a capability */
export interface AgentCapabilityOverride {
  /** Cat ID */
  agentId: string;
  /** Whether enabled for this agent (overrides global) */
  enabled: boolean;
}
...
export interface CapabilityEntry {
  ...
  enabled: boolean;
  /** Per-agent overrides (only stores differences from global) */
  overrides?: AgentCapabilityOverride[];
...
export interface CapabilityBoardItem {
  ...
  source: 'builtin' | 'external';
  enabled: boolean;
  /** Per-agent effective state (global + overrides resolved) */
...
export interface AgentFamily {
  ...
  name: string;
  /** All agentIds belonging to this family */
  agentIds: string[];
}
...
export interface CapabilityPatchRequest {
  ...
  /** Scope: global toggle or per-agent override */
  scope: 'global' | 'agent'|'global' | 'agent';
  /** Required when scope is'global' | 'agent' */
  agentId?: string;
  /** New enabled state */
  enabled: boolean;
