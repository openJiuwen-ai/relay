...
const GEMINI_OFFICE_CLAW_ENV_PLACEHOLDERS: Readonly<Record<string, string>> = {
...
function ensureGeminiOfficeClawEnv(name: string, env?: Record<string, string>): Record<string, string> | undefined {
  if (!isBuiltinServer(name)) return env;
  return {
    ...GEMINI_OFFICE_CLAW_ENV_PLACEHOLDERS,
    ...(env ?? {}),
  };
}
...
export async function writeGeminiMcpConfig(filePath: string, servers: McpServerDescriptor[]): Promise<void> {
  ...
  for (const s of servers) {
    ...
    if (s.enabled) {
      const entry: Record<string, unknown> = { command: s.command, args: s.args };
      const env = ensureGeminiOfficeClawEnv(s.name, s.env);
  ...
  for (const [name, value] of Object.entries(existingMcp)) {
    ...
    cfg.env = ensureGeminiOfficeClawEnv(name, currentEnv);
