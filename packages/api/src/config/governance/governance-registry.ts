...
import type { GovernanceHealthSummary, GovernancePackMeta } from '@openjiuwen/relay-shared';
...
export class GovernanceRegistry {
  constructor(private readonly officeClawRoot: string) {}

  private get filePath(): string {
    return safePath(this.officeClawRoot, REGISTRY_DIR, REGISTRY_FILENAME);
  }
  ...
  private async write(data: RegistryData): Promise<void> {
    const dir = safePath(this.officeClawRoot, REGISTRY_DIR);
