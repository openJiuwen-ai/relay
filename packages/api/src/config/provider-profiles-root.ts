...
export function registerProjectRoot(projectRoot: string): void {
  ...
  const dir = resolve(globalRoot, OFFICE_CLAW_DIR);
...
/**
 * Returns project roots whose agent-catalogs may reference a provider profile.
...
export async function listProviderProfilesProjectRoots(projectRoot: string): Promise<string[]> {
  const globalRoot = resolveGlobalRoot();
  const filePath = resolve(globalRoot, OFFICE_CLAW_DIR, KNOWN_ROOTS_FILENAME);
...
function isProjectRootMigrated(projectRoot: string): boolean {
  const globalRoot = resolveGlobalRoot();
  const filePath = resolve(globalRoot, OFFICE_CLAW_DIR, MIGRATED_ROOTS_FILENAME);
  try {
...
export function detectProjectLocalProfiles(projectRoot: string): string | null {
  ...
  const localMeta = resolve(absProject, OFFICE_CLAW_DIR, META_FILENAME);
