...
import type { AgentProvider } from '@openjiuwen/relay-shared';
...
export function resolveBuiltinClientForProvider(provider: AgentProvider): BuiltinAccountClient | null {
  try {
...
export function resolveExpectedProtocolForProvider(provider: AgentProvider): ProviderProfileProtocol | null {
  try {
 ...
 * per-agentId config file and assembles `${ocProviderName}/${defaultModel}` for -m routing.
