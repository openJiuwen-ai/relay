...
export interface TaskRunnerV2Options {
  ...
  ledger: RunLedger;
  /** Phase 1b: optional actor resolver — maps role + costTier to agentId */
  ...
  /** Resolves thread titles for immutable run snapshots */
  resolveThreadTitle?: (threadId: string) => Promise<string | null> | string | null;
...
export class TaskRunnerV2 {
  ...
  private resolveThreadTitle: TaskRunnerV2Options['resolveThreadTitle'];
  ...
  constructor(opts: TaskRunnerV2Options) {
    ...
    this.resolveThreadTitle = opts.resolveThreadTitle;
  ...
  replaceDynamic(task: AnyTaskSpec, dynamicDefId: string, enabled = true): void {
  ...
  private scheduleTask(task: AnyTaskSpec, deferFirstTick = false): void {
    ...
    if (task.trigger.type === 'cron') {
      ...
      throw new Error(
        `[scheduler] ${task.id}: unsupported trigger type ${(task.trigger as { type?: unknown }).type ?? 'unknown'}`,
      );
    }
  }
  ...
  private scheduleCronTick(task: AnyTaskSpec): void {
    ...
    if (ms > TaskRunnerV2.MAX_TIMER_DELAY) {
      ...
      this.timers.set(task.id, timer);
      this.logger.info(
        `[scheduler] ${task.id}: registered (profile=${task.profile}, cron="${task.trigger.expression}", next in ${ms}ms, waiting chunk ${TaskRunnerV2.MAX_TIMER_DELAY}ms)`,
      );
      return;
    }
  ...
  private async resolveSnapshotThreadTitle(threadId: string | null): Promise<string | null> {
    if (!threadId || !this.resolveThreadTitle) return null;
    try {
      return await this.resolveThreadTitle(threadId);
    } catch (err) {
      this.logger.error(`[scheduler] failed to resolve thread title for snapshot thread=${threadId}`, err);
      return null;
    }
  }

  private async createTaskSnapshotJson(task: AnyTaskSpec): Promise<string> {
    const dynDefId = this.dynamicTaskIds.get(task.id);
    const dynamicDef = dynDefId ? this.dynamicTaskStore?.getById(dynDefId) : null;
    const deliveryThreadId = dynamicDef?.deliveryThreadId ?? null;
    const threadTitle = await this.resolveSnapshotThreadTitle(deliveryThreadId);
    const taskEnabled = task.enabled();
    let effectiveEnabled = taskEnabled;
    if (effectiveEnabled && this.globalControlStore) {
      if (!this.globalControlStore.getGlobalEnabled()) effectiveEnabled = false;
    ...
    const snapshot: TaskRunSnapshot = {
      version: 1,
      id: task.id,
      source: dynDefId ? 'dynamic' : 'builtin',
      templateId: dynamicDef?.templateId ?? null,
      label: task.display?.label ?? dynamicDef?.display.label ?? null,
      category: task.display?.category ?? dynamicDef?.display.category ?? null,
      description: task.display?.description ?? dynamicDef?.display.description ?? null,
      enabled: taskEnabled,
      effectiveEnabled,
  ...
  private createDynamicDefSnapshotJson(def: DynamicTaskDef): string {
    const snapshot: TaskRunSnapshot = {
      version: 1,
      id: def.id,
      source: 'dynamic',
      ...
      description: def.display.description ?? null,
      enabled: def.enabled,
  ...
  private handleMissedOnceTask(def: DynamicTaskDef, store: DynamicTaskStore): void {
    ...
    this.ledger.record({
      ...
      error_summary: null,
      task_snapshot_json: this.createDynamicDefSnapshotJson(def),
    });
  ...
  private async executePipeline(task: AnyTaskSpec, isManualTrigger?: boolean): Promise<void> {
    await executeTaskPipeline({
      ...
      taskSnapshotJson: await this.createTaskSnapshotJson(task),
