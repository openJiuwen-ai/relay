...
/**
 * F139 Phase 1b: ActorResolver — maps actor.role + costTier to a agentId.
 *
 * Uses injectable roster getter (decoupled from agent-config-loader singleton)
 ...
 * Returns agentId or null if no match.
 */
export function createActorResolver(getRoster: RosterGetter): (role: ActorRole, costTier: CostTier) => string | null {
  return (role: ActorRole, costTier: CostTier): string | null => {
    ...
    const candidates = Object.entries(roster)
      ...
      .map(([agentId, entry]) => ({ agentId, lead: entry.lead }));
