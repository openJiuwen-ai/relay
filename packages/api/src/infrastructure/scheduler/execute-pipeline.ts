...
export interface PipelineContext {
  ...
  /** Run-time task metadata snapshot persisted with every ledger row */
...
export async function executeTaskPipeline(ctx: PipelineContext): Promise<void> {
  ...
  if (globalControlStore && !isManualTrigger) {
    if (!globalControlStore.getGlobalEnabled()) {
      ledger.record({
        ...
        error_summary: null,
        task_snapshot_json: taskSnapshotJson ?? null,
      });
      return;
    }
  ...
  try {
    ...
    const assignedAgentId = task.actor && actorResolver ? actorResolver(task.actor.role, task.actor.costTier) : null;
