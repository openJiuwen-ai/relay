...
import type { ConnectorSource } from '@openjiuwen/relay-shared';
...
export type CiRouteResult =
  | { kind: 'notified'; threadId: string; agentId: string; messageId: string; bucket: CiBucket; content: string }
...
export class CiCdRouter {
  ...
  private async deliver(poll: CiPollResult, tracking: PrTrackingEntry, fingerprint: string): Promise<CiRouteResult> {
    ...
    log.info(
      `[CiCdRouter] CI ${poll.aggregateBucket} → ${tracking.agentId} in thread ${tracking.threadId} (${fingerprint})`,
    );
