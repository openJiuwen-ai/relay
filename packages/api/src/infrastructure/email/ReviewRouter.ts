...
/**
 * Review Router
 * When a GitHub review email is detected, route it to the correct agent + thread.
 *
 * Routing strategy (#668 simplified):
 *   1. PrTrackingStore lookup (唯一路径: repo+pr → agentId+threadId)
 *   2. Not registered → log + skip (no thread creation, no noise)
 *
 * Previous Layer 2 (PR title agent tag fallback) and Layer 3 (Triage thread)
...
export class ReviewRouter {
  ...
  private async deliverAndMark(event: GithubReviewEvent): Promise<RouteResult> {
    ...
    try {
      ...
      log.info(
        `[ReviewRouter] Registry hit: PR ${event.repository}#${event.prNumber} → agent=${tracking.agentId} thread=${tracking.threadId}`,
      );

      const posted = await this.postReviewMessage(tracking.threadId, tracking.agentId, resolvedUserId, event);
      await processedEmailStore.markProcessed(event.emailUid);
