 ...
 * When a review is detected, emits an event to trigger agent invocation.
 *
...
export interface GithubReviewEvent {
  ...
  readonly agentTag: AgentTag | undefined;
  ...
  private async poll(): Promise<void> {
    ...
    try {
      ...
      for await (const message of this.client.fetch(
        ...
        const agentTag = extractAgentFromTitle(parsed.title) ?? undefined;
        ...
        items.push({
          ...
          event: {
            ...parsed,
            agentTag,
            agentId: agentTag ? agentTagToAgentId(agentTag) : undefined,
      ...
      for (const item of items) {
        ...
        this.log.info(
          `[GithubReviewWatcher] Review detected: PR #${item.event.prNumber} agent=${item.event.agentTag ?? 'none'} (${item.event.reviewType})`,
        );
