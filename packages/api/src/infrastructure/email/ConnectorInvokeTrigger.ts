 ...
 * Programmatically triggers a agent invocation after a connector message is posted.
 *
 * Phase 3b: Closes the loop — review email → connector message → agent invocation.
...
import { type AgentId, type MessageContent, officeClawRegistry, type RichBlock } from '@openjiuwen/relay-shared';
...
import type { AgentRouter } from '../../domains/agents/services/agents/routing/AgentRouter.js';
...
export interface ConnectorInvokeTriggerOptions {
  ...
  /** Per-agent outbound deliver timeout in ms (default 10000). Prevents hanging deliver from blocking cleanup. */
...
export class ConnectorInvokeTrigger {
  ...
  trigger(
    ...
    if (priority === 'urgent' && invocationTracker.has(threadId, agentId)) {
      this.handleUrgentTrigger(threadId, agentId, userId, message, messageId, policy?.reason, sender).catch((err) => {
        this.opts.log.error(`[ConnectorInvokeTrigger] Unhandled: ${err instanceof Error ? err.message : String(err)}`);
      });
      return;
    }

    // Normal connector policy: if this agent is already running in this thread, enqueue.
    if (invocationTracker.has(threadId, agentId)) {
      this.enqueueWhileActive(threadId, agentId, userId, message, messageId, sender);
      return;
    }

    // No active invocation → direct execution (existing flow)
    this.executeInBackground(threadId, agentId, userId, message, messageId, undefined, contentBlocks).catch((err) => {
  ...
  private enqueueWhileActive(
    ...
    if (result.outcome === 'full') {
      ...
      log.warn({ threadId, agentId, userId }, '[ConnectorInvokeTrigger] Queue full, connector message not enqueued');
      return 'full';
    }
    ...
    log.info(
      { threadId, agentId, outcome: result.outcome },
  ...
  private async handleUrgentTrigger(
    ...
    const activeOwner = invocationTracker.getUserId(threadId, agentId);
    ...
    if (createResult.outcome === 'duplicate') {
      log.info(
        { threadId, agentId, invocationId: createResult.invocationId },
        '[ConnectorInvokeTrigger] Urgent duplicate ignored',
      );
      return;
    }

    const cancelResult = invocationTracker.cancel(threadId, agentId, userId, 'preempted');
    // F108 P1-4 fix: abort only the target agent's dispatches, not the entire thread
    getMultiMentionOrchestrator().abortBySlot(threadId, agentId);
    log.info(
      { threadId, agentId, cancelled: cancelResult.cancelled, reason: reason ?? 'connector_urgent' },
      '[ConnectorInvokeTrigger] Urgent connector preempt',
    );

    if (cancelResult.cancelled || !invocationTracker.has(threadId, agentId)) {
      if (cancelResult.cancelled) {
        this.opts.queueProcessor?.clearPause(threadId, agentId);
      }
      await this.executeInBackground(threadId, agentId, userId, message, messageId, createResult.invocationId);
      return;
    }

    if (invocationTracker.has(threadId, agentId)) {
      // Avoid queue race: enqueue first while thread is still observed active.
      const enqueueOutcome = this.enqueueWhileActive(threadId, agentId, userId, message, messageId, sender);
  ...
  private async executeInBackground(
    ...
    const controller = invocationTracker.start(threadId, agentId, userId, targetAgents);
    ...
    try {
      ...
      if (this.opts.streamingHook) {
        streamStartPromise = this.opts.streamingHook
          .onStreamStart(threadId, agentId, createResult.invocationId)
          .catch((err) => {
      ...
      for await (const msg of router.routeExecution(userId, message, threadId, messageId, targetAgents, intent, {
        ...(contentBlocks ? { contentBlocks } : {}),
        ...(controller?.signal ? { signal: controller.signal } : {}),
        queueHasQueuedMessages: (tid: string) => invocationQueue.hasQueuedUserMessagesForThread(tid),
        hasQueuedOrActiveAgent: (tid: string, agentId: string) => invocationQueue.hasActiveOrQueuedAgent(tid, agentId),
        cursorBoundaries,
        persistenceContext,
        ...
        if (msg.type === 'done' && msg.agentId) {
          ...
          // ISSUE-9: snapshot richBlocks for current turn before next agent overwrites
          // Cloud-P1-5 fix: only reuse turn if still open (currentTurnAgentId matches)
        ...
        if (msg.type === 'text' && typeof msg.content === 'string') {
          ...
          // ISSUE-9: per-turn text collection (new turn on agentId change or after done)
          if (msg.agentId) {
          ...
          if (nonEmptyTurns.length > 1) {
            for (const turn of nonEmptyTurns) {
              ...
              try {
                ...
                deliveryFailed = true;
                log.error({ err, threadId, agentId: turn.agentId }, '[ConnectorInvokeTrigger] Outbound delivery error');
              }
            }
      ...
      log.info(
        `[ConnectorInvokeTrigger] Invocation ${createResult.invocationId} completed for ${agentId} in thread ${threadId}`,
      );
    } catch (err) {
      ...
      try {
        ...
        log.warn(
          { err: statusErr, invocationId: createResult.invocationId },
          '[ConnectorInvokeTrigger] status update failed',
        );
      }
      ...
      invocationTracker.complete(threadId, agentId, controller);
      // F39 P1 fix: Notify queue processor for auto-dequeue chain
      // (same pattern as messages.ts and invocations.ts)
      this.opts.queueProcessor?.onInvocationComplete(threadId, agentId, finalStatus).catch(() => {
