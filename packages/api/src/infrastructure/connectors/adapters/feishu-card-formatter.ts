...
export function formatFeishuCard(blocks: RichBlock[], agentDisplayName: string, textContent?: string): LarkCard {
  const firstCard = blocks.find((b) => b.kind === 'card');
  const title =
    firstCard && firstCard.kind === 'card' ? `[${agentDisplayName}] ${firstCard.title}` : `[${agentDisplayName}]`;
