...
import { type AgentId, officeClawRegistry, type RichBlock } from '@openjiuwen/relay-shared';
...
export interface OutboundPresentation {
  ...
  readonly suppressAgentPrefix?: boolean | undefined;
...
export class OutboundDeliveryHook {
  ...
  async deliver(
    ...
    const entry = agentId ? officeClawRegistry.tryGet(agentId) : undefined;
    const agentDisplayName = entry?.config.displayName ?? '';
    const textPrefix = !presentation?.suppressAgentPrefix && agentDisplayName ? `[${agentDisplayName}] ` : '';
    ...
    await Promise.allSettled(
      bindings.map(async (binding) => {
        ...
        try {
          ...
          // This ensures each agent's reply is a distinct card with identity header,
          ...
          if (hasRichBlocks && adapter.sendMedia) {
            ...
            for (const block of richBlocks) {
              ...
              // (raw url could be an arbitrary local path like /etc/passwd, exploitable by file-upload adapters)
              if (block.kind === 'file' && 'url' in block && block.url) {
                const fileUrl = block.url as string;
                const isSendFileToolBlock = typeof block.id === 'string' && block.id.startsWith('send-file-');
                const directLocalPath =
                  isSendFileToolBlock && isAbsolute(fileUrl) && !fileUrl.startsWith('/uploads/') && !fileUrl.startsWith('/api/')
                    ? fileUrl
                    : undefined;
                const absPath = resolve?.(fileUrl) ?? directLocalPath;
