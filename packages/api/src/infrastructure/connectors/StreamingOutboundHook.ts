...
export class StreamingOutboundHook {
  ...
  async onStreamStart(threadId: string, agentId?: AgentId, invocationId?: string): Promise<void> {
    ...
    for (const binding of bindings) {
      ...
      try {
        const agentEntry = agentId ? officeClawRegistry.tryGet(agentId) : undefined;
        const prefix = agentEntry ? `[${agentEntry.config.displayName}] ` : '';
