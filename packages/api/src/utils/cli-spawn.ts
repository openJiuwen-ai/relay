...
const REPEATED_REASONING_LIMIT = 25;

function extractReasoningText(event: unknown): string | null {
  ...
  if (itemRecord.type !== 'reasoning') return null;
  return typeof itemRecord.text === 'string' ? itemRecord.text.trim() : null;
}
...
export async function* spawnCli(
  ...
  try {
    ...
    for (;;) {
      ...
      const reasoningText = extractReasoningText(value);
      if (reasoningText && reasoningText.length > 0) {
        if (reasoningText === lastReasoningText) {
          repeatedReasoningCount += 1;
        } else {
        ...
        repeatedReasoningCount = 0;
      }

      if (reasoningText && repeatedReasoningCount >= REPEATED_REASONING_LIMIT) {
        yield {
          type: 'error',
          message: `Reasoning loop detected: same reason_content ${repeatedReasoningCount} times with no tool calls, terminating`,
        };
        killChild();
        break;
      }
