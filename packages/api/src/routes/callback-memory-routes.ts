...
export async function registerCallbackMemoryRoutes(
  ...
  app.get('/api/callbacks/search-evidence', async (request, reply) => {
    ...
    try {
      ...
      app.log.error({ err }, '[callbacks/search-evidence] evidenceStore.search failed');
  ...
  app.post('/api/callbacks/reflect', async (request, reply) => {
    ...
    try {
      ...
      app.log.error({ err }, '[callbacks/reflect] reflectionService.reflect failed');
      return {
        reflection: '',
        degraded: true,
  ...
  app.post('/api/callbacks/retain-memory', async (request, reply) => {
    ...
    try {
      await deps.markerQueue.submit({
        content,
        source: `callback:${record.agentId}:${invocationId}`,
        status: 'captured',
      });
      return { status: 'ok' };
    } catch (err) {
      app.log.error({ err, invocationId }, '[callbacks/retain-memory] markerQueue.submit failed');
