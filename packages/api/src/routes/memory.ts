...
import { agentIdSchema, createAgentId } from '@openjiuwen/relay-shared';
...
import { setLongTermMemoryEnabled } from '../config/memory-toggle-state.js';
...
const writeSchema = z.object({
  ...
  updatedBy: z.union([agentIdSchema(), z.literal('user')]),
});
...
export const memoryRoutes: FastifyPluginAsync<MemoryRoutesOptions> = async (app, opts) => {
  ...
  app.post('/api/handle_memory_toggle', async (request, reply) => {
    const parseResult = memoryToggleSchema.safeParse(request.body);
    ...
    return { enabled: setLongTermMemoryEnabled(parseResult.data.enabled) };
  });
  ...
  app.post('/api/memory', async (request, reply) => {
    ...
    const resolvedUpdatedBy = updatedBy === 'user' ? ('user' as const) : createAgentId(updatedBy);
