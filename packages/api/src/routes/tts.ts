...
import type { TtsStreamEvent, TtsSynthesizeRequest } from '@openjiuwen/relay-shared';
import type { FastifyInstance, FastifyPluginOptions } from 'fastify';
import { z } from 'zod';
import { getAgentVoice } from '../config/office-claw-voices.js';
import { chunkText } from '../domains/agents/services/tts/TtsChunker.js';
import type { TtsRegistry } from '../domains/agents/services/tts/TtsRegistry.js';
...
export async function ttsRoutes(app: FastifyInstance, opts: TtsRouteOptions): Promise<void> {
   ...
   * Synthesize text to speech for an agent.
   */
  app.post<{ Body: unknown }>('/api/tts/synthesize', async (request, reply) => {
    ...
    const { text, agentId, voice: voiceOverride, langCode: langCodeOverride, speed: speedOverride } = parsed.data;

    // Resolve voice config: explicit params > per-agent defaults
    const agentVoice = agentId ? getAgentVoice(agentId) : getAgentVoice('opus');
    const voice = voiceOverride ?? agentVoice.voice;
    const langCode = langCodeOverride ?? agentVoice.langCode;
    const speed = speedOverride ?? agentVoice.speed ?? 1.0;
    const requestedFormat = 'wav';
    // F066: Clone params from per-agent voice config
  ...
  app.post<{ Body: unknown }>('/api/tts/resynthesize', async (request, reply) => {
    ...
    try {
      const result = await synthesizer.resynthesize(parsed.data.text, parsed.data.agentId);
  ...
  app.post<{ Body: unknown }>('/api/tts/stream', async (request, reply) => {
    ...
    const agentVoice = getAgentVoice(agentId ?? 'opus');
    const voice = voiceOverride ?? agentVoice.voice;
    const langCode = langCodeOverride ?? agentVoice.langCode;
    const speed = speedOverride ?? agentVoice.speed ?? 1.0;
    const refAudio = agentVoice.refAudio;
