...
export const invocationsRoutes: FastifyPluginAsync<InvocationsRoutesOptions> = async (app, opts) => {
  ...
  app.post<{ Params: { id: string } }>('/api/invocations/:id/retry', async (request, reply) => {
    ...
    // ④ Rebuild intent from stored content + targetAgents
    const intent = parseIntent(storedMessage.content, record.targetAgents.length);
    ...
    const primaryCat = record.targetAgents[0] ?? 'unknown';
    const controller = opts.invocationTracker.start(record.threadId, primaryCat, record.userId, record.targetAgents);
            ...
            ...(opts.queueProcessor
              ? {
                  ...
                  hasQueuedOrActiveAgent: (tid: string, agentId: string) =>
                    opts.queueProcessor?.hasActiveOrQueuedAgent(tid, agentId) ?? false,
                }
              : {}),
            cursorBoundaries,
            persistenceContext,
            parentInvocationId: id,
            gatewayIdentity: { userId: record.userId },
          },
        )) {
