...
export const evidenceRoutes: FastifyPluginAsync<EvidenceRoutesOptions> = async (app, opts) => {
  app.get('/api/evidence/search', async (request, reply) => {
    ...
    try {
      ...
      app.log.error({ err, q }, '[evidence/search] evidence store error');
      return {
        results: [],
        degraded: true,
  ...
  app.get('/api/evidence/status', async () => {
    try {
      const stats = await (opts.evidenceStore as { stats?: () => Promise<unknown> }).stats?.();
      if (!stats) return { backend: 'noop', healthy: true, degraded: true, reason: 'stats_unavailable' };

      return stats;
    } catch (err) {
      app.log.error({ err }, '[evidence/status] stats query error');
      return { backend: 'unknown', healthy: false, reason: 'query_error' };
    }
  });
