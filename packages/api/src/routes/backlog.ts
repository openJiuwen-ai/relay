...
import type { BacklogDependencies, BacklogItem, AgentId, MissionHubSelfClaimScope, ThreadPhase } from '@openjiuwen/relay-shared';
import { agentIdSchema, officeClawRegistry } from '@openjiuwen/relay-shared';
...
import { BacklogTransitionError } from '../domains/agents/services/stores/ports/BacklogStore.js';
import { generateSortableId, type IMessageStore } from '../domains/agents/services/stores/ports/MessageStore.js';
...
export interface BacklogRoutesOptions {
  ...
  featuresDir?: string;
  resolveSelfClaimScope?: (agentId: AgentId) => MissionHubSelfClaimScope;
}
...
const createBacklogSchema = z.object({
  ...
  createdBy: z
    .union([z.literal('user'), agentIdSchema()])
    .optional()
    .default('user'),
});
...
function buildKickoffMessage(item: BacklogItem, phase: ThreadPhase): string {
  ...
  const parts = [
    ...
    suggestion
      ? [
          `<claim_suggestion>`,
          `  <agent_id>${escapeXml(suggestion.agentId)}</agent_id>`,
...
function isSelfClaimApprovedByCat(item: BacklogItem, agentId: AgentId): boolean {
  if (item.status !== 'approved' && item.status !== 'dispatched') return false;
  if (!item.suggestion) return false;
  if (item.suggestion.agentId !== agentId) return false;
  if (item.suggestion.status !== 'approved') return false;
  return item.suggestion.note === `self-claim:${agentId}`;
}

function isActiveLeaseOwner(item: BacklogItem, agentId: AgentId, now: number): boolean {
  return (
    item.status === 'dispatched' &&
    item.lease?.state === 'active' &&
    item.lease.ownerAgentId === agentId &&
...
export const backlogRoutes: FastifyPluginAsync<BacklogRoutesOptions> = async (app, opts) => {
  ...
  const resolveSelfClaimScope = opts.resolveSelfClaimScope ?? ((agentId: AgentId) => getMissionHubSelfClaimScope(agentId));
  ...
  app.post('/api/backlog/items', async (request, reply) => {
    ...
    const item = await backlogStore.create({
      ...
      createdBy: parsed.data.createdBy as AgentId | 'user',
    });
  ...
  app.post('/api/backlog/import-active-features', async (request, reply) => {
    ...
    let featureDepsMap: Map<string, import('@openjiuwen/relay-shared').BacklogDependencies>;
    try {
  ...
  app.get('/api/backlog/self-claim-policy', async (request, reply) => {
    ...
    for (const agentId of ids) {
      scopes[agentId] = resolveSelfClaimScope(agentId as AgentId);
    }
  ...
  app.post<{ Params: { id: string } }>('/api/backlog/items/:id/self-claim', async (request, reply) => {
    ...
    const selfClaimScope = resolveSelfClaimScope(agentId);
    ...
    if (selfClaimScope === 'once') {
      const onceConsumed = otherItems.some((item) => isSelfClaimApprovedByCat(item, agentId));
      if (onceConsumed) {
        reply.status(403);
        return { error: 'Self-claim once policy already consumed for this agent' };
      }
    }

    if (selfClaimScope === 'thread') {
      const now = Date.now();
      const activeLeaseConflict = otherItems.some((item) => isActiveLeaseOwner(item, agentId, now));
    ...
    try {
      ...
      if (next.status === 'suggested') {
        ...
        if (next.suggestion.agentId !== agentId) {
          reply.status(409);
          return { error: 'Invalid backlog transition: suggested owner does not match self-claim agent' };
        }
        const approved = await backlogStore.decideClaim(itemId, {
          decision: 'approve',
          decidedBy: userId,
          note: `self-claim:${agentId}`,
        });
        if (!approved) {
          reply.status(404);
          return { error: 'Backlog item not found' };
      ...
      if (next.status === 'approved') {
        if (next.suggestion?.agentId && next.suggestion.agentId !== agentId) {
          reply.status(409);
          return { error: 'Invalid backlog transition: approved suggestion belongs to another agent' };
        }
  ...
  app.post<{ Params: { id: string } }>('/api/backlog/items/:id/lease/release', async (request, reply) => {
    ...
    try {
      const updated = await backlogStore.releaseLease(request.params.id, {
        actorId: userId,
        ...(parsed.data.agentId ? { agentId: parsed.data.agentId as AgentId } : {}),
      });
      if (!updated) {
        reply.status(404);
        return { error: 'Backlog item not found' };
