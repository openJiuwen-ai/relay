...
import { aggregateUsageByDay, type DailyUsageReport } from '../domains/agents/services/usage-aggregator.js';
...
/** Simple in-memory response cache with TTL, keyed by userId+days+agentId */
...
export const usageRoutes: FastifyPluginAsync<UsageRoutesOptions> = async (app, opts) => {
  app.get<{
    Querystring: { days?: string; agentId?: string; refresh?: string };
    ...
    const cacheKey = `${userId}:${days}:${agentId ?? ''}`;
    ...
    const report = aggregateUsageByDay(userRecords, { days, agentId });
