...
export const messageActionsRoutes: FastifyPluginAsync<MessageActionsRoutesOptions> = async (app, opts) => {
  ...
  app.delete<{ Params: { id: string } }>('/api/messages/:id', async (request, reply) => {
    ...
    if (mode === 'hard') {
      ...
      await Promise.resolve(opts.feedbackStore?.deleteByMessage(deleted.threadId, id)).catch((err) => {
        request.log.error({ err, threadId: deleted.threadId, messageId: id }, 'feedback cleanup failed');
      });
