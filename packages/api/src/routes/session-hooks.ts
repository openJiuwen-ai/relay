...
interface SessionHooksRouteOptions extends FastifyPluginOptions {
  ...
  /** Shared secret for hook authentication. If set, X-OfficeClaw-Hook-Token header is required. */
  hookToken?: string;
}
...
export async function sessionHooksRoutes(app: FastifyInstance, opts: SessionHooksRouteOptions): Promise<void> {
  ...
  app.post('/api/sessions/seal', async (request, reply) => {
    ...
    const strategy = getSessionStrategy(record.agentId as string);
  ...
  app.get<{
    ...
    // Look up the session record to find agentId + threadId
    const record = await sessionChainStore.getByCliSessionId(cliSessionId);
    if (!record) {
      reply.status(404);
      return { error: 'No session found for this CLI session ID' };
    ...
    // Get the full chain for this agent+thread, find the latest sealed session
    const chain = await sessionChainStore.getChain(record.agentId, record.threadId);
    ...
    const digest = await transcriptReader.readDigest(latest.id, latest.threadId, latest.agentId);
    if (!digest) {
      reply.status(404);
