...
import type { ILimbNode, LimbAuthLevel, LimbInvokeResult, LimbNodeRecord, LimbNodeStatus } from '@openjiuwen/relay-shared';
...
export class LimbRegistry {
  ...
  async invoke(
    ...
    context?: { agentId?: string; invocationId?: string },
    ...
    if (this.deps.accessPolicy) {
      authLevel = this.deps.accessPolicy.getEffectiveAuth(agentId, nodeId, matchedCap);
    }
    ...
    if (authLevel === 'leased' && this.deps.leaseManager) {
      const lease = this.deps.leaseManager.acquire(agentId, nodeId, capName);
      if (!lease) {
        return { success: false, error: `Capability '${capName}' on '${nodeId}' is currently leased by another agent` };
      }
