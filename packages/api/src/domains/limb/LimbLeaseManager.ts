...
import type { LimbLease } from '@openjiuwen/relay-shared';
...
export class LimbLeaseManager {
  ...
  acquire(agentId: string, nodeId: string, capability: string): LimbLease | null {
    ...
    for (const lease of this.leases.values()) {
      if (lease.nodeId === nodeId && lease.capability === capability) {
        ...
        if (lease.agentId === agentId) {
          return lease; // Idempotent: same agent already holds it
        }
        return null; // Another agent holds it
      }
    }
  ...
  /** 按 agentId 释放所有租约（猫 crash 时调用） */
  releaseAllByCat(agentId: string): string[] {
