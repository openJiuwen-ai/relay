...
import type { TtsSynthesizeRequest, VoiceChunkEvent, VoiceConfig } from '@openjiuwen/relay-shared';
...
export class StreamingTtsChunker {
  ...
  private async synthesizeAndBroadcast(text: string, index: number): Promise<void> {
    ...
    const { agentId, invocationId, threadId, voiceConfig, broadcaster, ttsRegistry } = this.config;
    ...
    try {
      ...
      if (!this.startBroadcasted) {
        ...
        log.info({ agentId, invocationId }, 'First chunk sent');
      }
