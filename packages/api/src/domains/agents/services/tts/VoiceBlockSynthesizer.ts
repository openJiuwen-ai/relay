...
export class VoiceBlockSynthesizer {
   ...
   */
  async resolveVoiceBlocks(blocks: RichBlock[], agentId: string): Promise<RichBlock[]> {
    ...
    for (const block of blocks) {
      ...
      // F085-P3: per-block speaker override for multi-agent voice
      const voiceAgentId = 'speaker' in block && typeof block.speaker === 'string' ? block.speaker : agentId;

      try {
        const result = await this.synthesizeWithRetry(text, voiceAgentId);
        resolved.push({
          ...block,
          url: result.audioUrl,
        ...
        log.error({ agentId, error: err }, 'Synthesis failed');
        resolved.push({
          id: block.id,
          kind: 'card' as const,
          v: 1 as const,
          ...
          actions: [
            {
              ...
              payload: { text, agentId: voiceAgentId },
            },
          ],
        });
      }
   ...
   * Re-attempts TTS synthesis with retry for a given text + agentId.
   */
  async resynthesize(text: string, agentId: string): Promise<{ audioUrl: string; durationSec?: number }> {
    return this.synthesizeWithRetry(text, agentId);
  }

  /**
   * F066 Phase 4: Synthesize with 1 automatic retry for transient errors.
   */
  private async synthesizeWithRetry(text: string, agentId: string): Promise<{ audioUrl: string; durationSec?: number }> {
    try {
      return await this.synthesizeToFile(text, agentId);
    } catch (err) {
   ...
   */
  private async synthesizeToFile(text: string, agentId: string): Promise<{ audioUrl: string; durationSec?: number }> {
    ...
    // Resolve per-agent voice
    const agentVoice = getAgentVoice(agentId);
    const voice = agentVoice.voice;
    const langCode = agentVoice.langCode;
    const speed = agentVoice.speed ?? 1.0;
    const format = 'wav' as const;

    // F066: Clone fields from E-type voice config
    const refAudio = agentVoice.refAudio;
    const refText = agentVoice.refText;
    const instruct = agentVoice.instruct;
    const temperature = agentVoice.temperature;
