...
const DEFAULT_TTL = 90 * 24 * 60 * 60; // 3 months
 ...
 * ARGV[1] = agentId, ARGV[2] = timestamp, ARGV[3] = ttl (or -1 for no expiration)
 *
 ...
 * KEYS[1] = feedback hash key, ARGV[1] = agentId
 */
const HGETDEL_LUA = `
...
export class RedisThreadStore implements IThreadStore {
  ...
  async get(threadId: string): Promise<Thread | null> {
    ...
    thread.participants = members as AgentId[];
    return thread;
  }
  ...
  async getParticipantsWithActivity(threadId: string): Promise<ThreadParticipantActivity[]> {
    ...
    const result: ThreadParticipantActivity[] = participants.map((agentId) => {
      const lastMessageAt = parseInt(activityData[`${agentId}:lastMessageAt`] ?? '0', 10);
      const messageCount = parseInt(activityData[`${agentId}:messageCount`] ?? '0', 10);
      return { agentId, lastMessageAt, messageCount };
    });
  ...
  async updateParticipantActivity(threadId: string, agentId: AgentId): Promise<void> {
  ...
  async updatePreferredCats(threadId: string, agentIds: AgentId[]): Promise<void> {
    const key = ThreadKeys.detail(threadId);
  ...
  async setMentionRoutingFeedback(
    ...
    await this.redis.hset(feedbackKey, agentId, JSON.stringify(feedback));
    if (this.ttlSeconds !== null) {
      await this.redis.expire(feedbackKey, this.ttlSeconds);
    }
  }

  async consumeMentionRoutingFeedback(threadId: string, agentId: AgentId): Promise<ThreadMentionRoutingFeedback | null> {
    const feedbackKey = ThreadKeys.mentionRoutingFeedback(threadId);
    const raw = (await this.redis.eval(HGETDEL_LUA, 1, feedbackKey, agentId)) as string | null;
    if (!raw) return null;
  ...
  async inviteExpert(threadId: string, expertId: string): Promise<void> {
    const detailKey = ThreadKeys.detail(threadId);
    const participantsKey = ThreadKeys.participants(threadId);
    const exists = await this.redis.hexists(detailKey, 'id');
    if (exists === 0) return;

    // Add to invitedExpertIds (stored as JSON array)
    const thread = await this.get(threadId);
    if (!thread) return;
    const invitedExpertIds = thread.invitedExpertIds ?? [];
    if (!invitedExpertIds.includes(expertId)) {
      const updated = [...invitedExpertIds, expertId];
      await this.redis.eval(HSET_IF_HAS_ID_LUA, 1, detailKey, 'invitedExpertIds', JSON.stringify(updated));
    }

    // Also add to participants set
    await this.redis.sadd(participantsKey, expertId);
  }

  async removeExpert(threadId: string, expertId: string): Promise<void> {
    const detailKey = ThreadKeys.detail(threadId);
    const participantsKey = ThreadKeys.participants(threadId);
    const exists = await this.redis.hexists(detailKey, 'id');
    if (exists === 0) return;

    // Remove from invitedExpertIds
    const thread = await this.get(threadId);
    if (!thread) return;
    const updated = (thread.invitedExpertIds ?? []).filter((id) => id !== expertId);
    await this.redis.eval(HSET_IF_HAS_ID_LUA, 1, detailKey, 'invitedExpertIds', JSON.stringify(updated));

    // Also remove from participants set
    await this.redis.srem(participantsKey, expertId);
  }

  async getInvitedExperts(threadId: string): Promise<string[]> {
    const thread = await this.get(threadId);
  ...
  private serializeThread(thread: Thread): Record<string, string> {
    ...
    if (thread.invitedExpertIds && thread.invitedExpertIds.length > 0) {
      result.invitedExpertIds = JSON.stringify(thread.invitedExpertIds);
    }
    return result;
  }
  ...
  private hydrateThread(data: Record<string, string>): Thread {
    ...
    const result: Thread = {
      ...
      invitedExpertIds: [], // Loaded from detail hash
    ...
    if (data.preferredCats) {
      try {
        ...
        if (Array.isArray(parsed)) {
          result.preferredCats = parsed as AgentId[];
        }
      } catch {
    ...
    if (data.invitedExpertIds) {
      try {
        const parsed = JSON.parse(data.invitedExpertIds);
        if (Array.isArray(parsed)) {
          result.invitedExpertIds = parsed as string[];
        }
      } catch {
        /* ignore malformed JSON */
      }
