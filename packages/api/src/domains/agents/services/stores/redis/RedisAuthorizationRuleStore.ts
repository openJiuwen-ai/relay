...
import type { AuthorizationRule, AgentId } from '@openjiuwen/relay-shared';
...
export class RedisAuthorizationRuleStore implements IAuthorizationRuleStore {
  ...
  async match(agentId: AgentId, action: string, threadId: string): Promise<AuthorizationRule | null> {
  ...
  async list(filter?: { agentId?: AgentId; threadId?: string }): Promise<AuthorizationRule[]> {
    const ruleIds = await this.redis.zrevrange(AuthRuleKeys.ALL, 0, -1);
