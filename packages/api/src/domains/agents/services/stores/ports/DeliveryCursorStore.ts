 ...
 *
 * Tracks per-user/per-agent/per-thread last delivered message ID.
...
import { createAgentId, officeClawRegistry } from '@openjiuwen/relay-shared';
...
const FALLBACK_CATS: readonly AgentId[] = [createAgentId('opus'), createAgentId('codex'), createAgentId('gemini')];

/** Get all agent IDs dynamically from registry, with static fallback */
function getAllCats(): readonly AgentId[] {
  const ids = officeClawRegistry.getAllIds();
  return ids.length > 0 ? ids.map((id) => createAgentId(id)) : FALLBACK_CATS;
}

function cursorKey(userId: string, agentId: AgentId, threadId: string): string {
...
export class DeliveryCursorStore {
  ...
  async getCursor(userId: string, agentId: AgentId, threadId: string): Promise<string | undefined> {
    const key = cursorKey(userId, agentId, threadId);
    const memValue = this.cursors.get(key);
    if (this.sessionStore) {
      try {
        const redisValue = await this.sessionStore.getDeliveryCursor(userId, agentId, threadId);
        if (redisValue != null) {
   ...
   */
  async ackCursor(userId: string, agentId: AgentId, threadId: string, deliveredToId: string): Promise<void> {
    const key = cursorKey(userId, agentId, threadId);
    ...
    if (this.sessionStore) {
      try {
        ...
        const advanced = await this.sessionStore.setDeliveryCursor(userId, agentId, threadId, effective);
        if (advanced) {
          ...
          try {
            const actual = await this.sessionStore.getDeliveryCursor(userId, agentId, threadId);
  ...
  /**
   * Get the last acknowledged mention message ID for an agent in a thread.
   * Returns undefined if no ack cursor exists (= all mentions are pending).
   */
  async getMentionAckCursor(userId: string, agentId: AgentId, threadId: string): Promise<string | undefined> {
    const key = cursorKey(userId, agentId, threadId);
    const memValue = this.mentionAckCursors.get(key);
    if (this.sessionStore) {
      try {
        const redisValue = await this.sessionStore.getMentionAckCursor(userId, agentId, threadId);
        if (redisValue != null) {
   ...
   */
  async ackMentionCursor(userId: string, agentId: AgentId, threadId: string, messageId: string): Promise<void> {
    const key = cursorKey(userId, agentId, threadId);
    ...
    if (this.sessionStore) {
      try {
        // Atomic CAS in Redis — monotonic check + write in one round-trip
        const advanced = await this.sessionStore.setMentionAckCursor(userId, agentId, threadId, effective);
        if (advanced) {
          // CAS accepted — sync in-memory to match Redis
          ...
          try {
            const actual = await this.sessionStore.getMentionAckCursor(userId, agentId, threadId);
  ...
  /**
   * Cleanup all per-agent delivery + mention-ack cursors for one user's thread.
  ...
  async deleteByThreadForUser(userId: string, threadId: string): Promise<number> {
    ...
    if (this.sessionStore) {
      for (const agentId of getAllCats()) {
        try {
          deleted += await this.sessionStore.deleteDeliveryCursor(userId, agentId, threadId);
        } catch (err) {
          log.warn({ err }, 'deleteDeliveryCursor failed, continue cleanup in-memory');
        }
        try {
          deleted += await this.sessionStore.deleteMentionAckCursor(userId, agentId, threadId);
        } catch (err) {
