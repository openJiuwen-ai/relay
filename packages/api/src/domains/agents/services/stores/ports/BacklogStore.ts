...
export type { IBacklogStore } from '@openjiuwen/relay-api-server-contracts/storage';

import type { IBacklogStore } from '@openjiuwen/relay-api-server-contracts/storage';
...
export class BacklogStore implements IBacklogStore {
  ...
  acquireLease(itemId: string, input: AcquireBacklogLeaseInput): BacklogItem | null {
    ...
    if (this.isLeaseActive(currentLease, now) && currentLease?.ownerAgentId !== input.agentId) {
    ...
    const updated: BacklogItem = {
      ...
      audit: [
        ...existing.audit,
        {
          ...
          timestamp: now,
          detail: `${input.agentId}:${nextLease.expiresAt}`,
        },
      ],
    };
    this.items.set(itemId, updated);
  ...
  heartbeatLease(itemId: string, input: HeartbeatBacklogLeaseInput): BacklogItem | null {
    ...
    if (lease.ownerAgentId !== input.agentId) {
  ...
  releaseLease(itemId: string, input: ReleaseBacklogLeaseInput): BacklogItem | null {
    ...
    if (input.agentId && lease.ownerAgentId !== input.agentId) {
      throw new BacklogTransitionError('Invalid backlog transition: lease owned by another agent');
    }
