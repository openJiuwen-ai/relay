 ...
 * agentId === null → user ("用户"), otherwise look up OFFICE_CLAW_CONFIGS.
 * For variant cats (e.g. sonnet, opus-45), includes variantLabel to distinguish same-family members.
 */
function getSenderName(agentId: string | null): string {
  if (agentId === null) return '用户';
  const entry = officeClawRegistry.tryGet(agentId);
  const config = entry?.config ?? OFFICE_CLAW_CONFIGS[agentId];
 ...
 * Marker includes dropped char count so the agent knows how much was lost.
 */
...
export function formatMessage(msg: StoredMessage, options?: { truncate?: number }): string {
  ...
  const sender = msg.source ? msg.source.label : getSenderName(msg.agentId);
...
export function assembleContext(messages: StoredMessage[], options?: ContextAssemblerOptions): AssembledContext {
  ...
  // Only filter agent messages (agentId !== null) starting with [错误] — user messages are legit.
  ...
  const deliveredMessages = messages.filter(
    (m) => isDelivered(m) && m.userId !== 'system' && !(m.agentId && m.content?.startsWith('[错误]')),
  );
