...
function normalizeSkill(t: TencentSkill): SkillHubSkill {
  ...
  const version = normalizeString(t.version);
...
export async function fetchSkillMetadata(skill: string): Promise<SkillHubSkill | null> {
  const slug = normalizeString(skill);
  if (!slug) return null;
  const result = await searchSkills(slug, { limit: 20 });
...
export async function fetchSkillAllFiles(
  owner: string,
  repo: string,
  skill: string,
