...
import type { AgentId, ReviewPolicy, Roster } from '@openjiuwen/relay-shared';
import { createAgentId } from '@openjiuwen/relay-shared';
import { getDefaultAgentId, getReviewPolicy, getRoster } from '../../../../config/office-claw-config-loader.js';
...
export interface ReviewerMatchOptions {
  ...
  /**
   * F032 Phase C: Thread activity scores (lastMessageAt per agent).
   * Caller should populate from threadStore.getParticipantsWithActivity().
   * Map: agentId → lastMessageAt timestamp
   */
...
export async function resolveReviewer(options: ReviewerMatchOptions): Promise<ReviewerMatchResult> {
  ...
  // If author not in roster, we can't determine family → use default agent
  if (!authorEntry) {
    return {
      reviewer: getDefaultAgentId(),
      isDegraded: false,
      candidates: [],
    };
  }
  ...
  const allCandidateIds = availableReviewers.map(([id]) => createAgentId(id));
  ...
  if (policy.requireDifferentFamily || differentFamily.length > 0) {
    ...
    if (sorted.length > 0) {
      return {
        reviewer: createAgentId(sorted[0]?.[0]),
        isDegraded: false,
  ...
  // 7. Ultimate fallback: default agent
  log.warn({ authorId }, `[resolveReviewer] No reviewers available. Using default agent.`);
  return {
    reviewer: getDefaultAgentId(),
    isDegraded: true,
 ...
 */
export async function getAvailableReviewers(options: ReviewerMatchOptions): Promise<readonly AgentId[]> {
  const result = await resolveReviewer(options);
  return result.candidates;
}

/**
 * Check if a specific agent can review another agent's code.
 * Returns { canReview, reason }.
 */
export function canReview(reviewer: AgentId, author: AgentId): { canReview: boolean; reason: string } {
  const roster = getRoster();
