...
import type { A2APart, A2ATask, AgentId } from '@openjiuwen/relay-shared';
...
export function transformA2ATaskToMessages(task: A2ATask, agentId: AgentId): AgentMessage[] {
  ...
  if (status === 'completed') {
    if (task.artifacts && task.artifacts.length > 0) {
      for (const artifact of task.artifacts) {
        ...
        if (text) {
          messages.push(msg('text', agentId, text));
        }
        for (const part of artifact.parts) {
          if (part.type === 'file' && part.file) {
            messages.push(msg('text', agentId, `[File: ${part.file.name} (${part.file.mimeType})]`));
          }
        }
      }
    }
    ...
    messages.push(msg('done', agentId));
  } else if (status === 'failed') {
    messages.push(msg('error', agentId, 'A2A task failed'));
  } else if (status === 'input-required') {
    messages.push(msg('system_info', agentId, 'A2A agent requires additional input'));
  }
