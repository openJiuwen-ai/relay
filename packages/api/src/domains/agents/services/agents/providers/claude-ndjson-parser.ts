...
export function transformClaudeEvent(
    ...
    if (s.type === 'content_block_stop') {
      if (streamState.thinkingBuffer.length > 0) {
        ...
        return {
          type: 'system_info',
          agentId,
          content: JSON.stringify({ type: 'thinking', agentId, text }),
          timestamp: Date.now(),
        };
      }
      return null;
  ...
  if (e.type === 'system' && e.subtype === 'compact_boundary') {
    ...
    return {
      type: 'system_info',
      agentId,
      content: JSON.stringify({ type: 'compact_boundary', agentId, preTokens }),
      timestamp: Date.now(),
    };
  }
  ...
  if (e.type === 'rate_limit_event') {
    ...
    return {
      type: 'system_info',
      agentId,
      content: JSON.stringify({ type: 'rate_limit', agentId, utilization, resetsAt }),
      timestamp: Date.now(),
    };
  }
