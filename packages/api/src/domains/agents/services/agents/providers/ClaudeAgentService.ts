...
import { getAgentEffort } from '../../../../../config/office-claw-config-loader.js';
 ...
 * F32-b: agentId is now a constructor parameter (defaults to 'opus' for backward compat)
 */
interface ClaudeAgentServiceOptions {
  /** F32-b: agentId for this instance (default: 'opus') */
  agentId?: AgentId;
  /** Inject a custom spawn function (for testing) */
  spawnFn?: SpawnFn;
  /** Model override (default: resolved via getAgentModel) */
  model?: string;
...
export class ClaudeAgentService implements AgentService {
  ...
  constructor(options?: ClaudeAgentServiceOptions) {
    this.agentId = options?.agentId ?? createAgentId('opus');
    this.spawnFn = options?.spawnFn;
    // F32-b: model from options > env (getAgentModel) > default
  ...
  async *invoke(prompt: string, options?: AgentServiceOptions): AsyncIterable<AgentMessage> {
    ...
    const args: string[] = [
      ...
      cliModel,
      '--effort',
      getAgentEffort(this.agentId as string),
      '--permission-mode',
      PERMISSION_MODE,
    ...
    try {
      ...
      log.info({ agentId: this.agentId, resolved: claudeCommand ?? null }, 'Resolving claude CLI command');
      if (!claudeCommand) {
        log.warn({ agentId: this.agentId }, 'Claude CLI not found');
        yield {
          type: 'error' as const,
        ...
        yield { type: 'done' as const, agentId: this.agentId, metadata, timestamp: Date.now() };
        return;
      }
      ...
      for await (const event of events) {
        ...
        const evtType =
          typeof event === 'object' && event !== null && 'type' in event
            ...
            : '__unknown';
        log.debug({ agentId: this.agentId, eventIndex: eventCount, type: evtType }, 'CLI event received');
        ...
        // F118 Phase C: Forward liveness warnings to frontend with agentId
        ...
        let result = transformClaudeEvent(event, this.agentId, streamState);
        if (result === null) {
          log.debug({ agentId: this.agentId, eventIndex: eventCount, rawType: evtType }, 'Event dropped by transform');
          continue;
        }
      ...
      log.info(
        { agentId: this.agentId, totalEvents: eventCount, textEvents: textEventCount, sessionId: metadata.sessionId },
        'Claude CLI invocation completed',
      );
      if (textEventCount === 0) {
        log.warn(
          { agentId: this.agentId, totalEvents: eventCount },
      ...
      // Guarantee done after error so invoke-single-agent can set isFinal correctly
      yield { type: 'done', agentId: this.agentId, metadata, timestamp: Date.now() };
    }
  }
}
