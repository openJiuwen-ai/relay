 ...
 *   1. invoke-single-agent.ts calls writeOpenCodeRuntimeConfig() before each invoke
 *   2. Config written to {projectRoot}/.office-claw/opencode-runtime-{agentId}.json
 ...
 *   5. Per-agentId files isolate multiple opencode members in the same session
 */
...
/**
 * Write an opencode runtime config file for a specific agent member.
 * Returns the absolute path to the written config file.
 *
 * File location: {projectRoot}/.office-claw/opencode-runtime-{agentId}.json
...
export function writeOpenCodeRuntimeConfig(
  ...
  const configPath = join(configDir, `opencode-runtime-${agentId}.json`);
