 ...
 * during an agent's invocation. The agent message doesn't have a StoredMessage.id
 * yet (still streaming), so blocks are buffered by (threadId, agentId) and
 * consumed at append time in route-serial/route-parallel.
 *
 * Key: (threadId, agentId) — only one active invocation per (thread, agent) at a time.
...
function bufferKey(threadId: string, agentId: string): string {
...
export class RichBlockBuffer {
  ...
  add(threadId: string, agentId: string, block: RichBlock, invocationId?: string): boolean {
    ...
    const key = bufferKey(threadId, agentId);
   ...
   */
  consume(threadId: string, agentId: string, invocationId?: string): RichBlock[] {
    const key = bufferKey(threadId, agentId);
