...
export class StartupReconciler {
  ...
  async reconcileOrphans(): Promise<StartupSweepResult> {
    ...
    const affectedThreads = new Map<string, { agentIds: AgentId[]; userId: string }>();
  ...
  private async sweepRunning(
    ...
    affectedThreads: Map<string, { agentIds: AgentId[]; userId: string }>,
    ...
    for (const id of ids) {
      try {
        ...
        if (updated) {
          ...
          taskProgressCleared += await this.clearTaskProgress(record.threadId, record.targetAgents);
  ...
  private trackAffectedThread(map: Map<string, { agentIds: AgentId[]; userId: string }>, record: InvocationRecord): void {
    const existing = map.get(record.threadId) ?? { agentIds: [], userId: record.userId };
    for (const agentId of record.targetAgents) {
  ...
  private async notifyAffectedThreads(
    ...
    let notified = 0;
    for (const [threadId, { agentIds, userId }] of affectedThreads) {
      const agentLabel = agentIds.length === 1 ? agentIds[0] : `${agentIds.length} cats`;
      const content =
        `⚠️ 服务重启 — ${agentLabel} 的进行中请求已中断，请重新发送。\n` +
        `Service restarted — interrupted in-progress request (${agentLabel}). Please resend your message.`;
      ...
      if (socketManager) {
        try {
          const errorAgentId = agentIds[0] ?? ('system' as AgentId);
          socketManager.broadcastAgentMessage(
            { type: 'error', agentId: errorAgentId, error: content, isFinal: true, timestamp: Date.now() },
            threadId,
          );
          broadcasted = true;
        } catch (err) {
  ...
  private async clearTaskProgress(threadId: string, targetAgents: AgentId[]): Promise<number> {
    let cleared = 0;
    for (const agentId of targetAgents) {
      try {
        await this.deps.taskProgressStore.deleteSnapshot(threadId, agentId);
        cleared++;
      } catch {
        /* best-effort */
      }
