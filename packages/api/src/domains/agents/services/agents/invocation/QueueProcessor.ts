...
import { buildPptModeSystemPrompt } from '../../../../ppt/ppt-context.js';
...
interface TrackerLike {
  start(threadId: string, agentId: string, userId: string, agentIds?: string[]): AbortController;
...
export interface RouterLike {
  ...
  ): AsyncIterable<{ type: string; agentId?: string; [key: string]: unknown }>;
...
export class QueueProcessor {
  ...
  /** F108: Per-slot mutex — prevents concurrent double-start per (thread, agent) pair */
  ...
  private static slotKey(threadId: string, agentId: string): string {
    return `${threadId}:${agentId}`;
  }

  /** Check if a slot's queue is paused (canceled/failed AND has queued entries). */
  isPaused(threadId: string, agentId?: string): boolean {
    if (agentId) {
      return (
        this.pausedSlots.has(QueueProcessor.slotKey(threadId, agentId)) && this.deps.queue.hasQueuedForThread(threadId)
      );
    }
  ...
  /** A2A dedup: check if a specific agent already has a queued or processing entry for this thread. */
  hasQueuedAgent(threadId: string, agentId: string): boolean {
    return this.deps.queue.hasQueuedAgent(threadId, agentId);
  }

  hasActiveOrQueuedAgent(threadId: string, agentId: string): boolean {
    return this.deps.queue.hasActiveOrQueuedAgent(threadId, agentId);
  }

  /** Returns pause reason when paused; otherwise undefined. */
  getPauseReason(threadId: string, agentId?: string): 'canceled' | 'failed' | undefined {
    if (!this.isPaused(threadId, agentId)) return undefined;
    if (agentId) {
      return this.pausedSlots.get(QueueProcessor.slotKey(threadId, agentId));
    }
   ...
   * F108: Now slot-aware — agentId identifies which slot completed.
  ...
  async onInvocationComplete(
    ...
    const sk = QueueProcessor.slotKey(threadId, agentId);
    if (status === 'succeeded') {
      this.pausedSlots.delete(sk);
      if (this.deps.queue.hasQueuedForThread(threadId)) {
        await this.tryExecuteNextAcrossUsers(threadId, agentId);
   ...
   */
  clearPause(threadId: string, agentId?: string): void {
    if (agentId) {
      this.pausedSlots.delete(QueueProcessor.slotKey(threadId, agentId));
    } else {
   ...
   */
  releaseSlot(threadId: string, agentId: string): void {
    this.processingSlots.delete(QueueProcessor.slotKey(threadId, agentId));
  }

  /**
   * @deprecated Use releaseSlot(threadId, agentId) instead. Kept for backward compat during migration.
   */
  ...
  /**
   * F122B: Try to auto-execute any queued autoExecute entries whose target agent slot is free.
   * Called immediately after enqueuing an agent entry.
   * Scans all entries and starts every one whose agent slot is free (parallel multi-agent).
   * Per-agent slot mutex (processingSlots + invocationTracker) prevents conflicts.
   */
  async tryAutoExecute(threadId: string): Promise<void> {
      ...
      this.deps.log.info(
        {
          ...
          entries: entries.map((entry) => ({
            id: entry.id,
            targetCat: entry.targetAgents[0] ?? 'unknown',
    ...
    for (const entry of entries) {
      const entryCat = entry.targetAgents[0] ?? 'unknown';
      ...
      // Continue scanning — start all entries with free agent slots (parallel dispatch)
    }
  }
  ...
  private async tryExecuteNextAcrossUsers(
    ...
    const entryCat = entry.targetAgents[0] ?? agentId;
    const entrySk = QueueProcessor.slotKey(threadId, entryCat);

    // F108 P1-2 fix: check the *entry's* agent slot, not just the completing agent's slot
    if (this.processingSlots.has(entrySk)) {
      this.deps.queue.rollbackProcessing(threadId, entry.id);
      return { started: false };
    }
    // Fix: skip if agent already has an active invocation via CLI/messages.ts (not in processingSlots).
  ...
  private async tryExecuteNextForUser(
    ...
    // F108 P1-3 fix: peek at next entry's target agent to check slot mutex BEFORE marking processing.
    ...
    const entryCat = nextEntry.targetAgents[0] ?? 'unknown';
    const sk = QueueProcessor.slotKey(threadId, entryCat);
    ...
    // Fix: skip if agent already has an active invocation via CLI/messages.ts (same guard as above)
    if (this.deps.invocationTracker.has(threadId, entryCat)) {
      return { started: false };
    }
  ...
  private async executeEntry(entry: QueueEntry): Promise<'succeeded' | 'failed' | 'canceled'> {
    ...
    const { threadId, userId, content, targetAgents, intent, messageId, resumeAgentId, pptContext, traceId } = entry;
    ...
    try {
      ...
      log.info(
        ...
        '[QueueProcessor] Queued message execution started',
      );

      // 2. Start tracking (slot key = primary target agent)
      controller = invocationTracker.start(threadId, primaryCat, userId, targetAgents);
      ...
      const modeSystemPrompt = pptContext ? buildPptModeSystemPrompt(pptContext) : undefined;
      ...
      for await (const msg of router.routeExecution(
        ...
        { intent },
        {
          ...
          hasQueuedOrActiveAgent: (tid: string, agentId: string) => queue.hasActiveOrQueuedAgent(tid, agentId),
          cursorBoundaries,
          persistenceContext,
          ...(modeSystemPrompt ? { modeSystemPrompt } : {}),
          ...(invocationId ? { parentInvocationId: invocationId } : {}),
          gatewayIdentity: { userId },
        ...
        if (hook && msg.agentId === primaryCat && msg.type === 'text' && (msg as { content?: string }).content) {
      ...
      log.info(
        ...
        '[QueueProcessor] Queued message execution completed',
      );

      // 10. Outbound delivery: send per-turn content to bound external chats.
      // Non-blocking by design: connector/outbound latency must not stall
      // invocation completion, queue advancement, or tracker cleanup.
      this.deliverOutbound(
        ...
        log.warn({ err, threadId, entryId: entry.id }, '[QueueProcessor] deliverOutbound failed (non-blocking)');
      });
      ...
      if (nonEmptyTurns.length > 1) {
        for (const turn of nonEmptyTurns) {
          ...
          try {
            ...
            deliveryFailed = true;
            log.error({ err, threadId, agentId: turn.agentId }, '[QueueProcessor] Outbound delivery error');
          }
        }
