 ...
 *
 * 安全契约:
 * - invocationId → { userId, agentId, callbackToken, expiresAt }
...
export interface InvocationRecord {
  ...
  /** F121: The A2A trigger message ID — the @mention message that caused this agent to be invoked */
...
export class InvocationRegistry {
  ...
  /** Track the latest invocationId per thread+agent (stale callback guard, cloud Codex P1). */
  ...
  create(
    ...
    // Track latest invocation per thread+agent (stale callback guard)
    this.latestByThreadCat.set(`${threadId}:${agentId as string}`, invocationId);
  ...
  /**
   * Read a live invocation record without requiring the callback token.
   * Used by server-side reconciliation paths that only need to know whether
   * the callback invocation is still alive/actionable.
   */
  get(invocationId: string): InvocationRecord | null {
    const record = this.records.get(invocationId);
    if (!record) return null;
    if (Date.now() > record.expiresAt) {
      this.cleanupLatestPointer(invocationId);
  ...
  /**
   * Check if an invocationId is the latest for its thread+agent slot.
  ...
  isLatest(invocationId: string): boolean {
    ...
    const key = `${record.threadId}:${record.agentId as string}`;
    return this.latestByThreadCat.get(key) === invocationId;
  }

  /** Get the latest invocationId for a given thread+agent slot, if any. */
  getLatestId(threadId: string, agentId: string): string | undefined {
    return this.latestByThreadCat.get(`${threadId}:${agentId}`);
  }
