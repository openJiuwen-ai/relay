/*
 * *
 *  * Copyright (C) Huawei Technologies Co., Ltd. 2026. All rights reserved.
 *
 */

/**
 * Agent Router
 * 解析 @ 提及，路由到对应的 Agent Service
 *
 * Features:
 * - 有 @ 提及时路由到指定猫 + 更新对话参与者
 * - 无 @ 提及时路由到最近回复的猫 (F078)
 * - 群组 mention: @all/@全体, @全体{breed}, @thread/@本帖 (F078)
 * - 无参与者的新对话默认路由到Claude (opus)
 * - 支持中英文提及模式
 * - ideate intent + 多猫 → 并行独立思考 (routeParallel)
 * - execute intent 或单猫 → 串行执行 (routeSerial)
 * - Session 管理委托给 SessionManager
 *
 * IMPORTANT: threadId 约束
 * 所有调用入口（execute, executeWithContext）必须传入正确的 threadId。
 * 跨线程鉴权、消息存储、InvocationTracker 都依赖此参数。
 * 虽然参数可选（兼容测试），但生产代码必须显式传入。
 */

import type { AgentId, MessageContent, OfficeClawConfigEntry } from '@openjiuwen/relay-shared';
import { officeClawRegistry, escapeRegExp } from '@openjiuwen/relay-shared';
import type { CatalogProvider, GatewayIdentity } from '@openjiuwen/relay-api-server-contracts';
import type { SessionStore } from '@openjiuwen/relay-shared/utils';
import { getDefaultAgentId, isAgentAvailable, toAllAgentConfigs } from '../../../../../config/office-claw-config-loader.js';
import { createModuleLogger } from '../../../../../infrastructure/logger.js';
import type { IntentResult } from '../../context/IntentParser.js';
import { parseIntent, stripIntentTags } from '../../context/IntentParser.js';
import { SessionManager } from '../../session/SessionManager.js';
import type { ISessionSealer } from '../../session/SessionSealer.js';
import type { TranscriptReader } from '../../session/TranscriptReader.js';
import type { TranscriptWriter } from '../../session/TranscriptWriter.js';
import { DeliveryCursorStore } from '../../stores/ports/DeliveryCursorStore.js';
import type { IDraftStore } from '../../stores/ports/DraftStore.js';
import type { IMessageStore } from '../../stores/ports/MessageStore.js';
import type { ISessionChainStore } from '../../stores/ports/SessionChainStore.js';
import type { ITaskStore } from '../../stores/ports/TaskStore.js';
import type { IThreadStore, ThreadRoutingPolicyV1, ThreadRoutingScope } from '../../stores/ports/ThreadStore.js';
import { DEFAULT_THREAD_ID } from '../../stores/ports/ThreadStore.js';
import { expertConfigToAgentConfig, getExpertCatalog } from '../../experts/ExpertCatalog.js';
import type { IWorkflowSopStore } from '../../stores/ports/WorkflowSopStore.js';
import type { AgentMessage, AgentService } from '../../types.js';
import type { InvocationRegistry } from '../invocation/InvocationRegistry.js';
import type { TaskProgressStore } from '../invocation/TaskProgressStore.js';
import type { AgentRegistry } from '../registry/AgentRegistry.js';
import type { PersistenceContext, RouteStrategyDeps } from '../routing/route-helpers.js';
import { routeParallel } from '../routing/route-parallel.js';
import { routeSerial } from '../routing/route-serial.js';

const log = createModuleLogger('agent-router');

interface StructuredMentionRef {
  catId: AgentId;
  mention: string;
}

interface ParsedMentionToken {
  mention: string;
  position: number;
  agentIds: AgentId[];
}

/** Parsed mention with position for ordering */
interface ParsedMention {
  agentId: AgentId;
  position: number;
}

/**
 * Build mention aliases and speech regex from the current agent configs.
 * Must be called after officeClawRegistry is populated (not at module load time).
 */
function buildMentionData(configs: Record<string, import('@openjiuwen/relay-shared').OfficeClawConfigEntry>) {
  const mentionAliases = Array.from(
    new Set(
      Object.values(configs).flatMap((config) => config.mentionPatterns.map((pattern) => pattern.replace(/^@/, ''))),
    ),
  ).sort((a, b) => b.length - a.length);

  const speechMentionRe = new RegExp(
    [
      '(^|\\s)',
      '(?:at|艾特|@\\s*[。｡\\.．])',
      '\\s*(?:咱的|我的)?\\s*',
      `(${mentionAliases.map(escapeRegExp).join('|')})`,
      '(?=$|\\s|[，。！？、,.:：;；])',
    ].join(''),
    'gi',
  );

  return { mentionAliases, speechMentionRe };
}

/**
 * F042: Infer routing scope from message text (v1).
 * Intentionally deterministic and conservative.
 */
function inferRoutingScope(message: string): ThreadRoutingScope | null {
  const lower = message.toLowerCase();
  const hasPrToken = /\bpr\b/i.test(lower);

  // Review-ish cues
  if (
    lower.includes('review') ||
    lower.includes('lgtm') ||
    lower.includes('merge') ||
    hasPrToken ||
    message.includes('合入') ||
    message.includes('开 PR') ||
    message.includes('云端 review') ||
    message.includes('帮我看看') ||
    message.includes('请 reviewer 看看') ||
    message.includes('请 review')
  ) {
    return 'review';
  }

  // Architecture-ish cues
  if (
    lower.includes('architecture') ||
    lower.includes('tradeoff') ||
    message.includes('架构') ||
    message.includes('设计') ||
    message.includes('方案')
  ) {
    return 'architecture';
  }

  return null;
}

/**
 * Options for AgentRouter constructor
 */
export interface AgentRouterOptions {
  agentRegistry: AgentRegistry;
  catalogProvider: CatalogProvider;
  registry: InvocationRegistry;
  messageStore: IMessageStore;
  /** F045 Gap #4: Redis-backed task progress snapshots */
  taskProgressStore?: TaskProgressStore;
  sessionStore?: SessionStore;
  deliveryCursorStore?: DeliveryCursorStore;
  threadStore?: IThreadStore;
  /** F24: Session chain store for context health tracking */
  sessionChainStore?: ISessionChainStore;
  /** F24 Phase C: Transcript writer for event recording */
  transcriptWriter?: TranscriptWriter;
  /** F24 Phase D: Transcript reader for bootstrap injection */
  transcriptReader?: TranscriptReader;
  /** F24 Phase B: Session sealer for auto-seal */
  sessionSealer?: ISessionSealer;
  /** #80: Streaming draft persistence store */
  draftStore?: IDraftStore;
  /** F065: Task store for bootstrap task snapshot injection */
  taskStore?: ITaskStore;
  /** F073 P4: Workflow SOP store for stage hint injection */
  workflowSopStore?: IWorkflowSopStore;
  /** F070 Phase 3a: Execution digest store for dispatch backflow */
  executionDigestStore?: import('../../../../projects/execution-digest-store.js').ExecutionDigestStore;
  /** F079 Bug 2: Socket manager for real-time vote result broadcast */
  socketManager?: import('../../../../../infrastructure/websocket/SocketManager.js').SocketManager;
  /** F089 Phase 2: tmux gateway for agent-in-pane execution */
  tmuxGateway?: import('../../../../terminal/tmux-gateway.js').TmuxGateway;
  /** F089 Phase 2: agent pane registry for observability */
  agentPaneRegistry?: import('../../../../terminal/agent-pane-registry.js').AgentPaneRegistry;
}

/**
 * Router that parses @ mentions and routes to appropriate agent services
 */
export class AgentRouter {
  private services: Record<string, AgentService>;
  private registry: InvocationRegistry;
  private catalogProvider: CatalogProvider;
  private messageStore: IMessageStore;
  private sessionManager: SessionManager;
  private deliveryCursorStore: DeliveryCursorStore;
  private threadStore: IThreadStore | null;
  private sessionChainStore: ISessionChainStore | undefined;
  private transcriptWriter: TranscriptWriter | undefined;
  private transcriptReader: TranscriptReader | undefined;
  private sessionSealer: ISessionSealer | undefined;
  private draftStore: IDraftStore | undefined;
  private taskProgressStore: TaskProgressStore | undefined;
  private taskStore: ITaskStore | undefined;
  private workflowSopStore: IWorkflowSopStore | undefined;
  private executionDigestStore:
    | import('../../../../projects/execution-digest-store.js').ExecutionDigestStore
    | undefined;
  private socketManager: import('../../../../../infrastructure/websocket/SocketManager.js').SocketManager | undefined;
  private tmuxGateway: import('../../../../terminal/tmux-gateway.js').TmuxGateway | undefined;
  private agentPaneRegistry: import('../../../../terminal/agent-pane-registry.js').AgentPaneRegistry | undefined;
  private speechMentionRe: RegExp;

  private rebuildRuntimeCaches(agentRegistry: AgentRegistry): void {
    this.services = {};
    for (const [agentId, service] of agentRegistry.getAllEntries()) {
      this.services[agentId] = service;
    }
    const allConfigs = officeClawRegistry.getAllConfigs();
    const { speechMentionRe } = buildMentionData(allConfigs);
    this.speechMentionRe = speechMentionRe;
  }

  constructor(options: AgentRouterOptions) {
    this.services = {};
    this.speechMentionRe = /$^/;
    this.rebuildRuntimeCaches(options.agentRegistry);

    this.registry = options.registry;
    this.catalogProvider = options.catalogProvider;
    this.messageStore = options.messageStore;
    this.sessionManager = new SessionManager(options.sessionStore);
    this.deliveryCursorStore = options.deliveryCursorStore ?? new DeliveryCursorStore(options.sessionStore);
    this.threadStore = options.threadStore ?? null;
    this.sessionChainStore = options.sessionChainStore;
    this.transcriptWriter = options.transcriptWriter;
    this.transcriptReader = options.transcriptReader;
    this.sessionSealer = options.sessionSealer;
    this.draftStore = options.draftStore;
    this.taskProgressStore = options.taskProgressStore;
    this.taskStore = options.taskStore;
    this.workflowSopStore = options.workflowSopStore;
    this.executionDigestStore = options.executionDigestStore;
    this.socketManager = options.socketManager;
    this.tmuxGateway = options.tmuxGateway;
    this.agentPaneRegistry = options.agentPaneRegistry;
  }

  refreshFromRegistry(agentRegistry: AgentRegistry): void {
    this.rebuildRuntimeCaches(agentRegistry);
  }

  private isRoutableCat(agentId: string | null | undefined): agentId is AgentId {
    return typeof agentId === 'string' && Object.hasOwn(this.services, agentId) && isAgentAvailable(agentId);
  }

  private async getThreadScopedVisibility(
    threadId: string,
    identity?: GatewayIdentity,
  ): Promise<{ visibleAgentIds: Set<string> | null; mentionConfigs: Record<string, OfficeClawConfigEntry> }> {
    let visibleAgentIds: Set<string> | null;
    let mentionConfigs: Record<string, OfficeClawConfigEntry>;

    if (!identity) {
      visibleAgentIds = null;
      mentionConfigs = officeClawRegistry.getAllConfigs();
    } else {
      const members = await this.catalogProvider.listRoutableMembers?.(identity);
      if (members) {
        visibleAgentIds = new Set(members.map((member) => member.agentId));
        mentionConfigs = Object.fromEntries(members.map((member) => [member.agentId, member.config]));
      } else {
        const { catalog } = await this.catalogProvider.readCatalog(identity);
        mentionConfigs = toAllAgentConfigs(catalog);
        visibleAgentIds = new Set(Object.keys(mentionConfigs));
      }
    }

    // 始终获取受邀专家并合并到可见集合（不管有无 identity）
    const catalog = getExpertCatalog();
    if (catalog.isInitialized && this.threadStore) {
      const invitedExperts = await this.threadStore.getInvitedExperts(threadId);
      if (Array.isArray(invitedExperts) && invitedExperts.length > 0) {
        // 确保 visibleAgentIds 不为 null（无 identity 时初始为 null，合并时创建 Set）
        if (visibleAgentIds === null) {
          visibleAgentIds = new Set<string>();
        }
        for (const expertId of invitedExperts) {
          if (!catalog.isExpert(expertId)) continue;
          // 合并专家配置（如果已存在则不覆盖）
          if (!mentionConfigs[expertId]) {
            const config = officeClawRegistry.tryGet(expertId)?.config ?? expertConfigToAgentConfig(catalog.getExpert(expertId)!);
            mentionConfigs[expertId] = config;
          }
          visibleAgentIds.add(expertId);
        }
      }
    }

    return { visibleAgentIds, mentionConfigs };
  }

  private filterRoutableCats(agentIds: Iterable<string | null | undefined>, visibleAgentIds?: Set<string> | null): AgentId[] {
    const filtered: AgentId[] = [];
    const seen = new Set<string>();
    for (const agentId of agentIds) {
      if (!this.isRoutableCat(agentId)) continue;
      if (visibleAgentIds && !visibleAgentIds.has(agentId)) continue;
      if (seen.has(agentId)) continue;
      seen.add(agentId);
      filtered.push(agentId);
    }
    return filtered;
  }

  private async getLastRoutableAgentMessageCat(
    threadId: string,
    preferredSet: ReadonlySet<string>,
    visibleAgentIds?: Set<string> | null,
  ): Promise<AgentId | null> {
    try {
      const recent = await this.messageStore.getByThread(threadId, 20);
      for (let i = recent.length - 1; i >= 0; i--) {
        const agentId = recent[i]?.agentId;
        if (
          this.isRoutableCat(agentId) &&
          (!visibleAgentIds || visibleAgentIds.has(agentId)) &&
          (preferredSet.size === 0 || preferredSet.has(agentId))
        ) {
          return agentId;
        }
      }
    } catch (err) {
      log.warn({ threadId, err }, '[AgentRouter] failed to inspect last agent message for no-mention routing');
    }
    return null;
  }

  private async resolveImplicitTargetsFromThread(
    threadId: string,
    message: string,
    visibleAgentIds?: Set<string> | null,
  ): Promise<AgentId[]> {
    if (!this.threadStore) return [getDefaultAgentId()];

    const thread = await this.threadStore.get(threadId);

    // F32-b Phase 2 + #58: preferredCats = candidate scope, not dispatch list
    // R5: Object.hasOwn + dedupe; Cloud P1: Array.isArray guard for corrupted data
    const rawPref = Array.isArray(thread?.preferredCats) ? thread.preferredCats : [];
    const validPreferred = this.filterRoutableCats(rawPref, visibleAgentIds);
    const preferredSet = new Set(validPreferred.map(String));

    // #58: explicit #ideate with multiple preferred cats → dispatch all (user requested parallel)
    const hasExplicitIdeate = /#ideate\b/i.test(message);
    if (hasExplicitIdeate && validPreferred.length > 1) {
      return this.applyThreadRoutingPolicy(thread, message, validPreferred, visibleAgentIds);
    }

    // F078 + #58: last-replier takes priority, scoped to preferred cats when set
    const participantsWithActivity = await this.threadStore.getParticipantsWithActivity(threadId);
    if (participantsWithActivity.length > 0) {
      const lastReplier = participantsWithActivity[0]?.agentId;
      if (
        this.isRoutableCat(lastReplier) &&
        (!visibleAgentIds || visibleAgentIds.has(lastReplier as string)) &&
        (preferredSet.size === 0 || preferredSet.has(lastReplier as string))
      ) {
        return this.applyThreadRoutingPolicy(thread, message, [lastReplier], visibleAgentIds);
      }
    }

    // Hardening: callback-origin agent messages can be visible before activity state is present.
    // Prefer the latest visible agent message in this thread before falling back to preferred/default.
    const lastMessageCat = await this.getLastRoutableAgentMessageCat(threadId, preferredSet, visibleAgentIds);
    if (lastMessageCat) {
      return this.applyThreadRoutingPolicy(thread, message, [lastMessageCat], visibleAgentIds);
    }

    // No last-replier (or last-replier not in preferred set): use first preferred agent
    if (validPreferred.length > 0) {
      return this.applyThreadRoutingPolicy(thread, message, [validPreferred[0]], visibleAgentIds);
    }

    return this.applyThreadRoutingPolicy(thread, message, [getDefaultAgentId()], visibleAgentIds);
  }

  /** Pick a deterministic fallback agent when policy filters out all candidates. */
  private pickFallbackCat(exclude: Set<string>, visibleAgentIds?: Set<string> | null): AgentId | null {
    const def = getDefaultAgentId() as string;
    if ((!visibleAgentIds || visibleAgentIds.has(def)) && !exclude.has(def) && this.isRoutableCat(def)) return def as AgentId;

    for (const id of Object.keys(this.services).sort()) {
      if (visibleAgentIds && !visibleAgentIds.has(id)) continue;
      if (!exclude.has(id) && this.isRoutableCat(id)) return id as AgentId;
    }
    return null;
  }

  /** Apply thread routingPolicy (if any) to a candidate target list. */
  private applyThreadRoutingPolicy(
    thread: { routingPolicy?: ThreadRoutingPolicyV1 } | null | undefined,
    message: string,
    candidates: AgentId[],
    visibleAgentIds?: Set<string> | null,
  ): AgentId[] {
    const routableCandidates = this.filterRoutableCats(candidates, visibleAgentIds);
    const scope = inferRoutingScope(message);
    if (!scope) {
      if (routableCandidates.length > 0) return routableCandidates;
      const fallback = this.pickFallbackCat(new Set(), visibleAgentIds);
      return fallback ? [fallback] : [];
    }

    const policy = thread?.routingPolicy;
    if (!policy || policy.v !== 1 || !policy.scopes) {
      if (routableCandidates.length > 0) return routableCandidates;
      const fallback = this.pickFallbackCat(new Set(), visibleAgentIds);
      return fallback ? [fallback] : [];
    }

    const rule = policy.scopes[scope];
    if (!rule) {
      if (routableCandidates.length > 0) return routableCandidates;
      const fallback = this.pickFallbackCat(new Set(), visibleAgentIds);
      return fallback ? [fallback] : [];
    }
    if (typeof rule.expiresAt === 'number' && rule.expiresAt > 0 && rule.expiresAt < Date.now()) {
      if (routableCandidates.length > 0) return routableCandidates;
      const fallback = this.pickFallbackCat(new Set(), visibleAgentIds);
      return fallback ? [fallback] : [];
    }

    // Defensive guard: data might be malformed from external persistence.
    const avoidList = Array.isArray(rule.avoidCats) ? rule.avoidCats : [];
    const preferList = Array.isArray(rule.preferCats) ? rule.preferCats : [];
    const avoid = new Set(avoidList.map((id) => String(id)));
    const prefer = preferList.map((id) => String(id)).filter((id) => !avoid.has(id));

    const filtered = routableCandidates.filter((id) => !avoid.has(id as string));
    const out: AgentId[] = [];
    const seen = new Set<string>();

    for (const id of prefer) {
      if (!this.isRoutableCat(id) || (visibleAgentIds && !visibleAgentIds.has(id))) continue;
      if (seen.has(id)) continue;
      seen.add(id);
      out.push(id as AgentId);
    }

    for (const id of filtered) {
      const sid = id as string;
      if (seen.has(sid)) continue;
      seen.add(sid);
      out.push(id);
    }

    if (out.length > 0) return out;

    const fallback = this.pickFallbackCat(avoid, visibleAgentIds);
    return fallback ? [fallback] : routableCandidates;
  }

  /** Normalize speech patterns like "at 布偶" → "@布偶" */
  private normalizeSpeechMentions(message: string): string {
    return message.replace(this.speechMentionRe, (_match, prefix: string, mention: string) => `${prefix}@${mention}`);
  }

  private collectMentionPatterns(
    configs: Record<string, OfficeClawConfigEntry>,
    visibleAgentIds?: Set<string> | null,
  ): Array<{ pattern: string; agentIds: AgentId[] }> {
    const patternMap = new Map<string, Set<AgentId>>();
    for (const config of Object.values(configs)) {
      if (visibleAgentIds && !visibleAgentIds.has(config.id)) continue;
      for (const pattern of config.mentionPatterns) {
        const normalized = pattern.toLowerCase();
        const existing = patternMap.get(normalized);
        if (existing) {
          existing.add(config.id);
        } else {
          patternMap.set(normalized, new Set([config.id]));
        }
      }
    }

    return Array.from(patternMap.entries())
      .map(([pattern, agentIds]) => ({ pattern, agentIds: Array.from(agentIds) }))
      .sort((a, b) => b.pattern.length - a.pattern.length);
  }

  private parseMentionTokens(
    message: string,
    configs: Record<string, OfficeClawConfigEntry>,
    visibleAgentIds?: Set<string> | null,
  ): ParsedMentionToken[] {
    const lowerMessage = this.normalizeSpeechMentions(message).toLowerCase();
    const allPatterns = this.collectMentionPatterns(configs, visibleAgentIds);
    const consumed: Array<[number, number]> = [];
    const tokens: ParsedMentionToken[] = [];

    for (const { pattern, agentIds } of allPatterns) {
      let searchFrom = 0;
      while (searchFrom < lowerMessage.length) {
        const pos = lowerMessage.indexOf(pattern, searchFrom);
        if (pos === -1) break;

        const end = pos + pattern.length;
        const charAfter = lowerMessage[end];
        const isEndBoundary = !charAfter || /[\s,.:;!?()[\]{}<>，。！？、：；（）【】《》「」『』〈〉]/.test(charAfter);
        const isConsumed = consumed.some(([s, e]) => pos >= s && pos < e);

        if (isEndBoundary && !isConsumed) {
          consumed.push([pos, end]);
          const routableAgents = agentIds.filter((agentId) => this.isRoutableCat(agentId));
          if (routableAgents.length > 0) {
            tokens.push({
              mention: lowerMessage.slice(pos, end),
              position: pos,
              agentIds: routableAgents,
            });
          }
        }

        searchFrom = pos + 1;
      }
    }

    tokens.sort((a, b) => a.position - b.position);
    return tokens;
  }

  /**
   * F32-b: Parse @mentions with longest-match-first + token boundary.
   * Prevents `@opus-45` from also matching `@opus` via consumed interval exclusion.
   *
   * Algorithm:
   * 1. Collect ALL patterns from ALL cats, sort by length descending (longest first)
   * 2. For each pattern, find all occurrences in the message
   * 3. Check token boundary (char after pattern must be whitespace/punctuation/EOF)
   * 4. Check consumed intervals (skip if already matched by a longer pattern)
   * 5. Deduplicate by agentId, preserve first-occurrence ordering
   */
  private parseMentions(
    message: string,
    configs: Record<string, OfficeClawConfigEntry>,
    visibleAgentIds?: Set<string> | null,
  ): AgentId[] {
    const tokens = this.parseMentionTokens(message, configs, visibleAgentIds);
    const mentions: ParsedMention[] = [];
    const seenCats = new Set<string>();

    for (const token of tokens) {
      for (const agentId of token.agentIds) {
        if (!seenCats.has(agentId as string)) {
          seenCats.add(agentId as string);
          mentions.push({ agentId, position: token.position });
        } else {
          const existing = mentions.find((m) => m.agentId === agentId);
          if (existing && token.position < existing.position) {
            existing.position = token.position;
          }
        }
      }
    }

    mentions.sort((a, b) => a.position - b.position);
    return mentions.map((m) => m.agentId);
  }

  /**
   * F078: Parse group mentions (@all, @全体, @全体{breed}, @all-{breed}, @thread, @本帖, @全体参与者).
   * Returns matched AgentIds or null if no group mention found.
   * Called BEFORE individual parseMentions — group patterns are longer and take priority.
   *
   * P1 fix: uses token boundary check (same regex as parseMentions) to avoid
   * substring collisions like @allison→@all or @threadsafe→@thread.
   */
  private async parseGroupMentions(
    message: string,
    threadId: string,
    configs: Record<string, OfficeClawConfigEntry>,
    visibleAgentIds?: Set<string> | null,
  ): Promise<AgentId[] | null> {
    const lowerMessage = this.normalizeSpeechMentions(message).toLowerCase();

    // Reuse parseMentions' token boundary regex
    const boundaryRe = /[\s,.:;!?()[\]{}<>，。！？、：；（）【】《》「」『』〈〉]/;

    /** Check if pattern appears in message with a valid token boundary after it */
    const matchesWithBoundary = (pattern: string): boolean => {
      const lowerPattern = pattern.toLowerCase();
      let searchFrom = 0;
      while (searchFrom < lowerMessage.length) {
        const pos = lowerMessage.indexOf(lowerPattern, searchFrom);
        if (pos === -1) return false;
        const end = pos + lowerPattern.length;
        const charAfter = lowerMessage[end];
        if (!charAfter || boundaryRe.test(charAfter)) return true;
        searchFrom = pos + 1;
      }
      return false;
    };

    // Build all group patterns sorted longest-first for correct priority
    interface GroupPattern {
      pattern: string;
      resolve: () => Promise<AgentId[] | null>;
    }
    const patterns: GroupPattern[] = [];

    // Thread-scoped patterns
    for (const pattern of ['@全体参与者', '@thread', '@本帖']) {
      patterns.push({
        pattern,
        resolve: async () => {
          if (this.threadStore) {
            const participants = await this.threadStore.getParticipants(threadId);
            const valid = this.filterRoutableCats(participants, visibleAgentIds);
            if (valid.length > 0) return valid as AgentId[];
          }
          const fallback = this.pickFallbackCat(new Set(), visibleAgentIds);
          return fallback ? [fallback] : [];
        },
      });
    }

    // Breed-scoped patterns: @全体{displayName} and @all-{breedId}
    const breedMap = new Map<string, { displayName: string; agentIds: AgentId[] }>();
    for (const [agentId, config] of Object.entries(configs)) {
      if (!config.breedId) continue;
      if (!Object.hasOwn(this.services, agentId)) continue;
      const existing = breedMap.get(config.breedId);
      if (existing) {
        existing.agentIds.push(agentId as AgentId);
      } else {
        breedMap.set(config.breedId, {
          displayName: config.breedDisplayName ?? config.displayName,
          agentIds: [agentId as AgentId],
        });
      }
    }
    for (const [breedId, info] of breedMap) {
      const agentIds = info.agentIds;
      patterns.push({
        pattern: `@全体${info.displayName}`,
        resolve: async () => this.filterRoutableCats(agentIds, visibleAgentIds),
      });
      patterns.push({ pattern: `@all-${breedId}`, resolve: async () => this.filterRoutableCats(agentIds, visibleAgentIds) });
    }

    // Global @all / @全体 (shortest — must be last)
    patterns.push({
      pattern: '@全体',
      resolve: async () => {
        const allCats = this.filterRoutableCats(Object.keys(this.services), visibleAgentIds);
        if (allCats.length > 0) return allCats;
        const fallback = this.pickFallbackCat(new Set(), visibleAgentIds);
        return fallback ? [fallback] : [];
      },
    });
    patterns.push({
      pattern: '@all',
      resolve: async () => {
        const allCats = this.filterRoutableCats(Object.keys(this.services), visibleAgentIds);
        if (allCats.length > 0) return allCats;
        const fallback = this.pickFallbackCat(new Set(), visibleAgentIds);
        return fallback ? [fallback] : [];
      },
    });

    // Sort longest-first to avoid prefix collisions (@全体Claude before @全体)
    patterns.sort((a, b) => b.pattern.length - a.pattern.length);

    for (const { pattern, resolve } of patterns) {
      if (matchesWithBoundary(pattern)) {
        return resolve();
      }
    }

    return null;
  }

  /**
   * F078: Unified mention parser — group mentions first, then individual.
   */
  private async parseAllMentions(
    message: string,
    threadId: string,
    configs: Record<string, OfficeClawConfigEntry>,
    mentionRefs?: readonly StructuredMentionRef[],
    visibleAgentIds?: Set<string> | null,
  ): Promise<AgentId[]> {
    const groupResult = await this.parseGroupMentions(message, threadId, configs, visibleAgentIds);
    if (groupResult !== null) return groupResult;

    const tokens = this.parseMentionTokens(message, configs, visibleAgentIds);
    if (tokens.length === 0) return [];

    const mentionRefQueues = new Map<string, StructuredMentionRef[]>();
    const mentionTokenCounts = new Map<string, number>();
    for (const token of tokens) {
      mentionTokenCounts.set(token.mention, (mentionTokenCounts.get(token.mention) ?? 0) + 1);
    }

    for (const ref of mentionRefs ?? []) {
      const mention = this.normalizeSpeechMentions(ref.mention).toLowerCase().trim();
      if (!mention || !mention.startsWith('@')) continue;
      const queue = mentionRefQueues.get(mention);
      if (queue) queue.push(ref);
      else mentionRefQueues.set(mention, [ref]);
    }
    for (const [mention, queue] of mentionRefQueues) {
      const allowedCount = mentionTokenCounts.get(mention) ?? 0;
      if (queue.length <= allowedCount) continue;
      mentionRefQueues.set(mention, queue.slice(queue.length - allowedCount));
    }

    const mentions: ParsedMention[] = [];
    const seenAgents = new Set<string>();

    for (const token of tokens) {
      const refQueue = mentionRefQueues.get(token.mention);
      const hintedRefIndex = refQueue?.findIndex((ref) => token.agentIds.includes(ref.catId)) ?? -1;
      const hintedAgentId = hintedRefIndex >= 0 && refQueue ? refQueue[hintedRefIndex]?.catId : undefined;
      const resolvedAgents =
        hintedAgentId && token.agentIds.length > 1 && token.agentIds.includes(hintedAgentId) ? [hintedAgentId] : token.agentIds;

      if (refQueue && hintedRefIndex >= 0 && resolvedAgents.length === 1 && resolvedAgents[0] === hintedAgentId) {
        refQueue.splice(hintedRefIndex, 1);
      }

      for (const agentId of resolvedAgents) {
        if (!seenAgents.has(agentId)) {
          seenAgents.add(agentId);
          mentions.push({ agentId, position: token.position });
        } else {
          const existing = mentions.find((m) => m.agentId === agentId);
          if (existing && token.position < existing.position) {
            existing.position = token.position;
          }
        }
      }
    }

    if (mentions.length === 0) return [];

    // 专家已在 getThreadScopedVisibility 中合并到 visibleAgentIds，此处不再需要事后过滤
    mentions.sort((a, b) => a.position - b.position);
    return mentions.map((m) => m.agentId);
  }

  /**
   * Read-only target resolution: mentions → last-replier (scoped to preferredCats) → default agent.
   * F32-b Phase 2 + #58: preferredCats is a candidate scope, not a dispatch list.
   * Does NOT mutate thread participants.
   */
  private async peekTargets(
    message: string,
    threadId: string,
    configs: Record<string, OfficeClawConfigEntry>,
    mentionRefs?: readonly StructuredMentionRef[],
    visibleAgentIds?: Set<string> | null,
  ): Promise<AgentId[]> {
    const mentionedCats = await this.parseAllMentions(message, threadId, configs, mentionRefs, visibleAgentIds);
    if (mentionedCats.length > 0) return mentionedCats;

    return this.resolveImplicitTargetsFromThread(threadId, message, visibleAgentIds);
  }

  /** Resolve target cats and persist new mentions as thread participants */
  private async resolveTargets(
    message: string,
    threadId: string,
    configs: Record<string, OfficeClawConfigEntry>,
    mentionRefs?: readonly StructuredMentionRef[],
    visibleAgentIds?: Set<string> | null,
  ): Promise<AgentId[]> {
    const mentionedCats = await this.parseAllMentions(message, threadId, configs, mentionRefs, visibleAgentIds);

    if (mentionedCats.length > 0) {
      if (this.threadStore) {
        await this.threadStore.addParticipants(threadId, mentionedCats);
      }
      return mentionedCats;
    }

    return this.resolveImplicitTargetsFromThread(threadId, message, visibleAgentIds);
  }

  /** Build shared strategy dependencies (public for ModeOrchestrator) */
  getStrategyDeps(): RouteStrategyDeps {
    const apiPort = process.env.API_SERVER_PORT ?? '3004';
    return {
      services: this.services,
      invocationDeps: {
        registry: this.registry,
        sessionManager: this.sessionManager,
        catalogProvider: this.catalogProvider,
        threadStore: this.threadStore,
        apiUrl: `http://127.0.0.1:${apiPort}`,
        ...(this.taskProgressStore ? { taskProgressStore: this.taskProgressStore } : {}),
        ...(this.sessionChainStore ? { sessionChainStore: this.sessionChainStore } : {}),
        ...(this.transcriptWriter ? { transcriptWriter: this.transcriptWriter } : {}),
        ...(this.transcriptReader ? { transcriptReader: this.transcriptReader } : {}),
        ...(this.sessionSealer ? { sessionSealer: this.sessionSealer } : {}),
        ...(this.taskStore ? { taskStore: this.taskStore } : {}),
        ...(this.workflowSopStore ? { workflowSopStore: this.workflowSopStore } : {}),
        ...(this.executionDigestStore ? { executionDigestStore: this.executionDigestStore } : {}),
        ...(this.tmuxGateway ? { tmuxGateway: this.tmuxGateway } : {}),
        ...(this.agentPaneRegistry ? { agentPaneRegistry: this.agentPaneRegistry } : {}),
      },
      messageStore: this.messageStore,
      deliveryCursorStore: this.deliveryCursorStore,
      ...(this.draftStore ? { draftStore: this.draftStore } : {}),
      ...(this.socketManager ? { socketManager: this.socketManager } : {}),
    };
  }

  /**
   * Resolve targets and intent.
   * Default: read-only peek (safe to call before route()).
   * With persist: true, also writes @mentions to thread participants.
   */
  async resolveTargetsAndIntent(
    message: string,
    threadId?: string,
    options?: { persist?: boolean; identity?: GatewayIdentity; mentionRefs?: readonly StructuredMentionRef[] },
  ): Promise<{ targetAgents: AgentId[]; intent: IntentResult; configByAgentId: Record<string, OfficeClawConfigEntry> }> {
    const resolvedThreadId = threadId ?? DEFAULT_THREAD_ID;
    const { visibleAgentIds, mentionConfigs } = await this.getThreadScopedVisibility(resolvedThreadId, options?.identity);
    const targetAgents = options?.persist
      ? await this.resolveTargets(message, resolvedThreadId, mentionConfigs, options?.mentionRefs, visibleAgentIds)
      : await this.peekTargets(message, resolvedThreadId, mentionConfigs, options?.mentionRefs, visibleAgentIds);
    const intent = parseIntent(message, targetAgents.length);
    log.debug(
      {
        threadId: resolvedThreadId,
        targetAgents,
        intent: intent.intent,
        explicit: intent.explicit,
        persist: options?.persist,
        hasAtSign: message.includes('@'),
        contentLen: message.length,
      },
      '[AgentRouter] resolveTargetsAndIntent result',
    );
    return { targetAgents, intent, configByAgentId: mentionConfigs };
  }

  /**
   * Route message to appropriate agent(s) based on @ mentions and thread participants.
   * @deprecated Use routeExecution() instead — route() couples message writing with execution.
   *             Will be removed after S4 migration is complete.
   */
  async *route(
    userId: string,
    message: string,
    threadId?: string,
    contentBlocks?: readonly MessageContent[],
    uploadDir?: string,
    signal?: AbortSignal,
    options?: {
      modeSystemPrompt?: string;
      modeSystemPromptByCat?: Record<string, string>;
      gatewayIdentity?: GatewayIdentity;
    },
  ): AsyncIterable<AgentMessage> {
    const resolvedThreadId = threadId ?? DEFAULT_THREAD_ID;
    const { visibleAgentIds, mentionConfigs } = await this.getThreadScopedVisibility(
      resolvedThreadId,
      options?.gatewayIdentity,
    );
    const targetAgents = await this.resolveTargets(message, resolvedThreadId, mentionConfigs, undefined, visibleAgentIds);
    const intent = parseIntent(message, targetAgents.length);
    const cleanMessage = stripIntentTags(message);

    // Fetch thread for thinkingMode + update lastActive
    // Default to play mode when no threadStore is available: stream thinking stays isolated.
    let legacyThinkingMode: 'debug' | 'play' = 'play';
    if (this.threadStore) {
      const thread = await this.threadStore.get(resolvedThreadId);
      if (thread) {
        legacyThinkingMode = thread.thinkingMode ?? 'play';
      }
      await this.threadStore.updateLastActive(resolvedThreadId);
    }

    const storedUserMessage = await this.messageStore.append({
      userId,
      agentId: null,
      content: message, // Store original (with tags) for audit
      mentions: targetAgents,
      timestamp: Date.now(),
      threadId: resolvedThreadId,
      ...(contentBlocks ? { contentBlocks } : {}),
    });

    const strategyDeps = this.getStrategyDeps();
    const routeOptions = {
      contentBlocks,
      uploadDir,
      signal,
      promptTags: intent.promptTags,
      currentUserMessageId: storedUserMessage.id,
      thinkingMode: legacyThinkingMode,
      configByAgentId: mentionConfigs,
      ...(options?.gatewayIdentity ? { gatewayIdentity: options.gatewayIdentity } : {}),
      ...(options?.modeSystemPrompt ? { modeSystemPrompt: options.modeSystemPrompt } : {}),
      ...(options?.modeSystemPromptByCat ? { modeSystemPromptByCat: options.modeSystemPromptByCat } : {}),
    };

    if (intent.intent === 'ideate' && targetAgents.length > 1) {
      yield* routeParallel(strategyDeps, targetAgents, cleanMessage, userId, resolvedThreadId, routeOptions);
    } else {
      yield* routeSerial(strategyDeps, targetAgents, cleanMessage, userId, resolvedThreadId, routeOptions);
    }
  }

  /**
   * Execute agent invocation without writing the user message (ADR-008 S1).
   * Message writing is decoupled — the caller writes the message and passes its ID.
   *
   * @param userMessageId - ID of the already-stored user message
   * @param targetAgents - pre-resolved target cats (from resolveTargets)
   * @param intent - pre-parsed intent result
   */
  async *routeExecution(
    userId: string,
    message: string,
    threadId: string,
    userMessageId: string,
    targetAgents: AgentId[],
    intent: IntentResult,
    options?: {
      contentBlocks?: readonly MessageContent[];
      uploadDir?: string;
      callbackEnvOverrides?: Record<string, string>;
      signal?: AbortSignal;
      queueHasQueuedMessages?: (threadId: string) => boolean;
      hasQueuedOrActiveAgent?: (threadId: string, agentId: string) => boolean;
      modeSystemPrompt?: string;
      modeSystemPromptByCat?: Record<string, string>;
      /** ADR-008 S3: pass a Map to collect cursor boundaries; caller acks after succeeded */
      cursorBoundaries?: Map<string, string>;
      /** P1-2: pass to track persistence failures across generator boundary */
      persistenceContext?: PersistenceContext;
      /** F108: parentInvocationId for WorklistRegistry concurrent isolation */
      parentInvocationId?: string;
      gatewayIdentity?: GatewayIdentity;
      /** Explicit interrupted-session resume target for provider integrations that support resume semantics. */
      resumeAgentId?: AgentId;
      /** AskUserQuestion: whether the current channel supports interactive structured questions. */
      interactiveAsk?: boolean;
      /** End-to-end trace ID from frontend HTTP request for log correlation. */
      traceId?: string;
      /** Thread-scoped agent config view resolved alongside target selection. */
      configByAgentId?: Record<string, OfficeClawConfigEntry>;
    },
  ): AsyncIterable<AgentMessage> {
    const cleanMessage = stripIntentTags(message);
    const configByAgentId =
      options?.configByAgentId ?? (await this.getThreadScopedVisibility(threadId, options?.gatewayIdentity)).mentionConfigs;

    // Fetch thread for thinkingMode + update lastActive
    // Default to play mode when no threadStore is available: stream thinking stays isolated.
    let thinkingMode: 'debug' | 'play' = 'play';
    if (this.threadStore) {
      const thread = await this.threadStore.get(threadId);
      if (thread) {
        thinkingMode = thread.thinkingMode ?? 'play';
      }
      await this.threadStore.updateLastActive(threadId);
    }

    const strategyDeps = this.getStrategyDeps();
    const routeOptions = {
      contentBlocks: options?.contentBlocks,
      uploadDir: options?.uploadDir,
      callbackEnvOverrides: options?.callbackEnvOverrides,
      signal: options?.signal,
      queueHasQueuedMessages: options?.queueHasQueuedMessages,
      hasQueuedOrActiveAgent: options?.hasQueuedOrActiveAgent,
      modeSystemPrompt: options?.modeSystemPrompt,
      modeSystemPromptByCat: options?.modeSystemPromptByCat,
      promptTags: intent.promptTags,
      currentUserMessageId: userMessageId,
      thinkingMode,
      configByAgentId,
      ...(options?.cursorBoundaries ? { cursorBoundaries: options.cursorBoundaries } : {}),
      ...(options?.persistenceContext ? { persistenceContext: options.persistenceContext } : {}),
      ...(options?.parentInvocationId ? { parentInvocationId: options.parentInvocationId } : {}),
      ...(options?.gatewayIdentity ? { gatewayIdentity: options.gatewayIdentity } : {}),
      ...(options?.interactiveAsk ? { interactiveAsk: true } : {}),
      ...(options?.resumeAgentId ? { resumeAgentId: options.resumeAgentId } : {}),
      ...(options?.traceId ? { traceId: options.traceId } : {}),
    };

    if (intent.intent === 'ideate' && targetAgents.length > 1) {
      yield* routeParallel(strategyDeps, targetAgents, cleanMessage, userId, threadId, routeOptions);
    } else {
      yield* routeSerial(strategyDeps, targetAgents, cleanMessage, userId, threadId, routeOptions);
    }
  }

  /**
   * ADR-008 S3: Ack all cursor boundaries collected during execution.
   * Call ONLY after InvocationRecord.status = 'succeeded'.
   */
  async ackCollectedCursors(userId: string, threadId: string, boundaries: Map<string, string>): Promise<void> {
    for (const [agentId, boundaryId] of boundaries) {
      try {
        await this.deliveryCursorStore.ackCursor(userId, agentId as AgentId, threadId, boundaryId);
      } catch (err) {
        log.error({ agentId, err }, `[ackCollectedCursors] failed`);
      }
    }
  }
}
