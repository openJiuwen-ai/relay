...
/**
 * Build bootstrap context for an agent's current session.
 * Returns null if agent is on Session #1 (no prior context to inject).
 */
export async function buildSessionBootstrap(
  ...
  const chain = await sessionChainStore.getChain(agentId, threadId);
  ...
  const active = await sessionChainStore.getActive(agentId, threadId);
  ...
  try {
    if (opts.bootstrapDepth === 'generative') {
      ...
      const handoff = await transcriptReader.readHandoffDigest(prevSession.id, prevSession.threadId, prevSession.agentId);
      if (handoff) {
    ...
    if (!hasDigest) {
      const digest = await transcriptReader.readDigest(prevSession.id, prevSession.threadId, prevSession.agentId);
      if (digest) {
