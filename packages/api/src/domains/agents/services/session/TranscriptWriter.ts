 ...
 *   <dataDir>/threads/<threadId>/<agentId>/sessions/<sessionId>/
 ...
 *   { v:1, t:number, threadId, agentId, sessionId, cliSessionId, invocationId?, eventNo, event }
 */
...
export class TranscriptWriter {
  ...
  private sessionDir(session: TranscriptSessionInfo): string {
    return join(this.dataDir, 'threads', session.threadId, session.agentId, 'sessions', session.sessionId);
  }
}
