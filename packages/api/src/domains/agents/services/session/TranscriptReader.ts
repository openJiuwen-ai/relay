...
export class TranscriptReader {
  ...
  async readEvents(
    ...
    const sessionDir = this.sessionDir(threadId, agentId, sessionId);
   ...
   */
  async readDigest(sessionId: string, threadId: string, agentId: string): Promise<Record<string, unknown> | null> {
    const digestPath = join(this.sessionDir(threadId, agentId, sessionId), 'digest.extractive.json');
    try {
  ...
  async search(
    ...
    try {
      agentDirs = await readdir(threadDir);
    } catch {
      return [];
    }

    for (const agentId of agentDirs) {
      if (opts?.agents && !opts.agents.includes(agentId)) continue;

      const sessionsDir = join(threadDir, agentId, 'sessions');
   ...
   */
  async readHandoffDigest(sessionId: string, threadId: string, agentId: string): Promise<HandoffDigestResult | null> {
    const digestPath = join(this.sessionDir(threadId, agentId, sessionId), 'digest.handoff.md');
    try {
      const content = await readFile(digestPath, 'utf-8');
   ...
   */
  async readAllEvents(sessionId: string, threadId: string, agentId: string): Promise<TranscriptEvent[]> {
    const jsonlPath = join(this.sessionDir(threadId, agentId, sessionId), 'events.jsonl');
  ...
  getSessionDir(threadId: string, agentId: string, sessionId: string): string {
    return this.sessionDir(threadId, agentId, sessionId);
  }

  /** Check if a session has a transcript on disk. */
  async hasTranscript(sessionId: string, threadId: string, agentId: string): Promise<boolean> {
    try {
      await stat(join(this.sessionDir(threadId, agentId, sessionId), 'events.jsonl'));
      return true;
    } catch {
      return false;
    }
  ...
  private sessionDir(threadId: string, agentId: string, sessionId: string): string {
    return join(this.dataDir, 'threads', threadId, agentId, 'sessions', sessionId);
  }
