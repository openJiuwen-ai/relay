...
export async function backfillBoundSessionHistory(
  ...
  const { sessionChainStore, transcriptReader, messageStore, threadId, agentId, userId } = opts;
  ...
  try {
    ...
    for (const session of sealedSessions) {
      const hasTranscript = await transcriptReader.hasTranscript(session.id, threadId, session.agentId);
      if (!hasTranscript) continue;
      foundTranscript = true;

      const events = await transcriptReader.readAllEvents(session.id, threadId, session.agentId);
    ...
    console.error('[BoundSessionHistoryImporter] import failed:', err);
...
function mapTranscriptEventToMessage(
  ...
  const base: AppendMessageInput = {
    ...
    agentId: evtType === 'user' ? null : session.agentId,
    content,
    mentions: [],
    timestamp: evt.t,
