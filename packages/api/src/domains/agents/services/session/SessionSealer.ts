...
import type { AgentId, SealResult, SessionStatus } from '@openjiuwen/relay-shared';
...
export interface HandoffConfig {
  getBootstrapDepth: (agentId: string) => 'extractive' | 'generative';
  resolveProfile: (threadId: string, agentId: string) => Promise<{ apiKey: string; baseUrl: string } | null>;
...
export interface ISessionSealer {
  ...
  /**
   * F118 Hardening: Reconcile sessions stuck in 'sealing' state for a specific agent/thread.
   * Force-seals any session that has been in 'sealing' longer than maxAgeMs.
   * Returns count of reconciled sessions.
   */
  reconcileStuck(agentId: string, threadId: string, maxAgeMs?: number): Promise<number>;
...
export class SessionSealer implements ISessionSealer {
  constructor(
    ...
    private readonly getMaxPromptTokens?: (agentId: AgentId) => number,
   ...
   * Scans all sessions for the given agent/thread and force-seals any that have been
   * in 'sealing' longer than maxAgeMs. Returns count of reconciled sessions.
   */
  async reconcileStuck(agentId: string, threadId: string, maxAgeMs = 5 * 60_000): Promise<number> {
    const sessions = await this.store.getChain(agentId as AgentId, threadId);
    const now = Date.now();
    let count = 0;
  ...
  private async doFinalize(
    record: { id: string; threadId: string; agentId: string; cliSessionId: string; seq: number; createdAt: number },
    now: number,
  ): Promise<void> {
    ...
    if (this.threadStore && this.transcriptReader) {
      try {
        const digest = await this.transcriptReader.readDigest(record.id, record.threadId, record.agentId);
        if (digest) {
          ...
          const maxPrompt = this.getMaxPromptTokens?.(record.agentId as AgentId) ?? 180000;
    ...
    if (this.handoffConfig && this.transcriptReader) {
      try {
        const depth = this.handoffConfig.getBootstrapDepth(record.agentId);
        if (depth === 'generative') {
          const profile = await this.handoffConfig.resolveProfile(record.threadId, record.agentId);
          if (profile?.apiKey) {
            const allEvents = await this.transcriptReader.readAllEvents(record.id, record.threadId, record.agentId);
            if (allEvents.length > 0) {
              const handoffSummaries = formatEventsHandoff(allEvents);
              const chatMessages = formatEventsChat(allEvents);
              const extractive = await this.transcriptReader.readDigest(record.id, record.threadId, record.agentId);
              ...
              if (result) {
                const sessionDir = this.transcriptReader.getSessionDir(record.threadId, record.agentId, record.id);
