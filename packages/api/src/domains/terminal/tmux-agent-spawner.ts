...
import type { SpawnCliOverride } from '../agents/services/types.js';
...
export async function* spawnCliInTmux(
  ...
  const tmpDir = await mkdtemp(join(tmpdir(), `officeclaw-agent-${options.invocationId}-`));
 ...
 * Called per-invocation in invoke-single-agent.ts when tmux is available.
 */
