...
describe('formatMessage', () => {
  ...
  test('formats agent message with display name', async () => {
    const { formatMessage } = await import('../dist/domains/agents/services/context/ContextAssembler.js');
    const msg = mockMsg({ agentId: 'opus', content: '收到', timestamp: new Date('2026-02-07T14:03:00').getTime() });
    const result = formatMessage(msg);
  ...
  test('formats codex agent message', async () => {
    const { formatMessage } = await import('../dist/domains/agents/services/context/ContextAssembler.js');
    const msg = mockMsg({ agentId: 'codex', content: 'review done' });
    const result = formatMessage(msg);
  ...
  test('formats gemini agent message', async () => {
    const { formatMessage } = await import('../dist/domains/agents/services/context/ContextAssembler.js');
    const msg = mockMsg({ agentId: 'gemini', content: 'design ready' });
    const result = formatMessage(msg);
...
describe('assembleContext', () => {
  test('returns empty for no messages', async () => {
    const { assembleContext } = await import('../dist/domains/agents/services/context/ContextAssembler.js');
  ...
  test('formats mixed user and agent messages', async () => {
    const { assembleContext } = await import('../dist/domains/agents/services/context/ContextAssembler.js');
    const msgs = [
      mockMsg({ agentId: null, content: '@Claude 你好', timestamp: 1000 }),
      mockMsg({ agentId: 'opus', content: '你好用户', timestamp: 2000 }),
      mockMsg({ agentId: 'codex', content: '我也在', timestamp: 3000 }),
    ];
  ...
  test('includes user messages starting with "Error:" (no false-positive filtering)', async () => {
    const { assembleContext } = await import('../dist/domains/agents/services/context/ContextAssembler.js');
    const msgs = [
      mockMsg({ agentId: null, content: 'Error: Cannot find module "foo"', timestamp: 1000 }),
      mockMsg({ agentId: null, content: '正常消息', timestamp: 2000 }),
      mockMsg({ agentId: 'opus', content: '智能体回复', timestamp: 3000 }),
    ];
    const result = assembleContext(msgs);
    assert.equal(result.messageCount, 3);
  ...
  test('excludes userId=system messages from prompt context (error badge pollution)', async () => {
    const { assembleContext } = await import('../dist/domains/agents/services/context/ContextAssembler.js');
    const msgs = [
      mockMsg({ agentId: null, userId: 'user-1', content: '你好', timestamp: 1000 }),
      mockMsg({ agentId: null, userId: 'system', content: 'Error: stream_idle_stall: Gemini stopped', timestamp: 2000 }),
      mockMsg({ agentId: 'opus', userId: 'user-1', content: '智能体回复', timestamp: 3000 }),
    ];
    const result = assembleContext(msgs);
  ...
  test('respects MAX_CONTEXT_MSG_CHARS env var override', async () => {
    ...
    try {
      ...
      const { assembleContext: ac } = await import('../dist/domains/agents/services/context/ContextAssembler.js');
...
describe('cross-post sender variant: distinguish same-family cats', () => {
  before(async () => {
    const { officeClawRegistry } = await import('../node_modules/@openjiuwen/relay-shared/dist/index.js');
  ...
  test('formatMessage shows Claude(Sonnet) for sonnet agentId', async () => {
    const { formatMessage } = await import('../dist/domains/agents/services/context/ContextAssembler.js');
    const msg = mockMsg({
      agentId: 'sonnet',
    ...
    const result = formatMessage(msg);
    // Should show family name + variant, not just raw agentId
    ...
    assert.ok(
      ...
      `should not display raw agentId 'sonnet' as sender, got: ${result}`,
    );
  });

  test('formatMessage shows Claude(Opus 4.5) for opus-45 agentId', async () => {
    const { formatMessage } = await import('../dist/domains/agents/services/context/ContextAssembler.js');
    const msg = mockMsg({
      agentId: 'opus-45',
    ...
    assert.ok(
      ...
      `should not display raw agentId 'opus-45' as sender, got: ${result}`,
    );
  });
