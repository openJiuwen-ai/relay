...
describe('BacklogStore', () => {
  /** @type {import('../dist/domains/agents/services/stores/ports/BacklogStore.js').BacklogStore} */
  let store;
  let originalDateNow;
  let now;
  ...
  test('suggestClaim transitions open -> suggested', () => {
    ...
    assert.equal(suggested?.suggestion?.agentId, 'codex');
  ...
  test('lease lifecycle acquire -> heartbeat -> release', () => {
    ...
    assert.equal(acquired?.lease?.ownerAgentId, 'codex');
...
describe('BacklogStore markDone', () => {
  ...
  function createAndDispatch() {
    ...
    store.suggestClaim(item.id, { agentId: 'claude-opus', why: 'w', plan: 'p', requestedPhase: 'coding' });
...
describe('BacklogStore atomicDispatch', () => {
  ...
  beforeEach(async () => {
    ({ BacklogStore } = await import('../dist/domains/agents/services/stores/ports/BacklogStore.js'));
  });
