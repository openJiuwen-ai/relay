...
import { DeliveryCursorStore } from '../dist/domains/agents/services/stores/ports/DeliveryCursorStore.js';
import { InvocationRecordStore } from '../dist/domains/agents/services/stores/ports/InvocationRecordStore.js';
import { MessageStore } from '../dist/domains/agents/services/stores/ports/MessageStore.js';
import { ThreadStore } from '../dist/domains/agents/services/stores/ports/ThreadStore.js';
...
describe('Concurrent fault drills - in-memory stores', () => {
  ...
  it('S3 ack vs append race: cursor does not skip or duplicate messages', async () => {
    ...
    const ackPromise = Promise.resolve().then(() => deliveryCursorStore.ackCursor(userId, agentId, threadId, base.id));

    const [newMsg] = await Promise.all([appendPromise, ackPromise]);
    const cursor = await deliveryCursorStore.getCursor(userId, agentId, threadId);
    ...
    await deliveryCursorStore.ackCursor(userId, agentId, threadId, newMsg.id);
    const secondCursor = await deliveryCursorStore.getCursor(userId, agentId, threadId);
...
describe('Concurrent fault drills - Redis stores', { skip: !REDIS_URL ? 'REDIS_URL not set' : false }, () => {
  ...
  before(async () => {
    ...
    const shared = await import('@openjiuwen/relay-shared/utils');
    createRedisClient = shared.createRedisClient;
    SessionStore = shared.SessionStore;

    const invocMod = await import('../dist/domains/agents/services/stores/redis/RedisInvocationRecordStore.js');
    const threadMod = await import('../dist/domains/agents/services/stores/redis/RedisThreadStore.js');
    const messageMod = await import('../dist/domains/agents/services/stores/redis/RedisMessageStore.js');
    const threadKeyMod = await import('../dist/domains/agents/services/stores/redis-keys/thread-keys.js');
