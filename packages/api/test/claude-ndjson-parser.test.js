...
const { transformClaudeEvent } = await import('../dist/domains/agents/services/agents/providers/claude-ndjson-parser.js');
...
test('system/init → session_init', () => {
  ...
  assert.equal(result.agentId, CAT);
...
test('content_block_stop with thinking buffer → system_info(thinking)', () => {
  ...
  assert.equal(parsed.agentId, CAT);
