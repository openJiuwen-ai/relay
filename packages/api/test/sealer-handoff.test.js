...
describe('SessionSealer handoff digest integration', () => {
  ...
  async function createFixtures(handoffConfig) {
    ...
    const { TranscriptWriter } = await import('../dist/domains/agents/services/session/TranscriptWriter.js');
    const { TranscriptReader } = await import('../dist/domains/agents/services/session/TranscriptReader.js');
    ...
    const sealer = new SessionSealer(store, writer, threadStore, reader, (_agentId) => 180000, handoffConfig);
  ...
  test('generates and writes handoff digest when bootstrapDepth=generative', async () => {
    ...
    const handoffConfig = {
      getBootstrapDepth: (_agentId) => 'generative',
      resolveProfile: async (_threadId, _agentId) => ({
        apiKey: 'sk-test',
    ...
    writer.appendEvent(
      { sessionId: record.id, threadId: 'thread-1', agentId: 'opus', cliSessionId: 'cli-1', seq: 0 },
  ...
  test('skips handoff when bootstrapDepth=extractive', async () => {
    let fetchCalled = false;
    const handoffConfig = {
      getBootstrapDepth: (_agentId) => 'extractive',
