...
} from '../dist/domains/agents/services/tts/VoiceBlockSynthesizer.js';
...
describe('VoiceBlockSynthesizer.resolveVoiceBlocks — synthesis', () => {
  ...
  it('uses block.speaker override instead of agentId for voice lookup (F085-P3)', async () => {
  ...
  it('falls back to agentId when speaker is not set (backward compat)', async () => {
    let receivedVoice;
    const registry = makeMockRegistry({
      synthesize: async (args) => {
    ...
    // No speaker field — should use agentId ('codex')
    const block = { id: 'a1', kind: 'audio', v: 1, text: 'No speaker field' };
    await synthesizer.resolveVoiceBlocks([block], 'codex');

    const codexVoice = getCatVoice('codex');
    assert.equal(receivedVoice, codexVoice.voice, 'uses agentId voice when no speaker override');
  });
...
describe('VoiceBlockSynthesizer — F066 Phase 4 AC-10: resynthesize', () => {
  it('degraded card includes tts-resynthesize action with text and agentId', async () => {
    const registry = makeMockRegistry({
      synthesize: async () => {
    ...
    assert.equal(result[0].actions[0].payload.agentId, 'opus', 'payload has agentId');
  });
  ...
  it('degraded card uses speaker override in resynthesize action payload (F085-P3)', async () => {
    ...
    assert.equal(result[0].kind, 'card', 'degraded to card');
    assert.equal(result[0].actions[0].payload.agentId, 'gemini', 'uses speaker, not sender agentId');
  });
