...
function createToolFirstService(agentId) {
  return {
    async *invoke() {
      yield { type: 'tool_use', agentId, toolName: 'read_file', toolInput: '{}', timestamp: Date.now() };
      yield { type: 'tool_result', agentId, content: 'file content', timestamp: Date.now() };
      yield { type: 'done', agentId, timestamp: Date.now() };
    },
  };
}

function createSmallTextService(agentId) {
  return {
    async *invoke() {
      // Single small text chunk (< 2000 chars, < 2s from start)
      yield { type: 'text', agentId, content: 'Hi', timestamp: Date.now() };
      yield { type: 'done', agentId, timestamp: Date.now() };
    },
  };
}
...
function createMockDeps(services) {
  ...
  return {
    ...
    messageStore: {
      append: async () => ({ id: `msg-${counter}`, userId: '', agentId: null, content: '', mentions: [], timestamp: 0 }),
      getById: () => null,
      getRecent: () => [],
      getMentionsFor: () => [],
      getBefore: () => [],
...
function createThinkingThenToolService(agentId) {
  return {
    async *invoke() {
      yield {
        type: 'system_info',
        agentId,
        content: JSON.stringify({ type: 'invocation_created', invocationId: `inv-${agentId}` }),
        timestamp: Date.now(),
      };
      yield {
        type: 'system_info',
