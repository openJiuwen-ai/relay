...
describe('OutboundDeliveryHook — media delivery integration', () => {
  ...
  it('sends media_gallery image blocks via sendMedia', async () => {
    ...
    const hook = new OutboundDeliveryHook({
      bindingStore: {
        async getByThread() {
          return [{ connectorId: 'dingtalk', externalChatId: 'chat2', threadId: 'T2', userId: 'u1', createdAt: 0 }];
        },
      },
      adapters: new Map([['dingtalk', mockAdapter]]),
      log: { info() {}, warn() {}, error() {}, debug() {} },
    });
  ...
  it('R3-P1: mediaPathResolver resolves route URL to absPath in sendMedia payload', async () => {
    ...
    const hook = new OutboundDeliveryHook({
      bindingStore: {
        async getByThread() {
          return [{ connectorId: 'dingtalk', externalChatId: 'chat1', threadId: 'T1', userId: 'u1', createdAt: 0 }];
        },
      },
      adapters: new Map([['dingtalk', mockAdapter]]),
      log: { info() {}, warn() {}, error() {}, debug() {} },
  ...
  it('handles multiple bindings delivering to different adapters', async () => {
      ...
      bindingStore: {
        async getByThread() {
          return [
            ...
            { connectorId: 'dingtalk', externalChatId: 'dtc1', threadId: 'T4', userId: 'u1', createdAt: 0 },
          ];
        },
      },
      adapters: new Map([
    ...
    assert.equal(dingtalkMedia.length, 1);
  });
});
