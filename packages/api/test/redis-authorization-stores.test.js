...
describe('RedisAuthorizationRuleStore', { skip: !REDIS_URL ? 'REDIS_URL not set' : false }, () => {
  ...
  before(async () => {
    ...
    const storeModule = await import('../dist/domains/agents/services/stores/redis/RedisAuthorizationRuleStore.js');
  ...
  it('add() creates rule with generated id', async () => {
    ...
    assert.equal(rule.agentId, 'opus');
  ...
  it('list() returns all rules', async () => {
    await store.add({ agentId: 'opus', action: 'a', scope: 'global', decision: 'allow', createdBy: 'u' });
    await store.add({ agentId: 'codex', action: 'b', scope: 'global', decision: 'deny', createdBy: 'u' });
  ...
  it('list() filters by agentId', async () => {
    ...
    const filtered = await store.list({ agentId: 'opus' });
    assert.equal(filtered.length, 1);
    assert.equal(filtered[0].agentId, 'opus');
  });

  it('remove() deletes a rule', async () => {
    const rule = await store.add({ agentId: 'opus', action: 'x', scope: 'global', decision: 'allow', createdBy: 'u' });
  ...
  it('match() returns exact action match', async () => {
    await store.add({ agentId: 'opus', action: 'git_commit', scope: 'global', decision: 'allow', createdBy: 'u' });
  ...
  it('match() supports glob wildcard', async () => {
    await store.add({ agentId: 'opus', action: 'git_*', scope: 'global', decision: 'deny', createdBy: 'u' });
  ...
  it('match() prefers thread-scoped over global', async () => {
    await store.add({ agentId: 'opus', action: 'deploy', scope: 'global', decision: 'allow', createdBy: 'u' });
    await store.add({
      agentId: 'opus',
      action: 'deploy',
      scope: 'thread',
  ...
  it('match() returns null when no rule matches', async () => {
    await store.add({ agentId: 'codex', action: 'test', scope: 'global', decision: 'allow', createdBy: 'u' });
    const matched = await store.match('opus', 'deploy', 'thread-1');
    assert.equal(matched, null);
  });

  it('match() supports wildcard agentId', async () => {
    await store.add({ agentId: '*', action: 'run_tests', scope: 'global', decision: 'allow', createdBy: 'u' });
  ...
  it('evicts oldest rule when maxRules reached', async () => {
    ...
    await smallStore.add({ agentId: 'opus', action: 'a2', scope: 'global', decision: 'allow', createdBy: 'u' });
    await smallStore.add({ agentId: 'opus', action: 'a3', scope: 'global', decision: 'allow', createdBy: 'u' });
    // Should evict r1
    await smallStore.add({ agentId: 'opus', action: 'a4', scope: 'global', decision: 'allow', createdBy: 'u' });
...
describe('RedisPendingRequestStore', { skip: !REDIS_URL ? 'REDIS_URL not set' : false }, () => {
  ...
  before(async () => {
    ...
    const storeModule = await import('../dist/domains/agents/services/stores/redis/RedisPendingRequestStore.js');
  ...
  it('listWaiting() returns only waiting records', async () => {
    await store.create({ invocationId: 'i1', agentId: 'opus', threadId: 't1', action: 'a', reason: 'r' });
    const rec2 = await store.create({ invocationId: 'i2', agentId: 'opus', threadId: 't1', action: 'b', reason: 'r' });
    await store.create({ invocationId: 'i3', agentId: 'opus', threadId: 't1', action: 'c', reason: 'r' });
  ...
  it('listWaiting() filters by threadId', async () => {
    await store.create({ invocationId: 'i1', agentId: 'opus', threadId: 't1', action: 'a', reason: 'r' });
    await store.create({ invocationId: 'i2', agentId: 'opus', threadId: 't2', action: 'b', reason: 'r' });
...
describe('RedisAuthorizationAuditStore', { skip: !REDIS_URL ? 'REDIS_URL not set' : false }, () => {
  ...
  before(async () => {
    ...
    const storeModule = await import('../dist/domains/agents/services/stores/redis/RedisAuthorizationAuditStore.js');
  ...
  it('append() creates audit entry with generated id', async () => {
    ...
    assert.equal(entry.agentId, 'opus');
