...
import { transformDareEvent } from '../dist/domains/agents/services/agents/providers/dare-event-transform.js';
...
describe('transformDareEvent', () => {
  ...
  test('maps session.started → session_init', () => {
    ...
    const result = transformDareEvent(event, agentId);
    assert.ok(result);
  ...
  test('returns null for log events', () => {
    assert.strictEqual(transformDareEvent(envelope('log.info', { message: 'hi' }), agentId), null);
    assert.strictEqual(transformDareEvent(envelope('log.header', {}), agentId), null);
  });

  test('returns null for transport events', () => {
    assert.strictEqual(transformDareEvent(envelope('transport.raw', {}), agentId), null);
  });

  test('returns null for model.response (no user-facing content)', () => {
    assert.strictEqual(
      transformDareEvent(envelope('model.response', { iteration: 1, has_tool_calls: false }), agentId),
      null,
    );
  });
  ...
  test('ignores non-thinking transport.raw payloads', () => {
    ...
    assert.strictEqual(transformDareEvent(event, agentId), null);
  });

  test('returns null for non-object input', () => {
    assert.strictEqual(transformDareEvent('string', agentId), null);
    assert.strictEqual(transformDareEvent(null, agentId), null);
    assert.strictEqual(transformDareEvent(42, agentId), null);
  });
