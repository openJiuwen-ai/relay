 ...
 * User's own message: agentId=null, no source.
 * Cat message: agentId='opus' (or any AgentId).
...
describe('RedisThreadReadStateStore', { skip: !REDIS_URL ? 'REDIS_URL not set' : false }, () => {
  ...
  before(async () => {
    ...
    const storeModule = await import('../dist/domains/agents/services/stores/redis/RedisThreadReadStateStore.js');
    RedisThreadReadStateStore = storeModule.RedisThreadReadStateStore;
    const msgModule = await import('../dist/domains/agents/services/stores/redis/RedisMessageStore.js');
  ...
  it('getUnreadSummaries() counts cat messages as unread', async () => {
    const tid = uniqueId('t');
    // Cat messages share same userId as tenant — agentId distinguishes them
  ...
  it('getUnreadSummaries() excludes user own messages (agentId=null)', async () => {
    const tid = uniqueId('t');
    // Cat message (agentId='opus') — should be counted
    const m1 = await messageStore.append({
      userId: 'user1',
      agentId: 'opus',
      content: 'cat reply',
    ...
    // User's own message (agentId=null) — should NOT be counted
    await messageStore.append({
      userId: 'user1',
      agentId: null,
      content: 'my question',
    ...
    // Cat reply (agentId='opus') — should be counted
    await messageStore.append({
      userId: 'user1',
      agentId: 'opus',
      content: 'cat reply 2',
