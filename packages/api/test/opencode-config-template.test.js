...
} from '../dist/domains/agents/services/agents/providers/opencode-config-template.js';
...
describe('opencode Config Template (AC-9 + AC-10)', () => {
  ...
  test('does not include OfficeClaw MCP tools in config', () => {
    ...
    // MCP config should not reference any office_claw tools
    if (config.mcp) {
      const mcpKeys = Object.keys(config.mcp);
      for (const key of mcpKeys) {
        assert.ok(!key.startsWith('office_claw'), `MCP config must not include OfficeClaw tools: ${key}`);
...
describe('writeOpenCodeRuntimeConfig', () => {
  test('writes per-agentId config file to .office-claw directory', () => {
    ...
    try {
      ...
      assert.ok(configPath.includes('opencode-runtime-test-cat'), 'must include agentId');
  ...
  test('different agentIds produce different files', () => {
    const tmpRoot = mkdtempSync(join(tmpdir(), 'oc-config-test-'));
    try {
