...
describe('Thread soft-delete preserves data (Phase D)', () => {
  ...
  it('DELETE /api/threads/:id?deleteWorkspace=true deletes a non-shared directory', async () => {
    ...
    try {
      ...
      assert.equal(res.statusCode, 204);
      assert.equal(existsSync(projectDir), false, 'non-shared directory should be deleted');
