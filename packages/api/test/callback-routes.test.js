...
import { catRegistry } from '@openjiuwen/relay-shared';
...
describe('Callback Routes', () => {
  ...
  test('POST post-message succeeds with valid credentials', async () => {
    ...
    assert.equal(broadcasted[0].agentId, 'opus');
    ...
    const activity = threadStore.getParticipantsWithActivity('default');
    assert.equal(activity[0].catId, 'opus', 'callback-origin cat message should update last-replier activity');
  ...
  test('POST post-message calls outboundHook.deliver when wired', async () => {
    ...
    const outboundHook = {
      async deliver(threadId, content, agentId, richBlocks, threadMeta, origin) {
        deliverCalled = true;
        deliverArgs = { threadId, content, agentId, richBlocks, threadMeta, origin };
      },
    ...
    assert.equal(deliverArgs.agentId, 'opus');
  ...
  test('POST post-message auto-appends file location when generated file block exists but text omits it', async () => {
    const app = await createApp();
    const { invocationId, callbackToken } = registry.create('user-1', 'opus');
    const { getRichBlockBuffer } = await import('../dist/domains/agents/services/agents/invocation/RichBlockBuffer.js');
  ...
  test('POST post-message stores targetAgents in extra and uses as mentions (F098-C1)', async () => {
    const app = await createApp();
    const { invocationId, callbackToken } = registry.create('user-1', 'opus');
    ...
    const recent = messageStore.getRecent(10);
    assert.equal(recent.length, 1);
    // targetAgents should be used as mentions
    assert.deepEqual(recent[0].mentions, ['codex', 'gpt52']);
    // targetAgents should be stored in extra for frontend direction rendering
    assert.deepEqual(recent[0].extra?.targetAgents, ['codex', 'gpt52']);
  });

  test('POST post-message broadcasts targetAgents in real-time socket event (cloud P2)', async () => {
    const app = await createApp();
    const { invocationId, callbackToken } = registry.create('user-1', 'opus');
    ...
    assert.equal(broadcasted.length, 1);
    assert.deepEqual(
      broadcasted[0].extra?.targetAgents,
      ['codex', 'gpt52'],
      'real-time broadcast must include extra.targetAgents for immediate direction pill rendering',
    );
  });

  test('POST post-message single content @mention ignores extra explicit targetAgents (A2A fail-closed)', async () => {
    const app = await createApp();
    const { invocationId, callbackToken } = registry.create('user-1', 'opus');
    ...
    const recent = messageStore.getRecent(10);
    assert.equal(recent.length, 1);
    // Single content mention should win; extras from explicit targetAgents are pruned.
    const mentions = recent[0].mentions;
    assert.ok(mentions.includes('codex'), 'content @mention should be included');
    assert.equal(mentions.includes('gpt52'), false, 'extra explicit targetAgents should be pruned');
    assert.deepEqual(recent[0].extra?.targetAgents, ['gpt52']);
  });
  ...
  test('POST post-message keeps merged targets when content has multiple @mentions', async () => {
    ...
    assert.ok(mentions.includes('gemini'), 'multi-mention content should still merge explicit targetAgents');
  });
  ...
  test('GET thread-context filters scheduler trigger placeholder but keeps agent reply', async () => {
    ...
    assert.equal(body.messages[0].agentId, 'opus');
  ...
  test('GET thread-context supports agentId filter (cat + user)', async () => {
    const app = await createApp();
    const { invocationId, callbackToken } = registry.create('user-1', 'opus');
    ...
    const catResponse = await app.inject({
      method: 'GET',
      url: `/api/callbacks/thread-context?invocationId=${invocationId}&callbackToken=${callbackToken}&agentId=codex`,
    });
    ...
    const userResponse = await app.inject({
      method: 'GET',
      url: `/api/callbacks/thread-context?invocationId=${invocationId}&callbackToken=${callbackToken}&agentId=user`,
    });
  ...
  test('GET thread-context combines agentId + keyword filters', async () => {
    const app = await createApp();
    const { invocationId, callbackToken } = registry.create('user-1', 'opus');
    ...
    const response = await app.inject({
      method: 'GET',
      url: `/api/callbacks/thread-context?invocationId=${invocationId}&callbackToken=${callbackToken}&agentId=codex&keyword=redis`,
    });
    assert.equal(response.statusCode, 200);
    const body = JSON.parse(response.body);
    assert.equal(body.messages.length, 1);
  ...
  test('GET thread-context rejects unknown agentId filter', async () => {
    const app = await createApp();
    const { invocationId, callbackToken } = registry.create('user-1', 'opus');

    const response = await app.inject({
      method: 'GET',
      url: `/api/callbacks/thread-context?invocationId=${invocationId}&callbackToken=${callbackToken}&agentId=unknown-cat`,
    });

    assert.equal(response.statusCode, 400);
    const body = JSON.parse(response.body);
    assert.match(body.error, /Unknown agentId filter/);
  });
  ...
  test('GET list-tasks returns user-scoped tasks and supports filters', async () => {
    ...
    const filteredRes = await app.inject({
      method: 'GET',
      url: `/api/callbacks/list-tasks?invocationId=${invocationId}&callbackToken=${callbackToken}&agentId=codex&status=blocked`,
    });
  ...
  test('POST register-pr-tracking succeeds with valid input', async () => {
    ...
    assert.equal(body.entry.agentId, 'opus');
  ...
  test('POST register-pr-tracking ignores payload agentId, uses invocation identity', async () => {
    const { MemoryPrTrackingStore } = await import('../dist/infrastructure/email/PrTrackingStore.js');
    const prTrackingStore = new MemoryPrTrackingStore();
    ...
    // Invocation is opus, payload sends bogus agentId — server must ignore payload
    const { invocationId, callbackToken } = registry.create('user-1', 'opus');
    ...
    const response = await app.inject({
      ...
      payload: {
        ...
        repoFullName: 'zts212653/office-claw',
        prNumber: 1,
        agentId: 'nonexistent-cat', // bogus — should be ignored
      },
    });

    assert.equal(response.statusCode, 200, 'payload agentId is ignored, so bogus value must not cause 400');
    const body = JSON.parse(response.body);
    assert.equal(body.entry.agentId, 'opus', 'must use invocation agentId, not payload');
  });
  ...
  test('POST register-pr-tracking uses invocation agentId, not payload agentId (authority bug)', async () => {
    const { MemoryPrTrackingStore } = await import('../dist/infrastructure/email/PrTrackingStore.js');
    const prTrackingStore = new MemoryPrTrackingStore();
    ...
    const response = await app.inject({
      ...
      payload: {
        ...
        repoFullName: 'zts212653/office-claw',
        prNumber: 832,
        agentId: 'opus', // ← LLM passed wrong agentId
      },
    });
    ...
    // Server must use the authoritative agentId from invocation record, not the payload
    assert.equal(body.entry.agentId, 'opencode', 'must use invocation agentId, not payload agentId');

    const stored = prTrackingStore.get('zts212653/office-claw', 832);
    assert.equal(stored.agentId, 'opencode', 'stored entry must have authoritative agentId');
  });
  ...
  test('POST dispatch-agent-task resolves displayName and returns target response', async () => {
    const { callbacksRoutes } = await import('../dist/routes/callbacks.js');
    const app = Fastify();

    let completionHook = null;
    const invocationQueue = {
      hasQueuedAgentForCat: () => false,
      enqueue: (input) => ({
        outcome: 'enqueued',
        entry: {
          id: 'entry-1',
    ...
    const queueProcessor = {
      ...
      tryAutoExecute: async () => {
        completionHook?.('entry-1', 'succeeded', '评估完成：通过');
      },
    };
    ...
    const { invocationId, callbackToken } = registry.create('user-1', 'codex', 'thread-dispatch');
    const targetDisplayName = catRegistry.getOrThrow('assistant').config.displayName;

    const response = await app.inject({
      method: 'POST',
      url: '/api/callbacks/dispatch-agent-task',
      payload: {
        invocationId,
        callbackToken,
    ...
    assert.equal(body.resolvedTargetCatId, 'assistant');
    assert.equal(body.responseText, '评估完成：通过');
  });

  test('POST dispatch-agent-task returns structured unknown_target failure', async () => {
    const app = await createApp();
    const { invocationId, callbackToken } = registry.create('user-1', 'codex', 'thread-dispatch-2');
    ...
    assert.equal(body.errorCode, 'unknown_target');
    assert.equal(body.phase, 'resolution');
  });
});
