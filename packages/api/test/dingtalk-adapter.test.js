...
describe('DingTalkAdapter', () => {
  ...
  describe('parseEvent()', () => {
    ...
    it('parses group messages with chatType group', () => {
      const result = makeAdapter().parseEvent({
        msgtype: 'text',
        conversationType: '2',
        conversationId: 'conv_group',
        openConversationId: 'open_conv_group',
        msgId: 'msg_grp',
      ...
      assert.equal(result.chatId, 'open_conv_group');
      assert.equal(result.chatType, 'group');
      assert.equal(result.text, 'Hello group');
      assert.equal(result.senderNick, 'GroupUser');
      assert.equal(result.conversationTitle, 'Test Group');
    });
  ...
  describe('sendReply()', () => {
    it('calls sendMessage with text msgType', async () => {
      ...
      assert.equal(calls[0].msgType, 'sampleText');
  ...
  describe('sendMarkdown()', () => {
    it('calls sendMessage with markdown msgType', async () => {
      ...
      assert.equal(calls.length, 1);
      assert.equal(calls[0].chatId, 'staff_001');
      assert.equal(calls[0].msgType, 'sampleMarkdown');
  ...
  describe('sendFormattedReply()', () => {
    it('attempts AI Card, falls back to markdown on error', async () => {
      ...
      assert.equal(sendCalls[0].msgType, 'sampleMarkdown');
    ...
    it('AI Card uses real conversationId from inbound parseEvent, not staffId', async () => {
      ...
      const envelope = { header: 'Agent', body: 'Reply', origin: 'direct' };
    ...
    it('falls back to markdown when no conversationId mapped (cold restart)', async () => {
      ...
      const envelope = { header: 'Agent', body: 'Reply after restart', origin: 'direct' };
  ...
  describe('sendPlaceholder() + editMessage() + deleteMessage()', () => {
    it('creates AI Card and returns outTrackId', async () => {
      ...
      assert.ok(outTrackId.startsWith('oc-'));
      assert.equal(cardCalls.length, 1);
    });
  ...
  describe('sendMedia()', () => {
    ...
    it('falls back to text link for audio URL', async () => {
      ...
      assert.equal(sendCalls.length, 1);
      assert.equal(sendCalls[0].msgType, 'sampleText');
