...
describe('RedisMessageStore', { skip: !REDIS_URL ? 'REDIS_URL not set' : false }, () => {
  ...
  before(async () => {
    ...
    const storeModule = await import('../dist/domains/agents/services/stores/redis/RedisMessageStore.js');
  ...
  it('getRecent() returns messages in chronological order', async () => {
    const now = Date.now();
    await store.append({ userId: 'u', agentId: null, content: 'first', mentions: [], timestamp: now });
    await store.append({ userId: 'u', agentId: 'opus', content: 'second', mentions: [], timestamp: now + 1 });
    await store.append({ userId: 'u', agentId: null, content: 'third', mentions: [], timestamp: now + 2 });
  ...
  it('getRecent() filters by userId', async () => {
    const now = Date.now();
    await store.append({ userId: 'alice', agentId: null, content: 'alice msg', mentions: [], timestamp: now });
    await store.append({ userId: 'bob', agentId: null, content: 'bob msg', mentions: [], timestamp: now + 1 });
  ...
  it('getMentionsFor() returns messages mentioning a specific cat', async () => {
    const now = Date.now();
    await store.append({ userId: 'u', agentId: null, content: 'hi opus', mentions: ['opus'], timestamp: now });
    await store.append({ userId: 'u', agentId: null, content: 'hi codex', mentions: ['codex'], timestamp: now + 1 });
    await store.append({
      userId: 'u',
      agentId: null,
      content: 'hi both',
  ...
  it('getBefore() returns messages before timestamp', async () => {
    const base = Date.now();
    await store.append({ userId: 'u', agentId: null, content: 'old', mentions: [], timestamp: base });
    await store.append({ userId: 'u', agentId: null, content: 'mid', mentions: [], timestamp: base + 100 });
    await store.append({ userId: 'u', agentId: null, content: 'new', mentions: [], timestamp: base + 200 });
  ...
  it('getBefore() respects limit', async () => {
    const base = Date.now();
    for (let i = 0; i < 5; i++) {
      await store.append({ userId: 'u', agentId: null, content: `msg${i}`, mentions: [], timestamp: base + i });
    }
