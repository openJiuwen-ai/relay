...
describe('replyTo threading', () => {
  ...
  test('hydrateReplyPreview returns sender + truncated content for existing parent', async () => {
    ...
    assert.ok(preview);
    assert.equal(preview.senderAgentId, 'opus');
  ...
  test('hydrateReplyPreview returns null senderAgentId for user messages', async () => {
    const { MessageStore, hydrateReplyPreview } = await import(
      '../dist/domains/agents/services/stores/ports/MessageStore.js'
    );
    const store = new MessageStore();
    ...
    const preview = await hydrateReplyPreview(store, parent.id);
    assert.ok(preview);
    assert.equal(preview.senderAgentId, null);
