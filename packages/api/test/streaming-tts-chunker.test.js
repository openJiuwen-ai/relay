...
import { StreamingTtsChunker } from '../dist/domains/agents/services/tts/StreamingTtsChunker.js';
...
describe('StreamingTtsChunker', () => {
  ...
  it('broadcasts correct event shape', async () => {
    ...
    assert.equal(event.agentId, 'opus');
  ...
  it('emits voice_stream_start before first chunk and sets hasStarted', async () => {
    ...
    assert.equal(broadcaster.events[0].data.agentId, 'opus');
