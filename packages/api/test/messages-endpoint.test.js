...
describe('GET /api/messages', () => {
  ...
  it('returns messages with correct format', async () => {
    ...
    assert.equal(body.messages[0].agentId, null);
    assert.equal(body.messages[0].content, 'hello');

    // Assistant message
    assert.equal(body.messages[1].type, 'assistant');
    assert.equal(body.messages[1].agentId, 'opus');
  ...
  it('maps canonical system messages to type=system', async () => {
    ...
    assert.equal(body.messages[0].agentId, 'system');
  });

  it('returns persisted system error messages with agentId=null as type=system', async () => {
    messageStore.append({
      userId: 'default-user',
      agentId: null,
      content: 'ping gemini',
    ...
    assert.equal(body.messages[1].agentId, null);
  ...
  it('serializes extra.targetAgents when present (F098-C1 regression)', async () => {
    messageStore.append({
      userId: 'default-user',
      agentId: 'opus',
      content: 'review done',
      ...
      origin: 'callback',
      timestamp: 7000,
      extra: { targetAgents: ['codex', 'gpt52'] },
    });
    ...
    const msg = body.messages[0];
    assert.deepStrictEqual(
      msg.extra?.targetAgents,
      ['codex', 'gpt52'],
      'extra.targetAgents must be serialized for frontend direction rendering',
    );
  });

  it('message without source and without agentId is type=user', async () => {
    messageStore.append({
      userId: 'default-user',
      agentId: null,
