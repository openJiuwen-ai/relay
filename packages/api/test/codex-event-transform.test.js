...
const { transformCodexEvent } = await import('../dist/domains/agents/services/agents/providers/codex-event-transform.js');
...
test('item.completed file_change with pptx artifact → tool_use only after workspace sunset', () => {
  const msg = transformCodexEvent(
    {
      type: 'item.completed',
      item: {
  ...
  assert.ok(!Array.isArray(msg), 'workspace sunset should not emit file rich blocks');
  assert.equal(msg?.type, 'tool_use');
  assert.equal(msg?.toolName, 'file_change');
  assert.deepEqual(msg?.toolInput, {
    status: 'completed',
    changes: 1,
...
test('item.completed file_change with ppt page html → tool_use + ppt_studio_page signal', () => {
  const msg = transformCodexEvent(
    {
      type: 'item.completed',
      item: {
        type: 'file_change',
        status: 'completed',
        changes: [{ path: 'output/demo/pages/page-1.pptx.html' }],
      },
    },
    CAT,
    undefined,
  ...
  assert.ok(Array.isArray(msg), 'ppt page html should emit both tool event and hidden ppt studio signal');
  assert.equal(msg[0]?.type, 'tool_use');
  assert.equal(msg[0]?.toolName, 'file_change');
  assert.deepEqual(msg[0]?.toolInput, {
    status: 'completed',
    changes: 1,
    paths: ['output/demo/pages/page-1.pptx.html'],
  });

  assert.equal(msg[1]?.type, 'system_info');
  const payload = JSON.parse(msg[1]?.content ?? '{}');
  assert.equal(payload.type, 'ppt_studio_page');
  assert.equal(payload.worktreeId, undefined);
  assert.equal(payload.htmlPath, 'output/demo/pages/page-1.pptx.html');
  assert.equal(payload.pageNumber, 1);
  assert.equal(payload.pagesDir, 'output/demo/pages');
});
...
test('top-level reasoning loop error maps to agent error', () => {
  const event = {
    type: 'error',
    message: 'Reasoning loop detected: same reason_content 25 times with no tool calls, terminating',
  };
  const msg = transformCodexEvent(event, CAT);
  assert.equal(msg?.type, 'error');
  assert.equal(msg?.error, event.message);
});
