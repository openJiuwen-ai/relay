...
const { QueueProcessor } = await import('../dist/domains/agents/services/agents/invocation/QueueProcessor.js');
...
describe('QueueProcessor', () => {
  ...
  it('queued execution broadcasts intent_mode with invocationId when processing starts', async () => {
    const entry = enqueueEntry(deps.queue, { targetAgents: ['codex'], intent: 'execute' });
    deps.queue.backfillMessageId('t1', 'u1', entry.id, 'msg-1');
  ...
  it('emits queue_updated(action=completed) after entry is removed from queue', async () => {
    const entry = enqueueEntry(deps.queue, { targetAgents: ['codex'] });
    deps.queue.backfillMessageId('t1', 'u1', entry.id, 'msg-1');
  ...
  it('concurrent tryExecuteNext on same thread + same cat → only one starts (F108: per-slot mutex)', async () => {
    ...
    enqueueEntry(slowDeps.queue, { content: 'a', targetAgents: ['opus'] });
    enqueueEntry(slowDeps.queue, { content: 'b', targetAgents: ['opus'] });
  ...
  it('chain auto-dequeue: entry1 succeed → entry2 auto-starts', async () => {
    ...
    const e1 = enqueueEntry(deps.queue, { userId: 'u1', content: 'first', targetAgents: ['a'] });
    deps.queue.backfillMessageId('t1', 'u1', e1.id, 'msg-1');
    const e2 = enqueueEntry(deps.queue, { userId: 'u2', content: 'second', targetAgents: ['b'] });
  ...
  describe('slot-aware mutex and dequeue (F108)', () => {
    it('processing mutex is per-slot: different cats can execute concurrently in same thread', async () => {
      ...
      const e1 = enqueueEntry(deps.queue, { content: 'opus task', targetAgents: ['opus'] });
      deps.queue.backfillMessageId('t1', 'u1', e1.id, 'msg-opus');
      const e2 = enqueueEntry(deps.queue, { content: 'codex task', targetAgents: ['codex'] });
    ...
    it('slot completion does not affect pause state of different slot', async () => {
      ...
      enqueueEntry(deps.queue, { content: 'opus task', targetAgents: ['opus'] });
      enqueueEntry(deps.queue, { content: 'codex task', targetAgents: ['codex'] });
    ...
    it('releaseSlot is slot-specific', async () => {
      ...
      const e1 = enqueueEntry(slowDeps.queue, { content: 'opus slow', targetAgents: ['opus'] });
      slowDeps.queue.backfillMessageId('t1', 'u1', e1.id, 'msg-1');
      const e2 = enqueueEntry(slowDeps.queue, { content: 'codex fast', targetAgents: ['codex'] });
    ...
    it('onInvocationComplete requires agentId parameter', async () => {
      enqueueEntry(deps.queue);

      // New signature: onInvocationComplete(threadId, agentId, status)
      await processor.onInvocationComplete('t1', 'opus', 'succeeded');
      // Should not throw — agentId is now required
    });
    ...
    it('tryExecuteNextAcrossUsers checks entryCat slot, not just completing cat slot (P1-2)', async () => {
      ...
      const codexEntry = enqueueEntry(deps.queue, { targetAgents: ['codex'] });
      ...
      const codexEntry2 = enqueueEntry(deps.queue, { targetAgents: ['codex'] });
    ...
    it('tryExecuteNextForUser does not leave entry stuck in processing when slot is busy (P1-3)', async () => {
      ...
      const entry1 = enqueueEntry(deps.queue, { targetAgents: ['codex'] });
  ...
  describe('tryAutoExecute (F122B agent auto-execute)', () => {
    ...
    it('autoExecute entry bypasses pause state', async () => {
      ...
      enqueueEntry(deps.queue, {
        userId: 'system',
        source: 'agent',
        targetAgents: ['codex'], // different cat slot — not paused
        autoExecute: true,
        callerAgentId: 'opus',
      });
    ...
    it('skips busy-slot entry and executes next free-slot autoExecute entry (P2 scan)', async () => {
      ...
      deps.invocationTracker.has = mock.fn((threadId, agentId) => agentId === 'opus');
  ...
  describe('tracker guard on completion chain (tryExecuteNextAcrossUsers)', () => {
    it('does NOT start queued entry when target cat has active CLI invocation', async () => {
      ...
      const entry = enqueueEntry(deps.queue, { targetAgents: ['opus'] });
      deps.queue.backfillMessageId('t1', 'u1', entry.id, 'msg-1');

      // invocationTracker reports opus is active (CLI invocation)
      deps.invocationTracker.has = mock.fn((_tid, agentId) => agentId === 'opus');
  ...
  describe('outbound delivery via QueueProcessor (F088)', () => {
    ...
    it('single-cat execution: outboundHook.deliver called once with correct agentId + content', async () => {
      const deliverCalls = [];
      const outboundHook = {
        deliver: mock.fn(async (threadId, content, agentId, richBlocks, threadMeta) => {
          deliverCalls.push({ threadId, content, agentId, richBlocks, threadMeta });
        }),
      };
      const streamingHook = {
      ...
      assert.equal(deliverCalls[0].agentId, 'opus');
    ...
    it('multi-cat execution: outboundHook.deliver called per-turn with each agentId', async () => {
      const deliverCalls = [];
      const outboundHook = {
        deliver: mock.fn(async (threadId, content, agentId, richBlocks, threadMeta) => {
          deliverCalls.push({ threadId, content, agentId, richBlocks, threadMeta });
      ...
      const hookDeps = stubDeps({
        router: {
          routeExecution: mock.fn(async function* () {
            yield { type: 'text', agentId: 'opus', content: 'Opus says hi. ', timestamp: Date.now() };
            yield { type: 'done', agentId: 'opus', timestamp: Date.now() };
            yield { type: 'text', agentId: 'codex', content: 'Codex chimes in.', timestamp: Date.now() };
            yield { type: 'done', agentId: 'codex', timestamp: Date.now() };
          }),
          ackCollectedCursors: mock.fn(async () => {}),
        },
      ...
      const entry = enqueueEntry(hookDeps.queue, { targetAgents: ['opus'] });
      hookDeps.queue.backfillMessageId('t1', 'u1', entry.id, 'msg-1');
      ...
      assert.equal(deliverCalls[0].agentId, 'opus', 'first deliver should be for opus');
      assert.equal(deliverCalls[0].content, 'Opus says hi. ', 'opus content should match');
      assert.equal(deliverCalls[1].agentId, 'codex', 'second deliver should be for codex');
    ...
    it('delivery failure: cleanupPlaceholders NOT called when delivery partially fails', async () => {
      ...
      const hookDeps = stubDeps({
        router: {
          routeExecution: mock.fn(async function* () {
            yield { type: 'text', agentId: 'opus', content: 'Turn 1. ', timestamp: Date.now() };
            yield { type: 'done', agentId: 'opus', timestamp: Date.now() };
            yield { type: 'text', agentId: 'codex', content: 'Turn 2.', timestamp: Date.now() };
            yield { type: 'done', agentId: 'codex', timestamp: Date.now() };
          }),
          ackCollectedCursors: mock.fn(async () => {}),
        },
    ...
    it('all deliveries succeed: cleanupPlaceholders called', async () => {
      ...
      const hookDeps = stubDeps({
        router: {
          routeExecution: mock.fn(async function* () {
            yield { type: 'text', agentId: 'opus', content: 'Success text', timestamp: Date.now() };
            yield { type: 'done', agentId: 'opus', timestamp: Date.now() };
          }),
          ackCollectedCursors: mock.fn(async () => {}),
        },
    ...
    it('notifies outboundHook when queued delivery batch is complete', async () => {
      ...
      const hookDeps = stubDeps({
        router: {
          routeExecution: mock.fn(async function* () {
            yield { type: 'text', agentId: 'opus', content: 'Queued batch reply', timestamp: Date.now() };
            yield { type: 'done', agentId: 'opus', timestamp: Date.now() };
          }),
          ackCollectedCursors: mock.fn(async () => {}),
        },
    ...
    it('outboundHook set via late-bind setOutboundHook: deliver is called', async () => {
      const lateDeps = stubDeps({
        router: {
          routeExecution: mock.fn(async function* () {
            yield { type: 'text', agentId: 'opus', content: 'Late-bound delivery', timestamp: Date.now() };
            yield { type: 'done', agentId: 'opus', timestamp: Date.now() };
          }),
          ackCollectedCursors: mock.fn(async () => {}),
        },
      ...
      lateProcessor.setOutboundHook({
        deliver: mock.fn(async (threadId, content, agentId) => {
