...
function mockRouter(opts = {}) {
  const calls =
    /** @type {Array<{userId: string, message: string, threadId: string, userMessageId: string, targetAgents: string[], intent: object}>} */ ([]);
  ...
  return {
    ...
    /** @type {any} */
    router: {
      async *routeExecution(userId, message, threadId, userMessageId, targetAgents, intent, options) {
        calls.push({ userId, message, threadId, userMessageId, targetAgents, intent });

        if (opts.throwError) throw opts.throwError;

        if (opts.persistenceFail && options?.persistenceContext) {
          options.persistenceContext.failed = true;
          options.persistenceContext.errors.push({ agentId: targetAgents[0], error: 'disk full' });
        }
...
function mockInvocationTracker() {
  const starts = /** @type {Array<{threadId: string, agentId: string}>} */ ([]);
  const completes = /** @type {Array<{threadId: string, agentId: string}>} */ ([]);
  const cancelCalls = /** @type {Array<{threadId: string, agentId: string, userId: (string|undefined)}>} */ ([]);
  let aborted = false;
  let cancelDenied = false;
  /** @type {Map<string, string>} key = "threadId:agentId" or "threadId" for legacy */
  ...
  return {
    ...
    /** @type {any} */
    tracker: {
      start(threadId, agentId, _userId, _targetAgents) {
        starts.push({ threadId, agentId });
        const controller = { signal: { aborted } };
        return controller;
      },
      complete(threadId, agentId, _controller) {
        completes.push({ threadId, agentId });
      },
      has(threadId, _agentId) {
        return activeSlots.has(threadId);
      },
      getUserId(threadId, _agentId) {
        return activeSlots.get(threadId);
      },
      cancel(threadId, agentId, userId) {
        cancelCalls.push({ threadId, agentId, userId });
        if (cancelDenied) return { cancelled: false, agentIds: [] };
        const owner = activeSlots.get(threadId);
        if (!owner) return { cancelled: false, agentIds: [] };
        if (userId && owner !== userId) return { cancelled: false, agentIds: [] };
        activeSlots.delete(threadId);
        return { cancelled: true, agentIds: ['opus'] };
      },
    },
  };
}
...
describe('ConnectorInvokeTrigger', () => {
  ...
  it('creates InvocationRecord and calls routeExecution', async () => {
    ...
    assert.deepStrictEqual(recordMock.creates[0].targetAgents, ['opus']);
    ...
    assert.deepStrictEqual(routerMock.calls[0].targetAgents, ['opus']);
  });
  ...
  it('delivers outbound messages per-cat for multi-cat A2A responses (ISSUE-9)', async () => {
    ...
    const multiCatRouter = /** @type {any} */ ({
      async *routeExecution(userId, message, threadId, userMessageId, targetAgents, intent, options) {
        yield { type: 'text', agentId: 'opus', content: 'Hello from opus!', timestamp: Date.now() };
        yield {
          type: 'done',
          agentId: 'opus',
          content: '',
        ...
        yield { type: 'text', agentId: 'codex', content: 'Hello from codex!', timestamp: Date.now() };
        yield {
          type: 'done',
          agentId: 'codex',
          content: '',
    ...
    const deliverCalls = /** @type {Array<{threadId: string, content: string, agentId: string}>} */ ([]);
    const outboundHook = {
      deliver: async (threadId, content, agentId) => {
        deliverCalls.push({ threadId, content, agentId });
      },
    };
    ...
    assert.strictEqual(deliverCalls[0].agentId, 'opus');
    assert.strictEqual(deliverCalls[0].content, 'Hello from opus!');
    assert.strictEqual(deliverCalls[1].agentId, 'codex');
  ...
  it('BUG-4: WeChat multi-turn A2A merges into single deliver call', async () => {
    ...
    const deliverCalls = /** @type {Array<{threadId: string, content: string, agentId: any}>} */ ([]);
    const outboundHook = {
      deliver: async (threadId, content, agentId) => {
        deliverCalls.push({ threadId, content, agentId });
      },
    ...
    assert.strictEqual(deliverCalls[0].agentId, undefined, 'Merged delivery should not have a single agentId');
  ...
  it('BUG-4 P1: WeChat merge preserves richBlocks in merged content', async () => {
    ...
    const deliverCalls = /** @type {Array<{threadId: string, content: string, agentId: any, richBlocks: any}>} */ ([]);
    const outboundHook = {
      deliver: async (threadId, content, agentId, richBlocks) => {
        deliverCalls.push({ threadId, content, agentId, richBlocks });
      },
      getConnectorIds: async (_threadId) => ['weixin'],
    };
  ...
  it('R1-P1: delivers richBlocks-only reply (no text) via outbound hook', async () => {
    ...
    const deliverCalls = /** @type {Array<{threadId: string, content: string, agentId: string, richBlocks: any}>} */ ([]);
    const outboundHook = {
      deliver: async (threadId, content, agentId, richBlocks) => {
        deliverCalls.push({ threadId, content, agentId, richBlocks });
      },
  ...
  it('R1-P2: multi-cat outbound delivery preserves order (sequential await)', async () => {
    const multiCatRouter = /** @type {any} */ ({
      async *routeExecution(userId, message, threadId, userMessageId, targetAgents, intent, options) {
        yield { type: 'text', agentId: 'opus', content: 'First', timestamp: Date.now() };
        yield {
          type: 'done',
          agentId: 'opus',
          content: '',
        ...
        yield { type: 'text', agentId: 'codex', content: 'Second', timestamp: Date.now() };
        yield {
          type: 'done',
          agentId: 'codex',
          content: '',
  ...
  it('cloud-P1: hanging deliver does not block tracker cleanup', async () => {
    const hangingRouter = /** @type {any} */ ({
      async *routeExecution(userId, message, threadId, userMessageId, targetAgents, intent, options) {
        ...
        yield { type: 'text', agentId: 'codex', content: 'World', timestamp: Date.now() };
        yield {
          type: 'done',
          agentId: 'codex',
          content: '',
  ...
  it('cloud-P1-4: A→B→A ping-pong delivers 3 separate turns (not merged by agentId)', async () => {
    const pingPongRouter = /** @type {any} */ ({
      async *routeExecution(userId, message, threadId, userMessageId, targetAgents, intent, options) {
        // Turn 1: opus responds
        yield { type: 'text', agentId: 'opus', content: 'Opus turn 1', timestamp: Date.now() };
        yield {
          type: 'done',
          agentId: 'opus',
          content: '',
        ...
        yield { type: 'text', agentId: 'codex', content: 'Codex review', timestamp: Date.now() };
        yield {
          type: 'done',
          agentId: 'codex',
          content: '',
        ...
        yield { type: 'text', agentId: 'opus', content: 'Opus turn 3', timestamp: Date.now() };
        yield {
          type: 'done',
          agentId: 'opus',
          content: '',
    ...
    const deliverCalls = /** @type {Array<{agentId: string, content: string}>} */ ([]);
    const outboundHook = {
      deliver: async (threadId, content, agentId) => {
        deliverCalls.push({ agentId, content });
      },
    };
    ...
    assert.strictEqual(deliverCalls[2].agentId, 'opus');
  ...
  it('cloud-P1-5: richBlocks-only re-entry after silent cat gets own turn', async () => {
    ...
    const reentryRouter = /** @type {any} */ ({
      async *routeExecution(userId, message, threadId, userMessageId, targetAgents, intent, options) {
        ...
        yield { type: 'text', agentId: 'opus', content: 'Opus first', timestamp: Date.now() };
        yield {
          type: 'done',
          agentId: 'opus',
          content: '',
    ...
    const deliverCalls = /** @type {Array<{agentId: string, content: string, richBlocks: any}>} */ ([]);
    const outboundHook = {
      deliver: async (threadId, content, agentId, richBlocks) => {
        deliverCalls.push({ agentId, content, richBlocks });
      },
    };
    ...
    assert.strictEqual(deliverCalls[1].agentId, 'opus');
    assert.ok(deliverCalls[1].richBlocks?.length > 0, 'Second turn should have richBlocks');
  });

  it('cloud-P1-3: silent first cat → second cat reply uses actual speaker agentId', async () => {
    ...
    const silentFirstRouter = /** @type {any} */ ({
      async *routeExecution(userId, message, threadId, userMessageId, targetAgents, intent, options) {
        ...
        yield { type: 'text', agentId: 'codex', content: 'I can help!', timestamp: Date.now() };
        yield {
          type: 'done',
          agentId: 'codex',
          content: '',
    ...
    assert.strictEqual(deliverCalls[0].agentId, 'codex', 'Should use actual speaker agentId, not trigger agentId');
  ...
  describe('queue mode (active invocation running)', () => {
    it('enqueues connector message when another cat is running', async () => {
      ...
      assert.deepStrictEqual(entries[0].targetAgents, ['opus']);
    });
    ...
    it('preempts active invocation for urgent connector triggers', async () => {
      ...
      assert.deepStrictEqual(trackerMock.cancelCalls[0], { threadId: 'thread-1', agentId: 'opus', userId: 'user-1' });
    });

    it('clears queue pause before urgent preempt replacement execution', async () => {
      trackerMock.setActive('thread-1', 'user-1');
      const clearPauseCalls = /** @type {Array<{threadId: string, agentId: string}>} */ ([]);
      const mockQueueProcessor = /** @type {any} */ ({
        clearPause(threadId, agentId) {
          clearPauseCalls.push({ threadId, agentId });
        },
      ...
      assert.deepStrictEqual(
        clearPauseCalls,
        [{ threadId: 'thread-1', agentId: 'opus' }],
    ...
    it('emits queue_full_warning when queue is full', async () => {
      ...
      // Use different targetAgents to prevent merge
    ...
    it('P1 fix: direct execution calls queueProcessor.onInvocationComplete on success', async () => {
      ...
      const qpCalls = /** @type {Array<{threadId: string, agentId: string, status: string}>} */ ([]);
      const mockQueueProcessor = /** @type {any} */ ({
        async onInvocationComplete(threadId, agentId, status) {
          qpCalls.push({ threadId, agentId, status });
        },
      });
