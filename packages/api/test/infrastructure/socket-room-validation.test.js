 ...
 */
const ROOM_PATTERN = /^(thread:|worktree:|workspace:global$|user:)/;
...
describe('socket room name validation', () => {
  ...
  it('allows workspace:global', () => {
    assert.ok(ROOM_PATTERN.test('workspace:global'));
  });
  ...
  it('rejects arbitrary room names', () => {
    ...
    assert.ok(!ROOM_PATTERN.test('workspace:other'));
