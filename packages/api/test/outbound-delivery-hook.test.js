...
describe('OutboundDeliveryHook', () => {
  ...
  it('delivers to multiple bindings for same thread', async () => {
    const dingtalkMock = mockAdapter('dingtalk');
    const adapters = new Map([
      ['feishu', feishuMock.adapter],
      ['dingtalk', dingtalkMock.adapter],
    ]);
    hook = new OutboundDeliveryHook({
      bindingStore,
      adapters,
    ...
    bindingStore.bind('dingtalk', 'chat-2', 'thread-abc', 'user-1');
    await hook.deliver('thread-abc', 'Hello!');

    assert.equal(feishuMock.sent.length, 1);
    assert.equal(dingtalkMock.sent.length, 1);
  });
  ...
  it('prepends cat display name prefix when agentId is provided', async () => {
    bindingStore.bind('feishu', 'chat-1', 'thread-abc', 'user-1');
  ...
  it('sends plain content when agentId is omitted (backward compat)', async () => {
    bindingStore.bind('feishu', 'chat-1', 'thread-abc', 'user-1');
    await hook.deliver('thread-abc', 'Hello!');
    assert.equal(feishuMock.sent[0].content, 'Hello!');
  });

  it('sends plain content when agentId is unknown', async () => {
    bindingStore.bind('feishu', 'chat-1', 'thread-abc', 'user-1');
  ...
  describe('file block delivery via sendMedia', () => {
    ...
    it('sends local absolute file paths from send_file_to_user blocks', async () => {
      const mediaSent = [];
      const mediaAdapter = {
        connectorId: 'weixin',
        async sendReply() {},
      ...
      hook = new OutboundDeliveryHook({
        bindingStore,
        adapters: new Map([['weixin', mediaAdapter]]),
        log: noopLog(),
        mediaPathResolver: () => undefined,
      });
      bindingStore.bind('weixin', 'chat-1', 'thread-abc', 'user-1');

      const blocks = [{ id: 'send-file-1', kind: 'file', v: 1, url: '/tmp/report.pdf', fileName: 'report.pdf' }];
      await hook.deliver('thread-abc', 'Report', 'opus', blocks);

      assert.equal(mediaSent.length, 1);
      assert.equal(mediaSent[0].payload.type, 'file');
      assert.equal(mediaSent[0].payload.absPath, '/tmp/report.pdf');
      assert.equal(mediaSent[0].payload.fileName, 'report.pdf');
    });
  });
