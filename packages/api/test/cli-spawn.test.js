...
test('spawnCli terminates repeated reasoning loops', async () => {
  const proc = createMockProcess();
  const spawnFn = createMockSpawnFn(proc);

  const promise = collect(spawnCli({ command: 'test-cli', args: [], timeoutMs: 5_000 }, { spawnFn }));
  ...
  const results = await promise;
  const loopError = results.find(
    (item) =>
      item &&
      typeof item === 'object' &&
      item.type === 'error' &&
      typeof item.message === 'string' &&
      item.message.startsWith('Reasoning loop detected:'),
  );

  assert.ok(loopError, 'repeated reasoning should emit a loop error event');
  assert.ok(proc.kill.mock.callCount() >= 1, 'repeated reasoning should terminate the child process');
});
