...
test('yields session_init, text, and done on basic success', async () => {
  ...
  assert.equal(msgs[0].agentId, 'codex');
...
test('surfaces reasoning loop detection as agent error', async () => {
  const proc = createMockProcess();
  const spawnFn = createMockSpawnFn(proc);
  const service = new CodexAgentService({ spawnFn });

  const promise = collect(service.invoke('convert this word doc to ppt'));

  proc.stdout.write(`${JSON.stringify({ type: 'thread.started', thread_id: 'thread-loop' })}\n`);
  for (let i = 0; i < 25; i += 1) {
    proc.stdout.write(
      `${JSON.stringify({
        type: 'item.completed',
        item: { type: 'reasoning', text: 'I should think more before using a tool.' },
      })}\n`,
    );
  }
  ...
  const msgs = await promise;
  const errMsg = msgs.find((m) => m.type === 'error');
  assert.ok(errMsg, 'reasoning loop should surface as an agent error');
  assert.ok(errMsg.error.includes('Reasoning loop detected:'), 'error should include loop diagnostic');
});
...
test('all messages have agentId codex', async () => {
  const proc = createMockProcess();
  const spawnFn = createMockSpawnFn(proc);
  const service = new CodexAgentService({ spawnFn });
  ...
  for (const msg of msgs) {
    assert.equal(msg.agentId, 'codex', `expected agentId codex for ${msg.type} message`);
  }
});
