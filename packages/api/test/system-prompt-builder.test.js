...
describe('SystemPromptBuilder', () => {
  ...
  test('returns empty string for unknown agentId', async () => {
    const build = await getBuilder();
    const prompt = build({
      agentId: 'unknown-cat',
      mode: 'independent',
  ...
  test('buildInvocationContext injects server-authoritative current time guidance', async () => {
    const { buildInvocationContext } = await import('../dist/domains/agents/services/context/SystemPromptBuilder.js');
    const ctx = buildInvocationContext({
      catId: 'opus',
      mode: 'independent',
      teammates: [],
      mcpAvailable: false,
      runtimeClock: { nowMs: Date.UTC(2026, 3, 21, 5, 0, 0), timezone: 'Asia/Shanghai' },
    });
    assert.match(ctx, /当前真实时间：.*ISO=2026-04-21T05:00:00.000Z/);
    assert.ok(ctx.includes('不要根据上下文或历史消息自行推断'));
  });
  ...
  test('buildInvocationContext injects mention routing feedback when provided', async () => {
    ...
    const ctx = buildInvocationContext({
      ...
      mentionRoutingFeedback: {
        ...
        items: [{ targetAgentId: 'opus', reason: 'no_action' }],
      },
    });
  ...
  test('buildReviewerSection returns reviewer list for opus (different family reviewers)', async () => {
    const { buildReviewerSection } = await import('../dist/domains/agents/services/context/SystemPromptBuilder.js');
  ...
  test('buildInvocationContext injects most-recently-active participant', async () => {
    ...
    const ctx = buildInvocationContext({
      ...
      mcpAvailable: false,
      activeParticipants: [
        { agentId: 'opus', lastMessageAt: 2000, messageCount: 5 },
        { agentId: 'codex', lastMessageAt: 1000, messageCount: 3 },
      ],
    });
  ...
  test('buildInvocationContext skips self in activity list', async () => {
    ...
    const ctx = buildInvocationContext({
      ...
      mcpAvailable: false,
      activeParticipants: [
        { agentId: 'opus', lastMessageAt: 3000, messageCount: 8 },
        { agentId: 'codex', lastMessageAt: 2000, messageCount: 4 },
      ],
    });
  ...
  test('buildInvocationContext omits hint when only self has activity', async () => {
    ...
    const ctx = buildInvocationContext({
      ...
      mcpAvailable: false,
      activeParticipants: [
        { agentId: 'opus', lastMessageAt: 1000, messageCount: 1 },
        { agentId: 'codex', lastMessageAt: 0, messageCount: 0 },
      ],
    });
  ...
  test('buildSystemPrompt size with activeParticipants stays under 3250 chars (raised for F102-D17 MCP tools section)', async () => {
    ...
    const prompt = build({
      ...
      promptTags: ['critique'],
      activeParticipants: [
        { agentId: 'codex', lastMessageAt: Date.now(), messageCount: 5 },
        { agentId: 'opus', lastMessageAt: Date.now() - 1000, messageCount: 3 },
      ],
    });
  ...
  test('buildSystemPrompt size stays under 3300 chars with SOP hint (raised for F102-D17 MCP tools section)', async () => {
    ...
    const prompt = build({
      ...
      mcpAvailable: true,
      promptTags: ['critique'],
      activeParticipants: [{ agentId: 'codex', lastMessageAt: Date.now(), messageCount: 5 }],
      sopStageHint: {
        stage: 'quality_gate',
