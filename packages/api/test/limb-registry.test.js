...
describe('LimbRegistry invoke pipeline (Phase B)', () => {
  ...
  it('free capability succeeds without lease', async () => {
    ...
    const result = await registry.invoke('iphone-1', 'location.get', {}, { agentId: 'opus' });
    assert.equal(result.success, true);
  ...
  it('leased capability acquires and auto-releases lease after invoke', async () => {
    ...
    const result = await registry.invoke('iphone-1', 'camera.snap', {}, { agentId: 'opus' });
    assert.equal(result.success, true);
  ...
  it('leased capability allows sequential invokes from different cats (auto-release)', async () => {
    ...
    const r1 = await registry.invoke('iphone-1', 'camera.snap', {}, { agentId: 'opus' });
    assert.equal(r1.success, true);

    // Second cat can invoke because lease was auto-released
    const r2 = await registry.invoke('iphone-1', 'camera.snap', {}, { agentId: 'codex' });
  ...
  it('unknown command rejected by whitelist', async () => {
    ...
    const result = await registry.invoke('iphone-1', 'camera.unknown', {}, { agentId: 'opus' });
    assert.equal(result.success, false);
  ...
  it('gated capability rejects', async () => {
    ...
    const result = await registry.invoke('iphone-1', 'sms.send', {}, { agentId: 'opus' });
    assert.equal(result.success, false);
  ...
  it('policy override changes auth level', async () => {
    ...
    policy.setPolicy({ agentId: 'opus', nodeId: 'iphone-1', capability: 'camera', authLevel: 'free' });
  ...
  it('invoke records action log entry on success', async () => {
    ...
    await registry.invoke('iphone-1', 'camera.snap', {}, { agentId: 'opus', invocationId: 'inv-123' });
    const entries = actionLog.getByCat('opus');
    assert.equal(entries.length, 1);
    assert.equal(entries[0].status, 'completed');
    assert.equal(entries[0].agentId, 'opus');
  ...
  it('invoke records action log entry on failure', async () => {
    ...
    await registry.invoke('iphone-1', 'camera.snap', {}, { agentId: 'opus' });
    const entries = actionLog.getByCat('opus');
    assert.equal(entries.length, 1);
