...
function createLongToolService(agentId, { delayMs = 0 } = {}) {
  return {
    async *invoke() {
      yield {
        type: 'system_info',
      ...
      yield { type: 'tool_use', agentId, toolName: 'long_running', toolInput: '{}', timestamp: Date.now() };
      if (delayMs > 0) {
        await new Promise((resolve) => setTimeout(resolve, delayMs));
      }
      yield { type: 'tool_result', agentId, content: 'done after long wait', timestamp: Date.now() };
...
describe('Issue #83: /queue activeInvocations', () => {
  it('InvocationTracker.getActiveSlots returns active agentIds', async () => {
