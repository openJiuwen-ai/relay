...
describe('OpenCodeAgentService', () => {
  test('yields session_init, text, done from opencode events', async () => {
    ...
    const service = new OpenCodeAgentService({ agentId: 'opencode', spawnFn, model: 'claude-haiku-4-5' });
    ...
    assert.strictEqual(textMsg.agentId, 'opencode');
  });
  ...
  test('OPENCODE_CONFIG in callbackEnv clears ANTHROPIC env vars', async () => {
    const proc = createMockProcess();
    const spawnFn = mock.fn(() => proc);
    const service = new OpenCodeAgentService({ agentId: 'opencode', spawnFn, model: 'maas/glm-5' });
    const promise = collect(
      service.invoke('Test', {
        callbackEnv: {
