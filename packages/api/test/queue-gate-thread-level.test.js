 ...
 *
 * Fix: Use has(threadId) (no agentId) to check if ANY cat is active in the thread.
 */
...
function buildDeps(overrides = {}) {
  ...
  return {
    ...
    invocationTracker: {
      ...
      has: mock.fn((threadId, agentId) => {
        if (agentId === undefined) {
          // Thread-level check: any slot active?
          return threadId === 'thread-1';
        }
        // Slot-level check: only gemini is active
        return threadId === 'thread-1' && agentId === 'gemini';
      }),
      cancel: mock.fn(() => ({ cancelled: true, agentIds: ['opus'] })),
...
describe('Queue gate: thread-level enqueue (regression)', () => {
  ...
  it('no cat active in thread → message executes immediately', async () => {
    ...
    deps.invocationTracker.has.mock.mockImplementation((_threadId, _agentId) => false);
