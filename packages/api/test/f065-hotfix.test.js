...
describe('P1: Handoff digest sanitize', () => {
  test('sanitizeHandoffBody strips control characters', async () => {
    const { sanitizeHandoffBody } = await import('../dist/domains/agents/services/session/SessionBootstrap.js');
  ...
  test('bootstrap uses data-marker for handoff section', async () => {
    const { buildSessionBootstrap } = await import('../dist/domains/agents/services/session/SessionBootstrap.js');
    const fakeChainStore = {
      getChain: mock.fn(async () => [
        { id: 'prev-1', threadId: 'thread-1', agentId: 'opus', seq: 1, status: 'sealed', startedAt: Date.now() - 60000 },
        { id: 'current', threadId: 'thread-1', agentId: 'opus', seq: 2, status: 'active', startedAt: Date.now() },
      ]),
...
describe('P2-2: Input token cap', () => {
  test('buildPromptContent truncates when input exceeds cap', async () => {
    const { buildPromptContent } = await import('../dist/domains/agents/services/session/HandoffDigestGenerator.js');
    // Create large inputs
  ...
  test('generator system prompt includes no-directives constraint', async () => {
    const { SYSTEM_PROMPT } = await import('../dist/domains/agents/services/session/HandoffDigestGenerator.js');
    assert.ok(
