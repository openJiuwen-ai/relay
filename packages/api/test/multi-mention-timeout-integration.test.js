...
describe('Multi-Mention Timeout Integration', () => {
  const opus = createAgentId('opus');
  const codex = createAgentId('codex');
  const gemini = createAgentId('gemini');
  ...
  test('timeout after partial: preserves received, marks missing as timeout', () => {
    ...
    const codexResp = result.responses.find((r) => r.agentId === codex);
    const geminiResp = result.responses.find((r) => r.agentId === gemini);
