...
import { collectConfigSnapshot } from '../dist/config/ConfigRegistry.js';
import { resetLongTermMemoryEnabledForTest } from '../dist/config/memory-toggle-state.js';
import { MemoryStore } from '../dist/domains/agents/services/stores/ports/MemoryStore.js';
...
describe('Memory API Routes', () => {
  ...
  it('POST /api/memory with cat updatedBy works', async () => {
    ...
    assert.equal(res.statusCode, 201);
    const body = res.json();
    // Note: createAgentId wraps the string, but JSON serialization shows the underlying value
  ...
  it('POST /api/handle_memory_toggle updates the runtime memory flag', async () => {
    const res = await app.inject({
      method: 'POST',
      url: '/api/handle_memory_toggle',
      headers: { 'x-office-claw-user': 'test-user' },
    ...
    assert.equal(collectConfigSnapshot().memory.enabled, false);
  });
