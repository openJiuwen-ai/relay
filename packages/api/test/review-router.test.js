...
/** @returns {import('../dist/domains/agents/services/stores/ports/ThreadStore.js').IThreadStore} */
...
/** @returns {{ store: import('../dist/domains/agents/services/stores/ports/MessageStore.js').IMessageStore, messages: Array<{threadId: string, userId: string, content: string, mentions: string[], timestamp: number, source?: object}> }} */
...
describe('ReviewRouter', () => {
  ...
  describe('dedup', () => {
    ...
    it('unregistered PR does not claim dedup window (#668 P1)', async () => {
      ...
      if (r2.kind === 'routed') {
        assert.strictEqual(r2.agentId, 'opus');
  ...
  describe('Layer 1: PrTrackingStore registry', () => {
    it('routes to tracked thread+cat', async () => {
      ...
      if (result.kind === 'routed') {
        ...
        assert.strictEqual(result.agentId, 'opus');
  ...
  describe('priority', () => {
    it('registry takes priority even when cat tag is present', async () => {
      ...
      if (result.kind === 'routed') {
        ...
        assert.strictEqual(result.agentId, 'codex');
  ...
  describe('realtime connector event', () => {
    ...
    it('does not broadcast when PR is unregistered', async () => {
      ...
      await router.route(makeEvent({ catTag: undefined, agentId: undefined }));
