...
function createMockMessageStore() {
  ...
  return {
    ...
    getMentionsFor: (agentId, limit = 50) =>
      sorted()
        .filter((m) => m.mentions?.includes(agentId))
        .slice(-limit),
...
function createMockThreadStore(
  ...
  return {
    ...
    list: () => [],
    listByProject: () => [],
    addParticipants: (threadId, agentIds) => {
      if (!participants[threadId]) participants[threadId] = [];
      const now = Date.now();
      for (const agentId of agentIds) {
        if (!participants[threadId].includes(agentId)) {
          participants[threadId].push(agentId);
        }
        // Track activity
...
function createMockAgentService(agentId, responseText = 'Hello from mock') {
  const invoke = mock.fn(async function* (_prompt, options) {
    const sessionId = options?.sessionId ?? `${agentId}-session-new`;
    yield {
      type: 'session_init',
      agentId,
      sessionId,
...
describe('AgentRouter', () => {
  test('routingPolicy(review) avoids opus when default routing would pick opus', async () => {
    ...
    const { targetAgents } = await router.resolveTargetsAndIntent('帮我 review 一下', 'thread-policy');
    assert.equal(targetAgents[0], 'codex', 'Should pick deterministic non-opus fallback (codex) when opus is avoided');
  });
  ...
  test('routingPolicy(review) does not trigger on words containing "pr" like "prompt"', async () => {
    ...
    const { targetAgents } = await router.resolveTargetsAndIntent('prompt engineering 这块怎么做', 'thread-policy');
    assert.equal(targetAgents[0], 'opus', 'Should not classify "prompt" as PR/review scope');
  });
  ...
  test('routingPolicy tolerates malformed avoid/prefer lists without crashing', async () => {
    ...
    const { targetAgents } = await router.resolveTargetsAndIntent('请 review 这次改动', 'thread-malformed');
    assert.equal(targetAgents[0], 'opus');
  });
  ...
  test('routingPolicy(architecture) prefers opus even when participants would route elsewhere', async () => {
    ...
    const { targetAgents } = await router.resolveTargetsAndIntent('这个架构 tradeoff 怎么选', 'thread-arch');
    assert.equal(targetAgents[0], 'opus', 'Should prefer opus first for architecture scope');
    assert.ok(targetAgents.includes('codex'), 'Should keep existing participant after preferred cat');
  });
  ...
  test('routingPolicy does not override explicit @mention', async () => {
    ...
    const { targetAgents } = await router.resolveTargetsAndIntent('@opus 帮我 review', 'thread-mention');
    assert.deepEqual(targetAgents, ['opus']);
  });
  ...
  test('routes to opus (default) when no @ mention is present', async () => {
    ...
    assert.ok(messages.every((m) => m.agentId === 'opus'));
  });
  ...
  test('routes to codex when @codex is mentioned', async () => {
    ...
    assert.equal(mockGeminiService.invoke.mock.callCount(), 0);
    assert.ok(messages.every((m) => m.agentId === 'codex'));
  });
  ...
  test('routes to gemini when @gemini is mentioned', async () => {
    ...
    assert.ok(messages.every((m) => m.agentId === 'gemini'));
  });
  ...
  test('executes multiple cats in order when multiple @ mentions are present (#execute)', async () => {
    ...
    assert.equal(textMessages[0].agentId, 'opus');
    assert.equal(textMessages[1].agentId, 'codex');
  });
  ...
  test('multi-cat serial chain hides previous stream responses in play mode (#execute)', async () => {
    ...
    const mockCodexService = {
      invoke: mock.fn(async function* (prompt) {
        ...
        yield { type: 'session_init', agentId: 'codex', sessionId: 'codex-123', timestamp: Date.now() };
        yield { type: 'text', agentId: 'codex', content: 'Codex reviewed', timestamp: Date.now() };
  ...
  test('stores and uses session IDs per user per cat', async () => {
    ...
    const mockClaudeService = {
      invoke: mock.fn(async function* (_prompt, options) {
        ...
        yield { type: 'session_init', agentId: 'opus', sessionId: 'opus-session-1', timestamp: Date.now() };
  ...
  test('maintains separate sessions for different users', async () => {
    ...
    const mockClaudeService = {
      invoke: mock.fn(async function* (_prompt, options) {
        ...
        yield { type: 'session_init', agentId: 'opus', sessionId, timestamp: Date.now() };
        yield { type: 'text', agentId: 'opus', content: 'Hello', timestamp: Date.now() };
        yield { type: 'done', agentId: 'opus', timestamp: Date.now() };
      }),
    };
  ...
  test('invokes all three cats for triple mention (parallel, no order guarantee)', async () => {
    ...
    const agentIds = textMessages.map((m) => m.agentId).sort();
    assert.deepEqual(agentIds, ['codex', 'gemini', 'opus']);
  });
  ...
  test('continues chain when first cat throws an error', async () => {
    ...
    assert.equal(errors[0].agentId, 'opus');
  ...
  test('F078: no @ mention returns only last replier (most recent by activity)', async () => {
    ...
    assert.equal(result.targetAgents[0], 'codex', 'Most recently active cat (codex) should be the target');
    assert.equal(result.targetAgents.length, 1, 'F078: only last replier, not all participants');
  });
  ...
  test('error from first cat is not passed as context to second cat', async () => {
    ...
    const mockCodexService = {
      invoke: mock.fn(async function* (prompt) {
        codexReceivedPrompt = prompt;
        yield { type: 'session_init', agentId: 'codex', sessionId: 'c-1', timestamp: Date.now() };
        yield { type: 'text', agentId: 'codex', content: 'done', timestamp: Date.now() };
        yield { type: 'done', agentId: 'codex', timestamp: Date.now() };
      }),
    };
    const mockGeminiService = createMockAgentService('gemini');
  ...
  test('identity injection: codex prompt in serial chain contains Codex (#execute)', async () => {
    ...
    const mockCodexService = {
      invoke: mock.fn(async function* (prompt, options) {
        ...
        yield { type: 'text', agentId: 'codex', content: 'codex says hi', timestamp: Date.now() };
        yield { type: 'done', agentId: 'codex', timestamp: Date.now() };
      }),
    };
  ...
  test('parallel: 2 cats both invoked with mode=parallel (auto ideate)', async () => {
    ...
    const mockClaudeService = {
      invoke: mock.fn(async function* (prompt) {
        opusPrompt = prompt;
        yield { type: 'text', agentId: 'opus', content: 'Opus thinks', timestamp: Date.now() };
        yield { type: 'done', agentId: 'opus', timestamp: Date.now() };
      }),
    };
    const mockCodexService = {
      invoke: mock.fn(async function* (prompt) {
        codexPrompt = prompt;
        yield { type: 'text', agentId: 'codex', content: 'Codex thinks', timestamp: Date.now() };
        yield { type: 'done', agentId: 'codex', timestamp: Date.now() };
      }),
    };
  ...
  test('parallel: codex does NOT see opus response (independent thinking)', async () => {
    ...
    const mockCodexService = {
      invoke: mock.fn(async function* (prompt) {
        codexPrompt = prompt;
        yield { type: 'text', agentId: 'codex', content: 'Codex response', timestamp: Date.now() };
        yield { type: 'done', agentId: 'codex', timestamp: Date.now() };
      }),
    };
  ...
  test('parallel: #execute forces serial mode metadata even with multiple cats', async () => {
    ...
    const mockCodexService = {
      invoke: mock.fn(async function* (prompt) {
        codexPrompt = prompt;
        yield { type: 'text', agentId: 'codex', content: 'Serial codex', timestamp: Date.now() };
        yield { type: 'done', agentId: 'codex', timestamp: Date.now() };
      }),
    };
  ...
  test('parallel: all cat responses are stored in messageStore', async () => {
    ...
    const appendedAgentIds = appendedMessages.map((m) => m.agentId).filter(Boolean);
    assert.ok(appendedAgentIds.includes('opus'), 'Opus response should be stored');
    assert.ok(appendedAgentIds.includes('codex'), 'Codex response should be stored');
  });
  ...
  test('context history: serial multi-cat — both cats receive history', async () => {
    ...
    const mockClaudeService = {
      invoke: mock.fn(async function* (prompt) {
        opusPrompt = prompt;
        yield { type: 'text', agentId: 'opus', content: 'opus reply', timestamp: Date.now() };
        yield { type: 'done', agentId: 'opus', timestamp: Date.now() };
      }),
    };
    const mockCodexService = {
      invoke: mock.fn(async function* (prompt) {
        codexPrompt = prompt;
        yield { type: 'text', agentId: 'codex', content: 'codex reply', timestamp: Date.now() };
        yield { type: 'done', agentId: 'codex', timestamp: Date.now() };
      }),
    };
  ...
  test('context history: parallel multi-cat — both cats receive history', async () => {
    ...
    const mockClaudeService = {
      invoke: mock.fn(async function* (prompt) {
        opusPrompt = prompt;
        yield { type: 'text', agentId: 'opus', content: 'a', timestamp: Date.now() };
        yield { type: 'done', agentId: 'opus', timestamp: Date.now() };
      }),
    };
    const mockCodexService = {
      invoke: mock.fn(async function* (prompt) {
        codexPrompt = prompt;
        yield { type: 'text', agentId: 'codex', content: 'b', timestamp: Date.now() };
        yield { type: 'done', agentId: 'codex', timestamp: Date.now() };
      }),
    };
  ...
  test('parallel: resolveTargetsAndIntent returns correct intent', async () => {
    ...
    assert.equal(result1.targetAgents.length, 2);
...
describe('F078: Default to last replier', () => {
  test('no @mention routes to most recent replier only (not all participants)', async () => {
    ...
    const { targetAgents } = await router.resolveTargetsAndIntent('hello', 't1');
    assert.deepStrictEqual(targetAgents, ['opus'], 'should route to last replier only');
  });
  ...
  test('no participants defaults to opus', async () => {
    ...
    const { targetAgents } = await router.resolveTargetsAndIntent('hello', 't1');
    assert.deepStrictEqual(targetAgents, ['opus']);
  });

  test('no @mention falls back to latest visible agent message when activity is missing', async () => {
    const { AgentRouter } = await import('../dist/domains/agents/services/agents/routing/AgentRouter.js');
    ...
    const { targetCats } = await router.resolveTargetsAndIntent('同意', 't1');
    assert.deepStrictEqual(targetCats, ['codex']);
  });
  ...
  test('no @mention skips unavailable last replier and preferred cats', async () => {
    await withAvailabilityConfig({ codex: false }, async () => {
      ...
      const { targetAgents } = await router.resolveTargetsAndIntent('hello', 't1');
      assert.deepStrictEqual(targetAgents, ['gemini'], 'should skip unavailable last replier/preferred cats');
    });
  });
  ...
  test('explicit @mention still overrides last-replier default', async () => {
    ...
    const { targetAgents } = await router.resolveTargetsAndIntent('@codex 帮我看看', 't1');
    assert.deepStrictEqual(targetAgents, ['codex'], 'explicit @mention should override');
  });
});
...
describe('F078: Group mentions', () => {
  test('@all routes to all registered cats', async () => {
    ...
    const { targetAgents } = await router.resolveTargetsAndIntent('@all 大家好');
    assert.ok(targetAgents.length >= 3, 'should route to all registered cats');
    assert.ok(targetAgents.includes('opus'));
    assert.ok(targetAgents.includes('codex'));
    assert.ok(targetAgents.includes('gemini'));
  });
  ...
  test('@all skips unavailable cats', async () => {
    await withAvailabilityConfig({ codex: false }, async () => {
      ...
      assert.ok(targetAgents.includes('opus'));
      assert.ok(targetAgents.includes('gemini'));
      assert.ok(!targetAgents.includes('codex'), 'unavailable cat should be excluded from @all routing');
    });
  });
  ...
  test('@全体 routes to all registered cats', async () => {
    ...
    const { targetAgents } = await router.resolveTargetsAndIntent('@全体 大家好');
    assert.ok(targetAgents.length >= 3);
    assert.ok(targetAgents.includes('opus'));
  });
  ...
  test('@全体Claude routes to all ragdoll variants', async () => {
    ...
    const { officeClawRegistry, createAgentId } = await import('@openjiuwen/relay-shared');
    if (!officeClawRegistry.has('sonnet')) {
      officeClawRegistry.register('sonnet', {
        id: createAgentId('sonnet'),
        name: 'sonnet',
        displayName: 'Claude',
    ...
    const { targetAgents } = await router.resolveTargetsAndIntent('@全体Claude 你们好');
    assert.ok(targetAgents.includes('opus'), 'should include opus (ragdoll)');
    assert.ok(targetAgents.includes('sonnet'), 'should include sonnet (ragdoll)');
    assert.ok(!targetAgents.includes('codex'), 'should NOT include codex (maine-coon)');
    assert.ok(!targetAgents.includes('gemini'), 'should NOT include gemini (siamese)');
  });
  ...
  test('@all-ragdoll routes to ragdoll variants', async () => {
    ...
    const { targetAgents } = await router.resolveTargetsAndIntent('@all-ragdoll hello');
    assert.ok(targetAgents.includes('opus'));
    assert.ok(targetAgents.includes('sonnet'));
    assert.ok(!targetAgents.includes('codex'));
  });
  ...
  test('@thread routes to current thread participants', async () => {
    ...
    const { targetAgents } = await router.resolveTargetsAndIntent('@thread 大家看看', 't1');
    assert.deepStrictEqual(new Set(targetAgents), new Set(['opus', 'codex']));
  });
  ...
  test('@本帖 routes to thread participants', async () => {
    ...
    const { targetAgents } = await router.resolveTargetsAndIntent('@本帖 看看', 't1');
    assert.deepStrictEqual(new Set(targetAgents), new Set(['opus', 'gemini']));
  });
  ...
  test('@全体参与者 routes to thread participants', async () => {
    ...
    const { targetAgents } = await router.resolveTargetsAndIntent('@全体参与者 看看', 't1');
    assert.deepStrictEqual(new Set(targetAgents), new Set(['codex', 'gemini']));
  });

  test('legacy route() uses gatewayIdentity-scoped catalog members', async () => {
    const { AgentRouter } = await import('../dist/domains/agents/services/agents/routing/AgentRouter.js');
    const { AgentRegistry } = await import('../dist/domains/agents/services/agents/registry/AgentRegistry.js');
    ...
    const catalogProvider = {
      id: 'test-catalog',
      async readCatalog() {
        throw new Error('readCatalog should not be used when listRoutableMembers is provided');
      },
      async writeCatalog() {},
    ...
    const agentRegistry = new AgentRegistry();
    agentRegistry.register('codex', createMockAgentService('codex', 'scoped hello'));
    ...
    const outputs = [];
    for await (const msg of router.route('user-1', '@scope-only hello', 'thread-scope', undefined, undefined, undefined, {
      gatewayIdentity: { userId: 'scoped-user' },
    })) {
      outputs.push(msg);
    }

    assert.ok(outputs.some((msg) => msg.agentId === 'codex' && msg.type === 'text'), 'scoped route should reach codex');
  });
  ...
  test('@thread with no participants falls back to default cat', async () => {
    ...
    const { targetAgents } = await router.resolveTargetsAndIntent('@thread hello', 't1');
    assert.deepStrictEqual(targetAgents, ['opus'], 'no participants → fallback to default');
  });
  ...
  test('group mentions only include cats with registered services', async () => {
    ...
    assert.ok(targetAgents.includes('opus'));
    assert.ok(targetAgents.includes('codex'));
    assert.ok(!targetAgents.includes('gemini'), 'gemini has no service, should be excluded');
  });
  ...
  test('@allison does NOT trigger @all (token boundary)', async () => {
    ...
    const { targetAgents } = await router.resolveTargetsAndIntent('@allison hi');
    // @allison is not a known mention — should fall back to default, NOT trigger @all
    assert.ok(!targetAgents.includes('codex'), '@allison should not broadcast to all cats');
    assert.ok(!targetAgents.includes('gemini'), '@allison should not broadcast to all cats');
  });
  ...
  test('@threadsafe does NOT trigger @thread (token boundary)', async () => {
    ...
    const { targetAgents } = await router.resolveTargetsAndIntent('@threadsafe hi', 't1');
    // Should NOT route to thread participants — @threadsafe is not @thread
    assert.equal(targetAgents.length, 1, '@threadsafe should not trigger group mention');
  });
  ...
  test('@all-ragdollish does NOT trigger @all-ragdoll (token boundary)', async () => {
    ...
    const { targetAgents } = await router.resolveTargetsAndIntent('@all-ragdollish hi');
    // Should NOT trigger @all-ragdoll breed group
    assert.ok(!targetAgents.includes('sonnet'), '@all-ragdollish should not match @all-ragdoll');
  });
  ...
  test('@全体Claude咪 does NOT trigger @全体Claude (token boundary)', async () => {
    ...
    const { targetAgents } = await router.resolveTargetsAndIntent('@全体Claude咪 hi');
    // 咪 is not a boundary char — should NOT match @全体Claude
    assert.equal(targetAgents.length, 1, '@全体Claude咪 should not trigger breed group');
  });
});
...
describe('#58: preferredCats candidate scope (not dispatch list)', () => {
  test('multi preferredCats + last replier in preferred set → routes to last replier only', async () => {
    ...
    const { targetAgents } = await router.resolveTargetsAndIntent('hello', 't1');
    assert.deepStrictEqual(targetAgents, ['codex'], 'should route to last replier, not all preferred cats');
  });
  ...
  test('@mention overrides preferredCats', async () => {
    ...
    const { targetAgents } = await router.resolveTargetsAndIntent('@codex review this', 't1');
    assert.deepStrictEqual(targetAgents, ['codex'], '@mention should override preferredCats');
  });
  ...
  test('no preferredCats preserves existing last-replier behavior', async () => {
    ...
    const { targetAgents } = await router.resolveTargetsAndIntent('hello', 't1');
    assert.deepStrictEqual(targetAgents, ['gemini'], 'without preferredCats, last replier should still work');
  });
  ...
  test('@全体Claude still triggers parallel dispatch even with preferredCats', async () => {
    ...
    const { targetAgents } = await router.resolveTargetsAndIntent('@全体Claude discuss this', 't1');
    // @全体Claude is a breed group mention — should override preferredCats and route to all ragdolls
    assert.ok(targetAgents.length > 1, '@全体Claude should still trigger multi-cat dispatch');
    assert.ok(targetAgents.includes('opus'), 'should include opus');
  });
  ...
  test('explicit #ideate with multi preferredCats dispatches all preferred cats', async () => {
    ...
    const { targetAgents, intent } = await router.resolveTargetsAndIntent('#ideate discuss this together', 't1');
    assert.deepStrictEqual(targetAgents.sort(), ['codex', 'opus'], '#ideate should dispatch all preferred cats');
  ...
  test('no #ideate with multi preferredCats still routes to single cat', async () => {
    ...
    const { targetAgents } = await router.resolveTargetsAndIntent('just a normal message', 't1');
    assert.equal(targetAgents.length, 1, 'without #ideate, should still route to single cat');
  });
  ...
  test('refreshFromRegistry updates routable service set after runtime catalog changes', async () => {
    ...
    assert.deepEqual(before.targetAgents, ['codex'], 'codex mention should be parsed from officeClawRegistry');
    ...
    assert.deepEqual(after.targetAgents, ['codex'], 'codex should be routable after refresh');
  });
});
