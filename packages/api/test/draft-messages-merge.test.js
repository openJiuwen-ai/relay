...
import { DraftStore } from '../dist/domains/agents/services/stores/ports/DraftStore.js';
...
describe('GET /api/messages — draft merge (#80)', () => {
  ...
  it('includes active drafts on first page (no cursor)', async () => {
    ...
    assert.equal(draft.agentId, 'opus');
  });
  ...
  it('draft response includes extra.taskRuns for in-flight task grouping after refresh', async () => {
    const ts = Date.now();
    const taskRuns = {
      v: 1,
      segments: [
    ...
    draftStore.upsert({
      userId: 'user-1',
      threadId: 'thread-1',
      invocationId: 'inv-taskruns',
      catId: 'opus',
      content: 'Partial answer',
      thinking: 'Flat thinking fallback',
      taskRuns,
      updatedAt: ts + 100,
    });
    ...
    assert.equal(res.statusCode, 200);
    const body = JSON.parse(res.body);
    const draft = body.messages.find((m) => m.id === 'draft-inv-taskruns');
    assert(draft, 'Draft should be present');
    assert.equal(draft.extra?.stream?.invocationId, 'inv-taskruns');
    assert.equal(draft.extra?.taskRuns?.v, 1);
    assert.equal(draft.extra?.taskRuns?.segments?.length, 1);
    assert.equal(draft.extra?.taskRuns?.segments[0].taskId, 'step:1');
  });
  ...
  it('multiple concurrent drafts sorted by updatedAt', async () => {
    ...
    assert.equal(drafts[0].agentId, 'codex');
    assert.equal(drafts[1].agentId, 'opus');
  });
});
