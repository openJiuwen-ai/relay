 ...
 *
 * PATCH /api/threads/:threadId/sessions/:agentId/bind
...
describe('Session bind API route', () => {
  ...
  test('returns 400 for invalid agentId', async () => {
  ...
  test('creates new session when no active exists', async () => {
    ...
    try {
      ...
      assert.equal(body.session.status, 'active');
      assert.equal(body.session.agentId, 'opus');
  ...
  test('supports all valid agentIds', async () => {
    const { app, threadStore } = await buildApp();
    try {
      const thread = await threadStore.create('user-1', 'Test');

      for (const agentId of ['opus', 'codex', 'gemini']) {
        const res = await app.inject({
          method: 'PATCH',
          url: `/api/threads/${thread.id}/sessions/${agentId}/bind`,
          headers: { 'x-office-claw-user': 'user-1' },
          payload: { cliSessionId: `cli-${agentId}` },
        });

        assert.equal(res.statusCode, 200, `Expected 200 for ${agentId}`);
        const body = JSON.parse(res.payload);
        assert.equal(body.session.agentId, agentId);
      }
    } finally {
      await app.close();
    }
