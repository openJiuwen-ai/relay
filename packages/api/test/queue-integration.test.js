...
import { QueueProcessor } from '../dist/domains/agents/services/agents/invocation/QueueProcessor.js';
...
function mockRouter(opts = {}) {
  ...
  return {
    ...
    /** @type {any} */
    router: {
      async *routeExecution(userId, message, threadId, userMessageId, targetAgents, intent, _options) {
        ...
        yield { type: 'text', agentId: targetAgents[0], content: `Processed: ${message}`, timestamp: Date.now() };
        yield {
          type: 'done',
  ...
  return {
    ...
    tracker: {
      start(threadId, agentId, userId, agentIds) {
        starts.push({ threadId, agentId, userId, agentIds });
        activeThreads.add(threadId);
...
describe('Queue Integration (E2E scenarios)', () => {
  ...
  it('bugfix: ConnectorInvokeTrigger abort mid-loop → should NOT ack or mark succeeded', async () => {
    ...
    const ackCalls = /** @type {any[]} */ ([]);
    const customRouter = {
      async *routeExecution(_userId, _message, _threadId, _userMessageId, _targetAgents, _intent, _options) {
  ...
  it('bugfix: autoExecute entry orphaned when target cat busy at enqueue → recovered on completion', async () => {
    ...
    assert.deepStrictEqual(routerMock.calls[0].targetAgents, ['gpt52']);
  });
  ...
  it('bugfix: multi-cat autoExecute — tryExecuteNextAcrossUsers only picks one, re-scan picks the rest', async () => {
    ...
    const perSlotTracker = {
      start(threadId, agentId) {
        activeSlots.add(`${threadId}:${agentId}`);
        return new AbortController();
      },
      complete(threadId, agentId, _controller) {
        activeSlots.delete(`${threadId}:${agentId}`);
      },
      has(threadId, agentId) {
        if (agentId) return activeSlots.has(`${threadId}:${agentId}`);
