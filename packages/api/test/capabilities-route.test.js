...
describe('PATCH capabilities logic', () => {
  ...
  it('adds per-cat override', async () => {
    ...
    assert.ok(cap);
    cap.overrides = [{ agentId: 'codex', enabled: false }];
    await writeCapabilitiesConfig(dir, config);

    const updated = await readCapabilitiesConfig(dir);
    assert.ok(updated);
    assert.equal(updated.capabilities[0]?.overrides?.[0]?.agentId, 'codex');
  ...
  it('removes no-op override that matches global', async () => {
      ...
      capabilities: [
        {
          ...
          source: 'external',
          mcpServer: { command: 'echo', args: [] },
          overrides: [{ agentId: 'opus', enabled: false }],
        },
      ],
    });
    ...
    if (cap.overrides) {
      const ov = cap.overrides.find((o) => o.agentId === 'opus');
  ...
  it('skips prune when any scan failed (allScansOk=false)', async () => {
    ...
    assert.equal(skill.overrides?.[0]?.agentId, 'opus', 'Saved overrides preserved');
  });
...
describe('GET /api/capabilities (Fastify)', () => {
  ...
  it('returns CapabilityBoardResponse with items and catFamilies', async () => {
    ...
    for (const family of body.catFamilies) {
      ...
      assert.ok(Array.isArray(family.agentIds), 'family should have agentIds array');
    }
      ...
      capabilities: [
        {
          ...
          enabled: false,
          source: 'external',
          overrides: [{ agentId: 'codex', enabled: true }],
          mcpServer: {
            command: 'node',
            args: ['--input-type=module', '--eval', probeCode],
            workingDir: process.cwd(),
