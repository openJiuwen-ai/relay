...
import { buildSessionBootstrap } from '../dist/domains/agents/services/session/SessionBootstrap.js';

// --- Mock SessionChainStore ---

function createMockSessionChainStore(sessions = []) {
  return {
    getActive(agentId, threadId) {
      return sessions.find((s) => s.agentId === agentId && s.threadId === threadId && s.status === 'active') ?? null;
    },
    getChain(agentId, threadId) {
      return sessions.filter((s) => s.agentId === agentId && s.threadId === threadId).sort((a, b) => a.seq - b.seq);
    },
  };
}
  ...
  describe('buildSessionBootstrap', () => {
    it('returns null for first session (seq=0, no prior context)', async () => {
      const store = createMockSessionChainStore([
        { id: 'sess-0', agentId: 'opus', threadId: 'thread-1', status: 'active', seq: 0 },
      ]);
    ...
    it('returns bootstrap when no active session but sealed sessions exist (post-seal gap)', async () => {
      ...
      const store = createMockSessionChainStore([
        { id: 'sess-0', agentId: 'opus', threadId: 'thread-1', status: 'sealed', seq: 0 },
      ]);
      const reader = createMockTranscriptReader();
    ...
    it('returns bootstrap with identity for second session (seq=1 → display #2)', async () => {
      const store = createMockSessionChainStore([
        { id: 'sess-0', agentId: 'opus', threadId: 'thread-1', status: 'sealed', seq: 0 },
        { id: 'sess-1', agentId: 'opus', threadId: 'thread-1', status: 'active', seq: 1 },
      ]);
      const reader = createMockTranscriptReader();
    ...
    it('correctly counts sealed sessions for Session #3+ (seq=2)', async () => {
      const store = createMockSessionChainStore([
        { id: 'sess-0', agentId: 'opus', threadId: 'thread-1', status: 'sealed', seq: 0 },
        { id: 'sess-1', agentId: 'opus', threadId: 'thread-1', status: 'sealed', seq: 1 },
        { id: 'sess-2', agentId: 'opus', threadId: 'thread-1', status: 'active', seq: 2 },
      ]);
      const reader = createMockTranscriptReader();
    ...
    it('uses sealing session as previous when most recent is sealing (R6 P1-2)', async () => {
      ...
      const store = createMockSessionChainStore([
        { id: 'sess-0', agentId: 'opus', threadId: 'thread-1', status: 'sealed', seq: 0 },
        { id: 'sess-1', agentId: 'opus', threadId: 'thread-1', status: 'sealing', seq: 1 },
      ]);
