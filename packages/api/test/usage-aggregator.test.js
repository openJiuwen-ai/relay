...
describe('aggregateUsageByDay', () => {
  ...
  function makeRecord(id, ts, usageByCat, statusOrOpts = 'succeeded') {
    ...
    return {
      ...
      userId: 'user-1',
      userMessageId: null,
      targetAgents: Object.keys(usageByCat),
      intent: 'execute',
  ...
  test('empty records returns empty daily array with grandTotal zeros', async () => {
    const { aggregateUsageByDay } = await import('../dist/domains/agents/services/usage-aggregator.js');
  ...
  test('agentId filter returns only matching cat data', async () => {
    const { aggregateUsageByDay } = await import('../dist/domains/agents/services/usage-aggregator.js');
    const anchor = todayNoon();
    const records = [
      makeRecord('inv-1', anchor, {
    ...
    const result = aggregateUsageByDay(records, { days: 7, agentId: 'opus' });
