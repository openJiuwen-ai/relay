...
describe('session-strategy Phase 2', () => {
  ...
  describe('config override priority (getConfigSessionStrategy)', () => {
    ...
    test('returns sessionStrategy for breed agentId', async () => {
    ...
    test('returns sessionStrategy for variant agentId', async () => {
      const { getConfigSessionStrategy } = await loadConfigModule();
      const config = mockConfig({
        strategy: 'compress',
        compress: { trackPostCompression: true },
      });
      // 'test-alt' is the variant agentId, should resolve to parent breed features
    ...
    test('returns undefined for unknown agentId', async () => {
      const { getConfigSessionStrategy } = await loadConfigModule();
