...
describe('deliverOutboundFromWeb (F088 ISSUE-15)', () => {
  ...
  beforeEach(() => {
    ...
    mockOutboundHook = {
      async deliver(threadId, content, agentId, richBlocks, threadMeta) {
    ...
    mockStreamingHook = {
      async onStreamStart(threadId, agentId, invocationId) {
        streamCalls.start.push({ threadId, agentId, invocationId });
      },
  ...
  it('delivers single-turn text to outbound hook', async () => {
    ...
    const turns = [{ agentId: 'opus', textParts: ['Hello ', 'world'] }];
  ...
  it('delivers multi-turn text per-cat', async () => {
    const opts = makeOpts({ outboundHook: mockOutboundHook });
    const turns = [
      { agentId: 'opus', textParts: ['First cat'] },
      { agentId: 'codex', textParts: ['Second cat'] },
    ];
    const ctx = { failed: false, errors: [] };
    ...
    assert.equal(deliverCalls[1].agentId, 'codex');
  });

  it('does not call deliver when outboundHook is absent', async () => {
    const opts = makeOpts();
    const turns = [{ agentId: 'opus', textParts: ['test'] }];
    const ctx = { failed: false, errors: [] };
  ...
  it('includes richBlocks in delivery', async () => {
    ...
    const turns = [{ agentId: 'opus', textParts: ['msg'], richBlocks: blocks }];
    const ctx = { failed: false, errors: [] };
  ...
  it('calls streaming lifecycle: start → end → cleanup', async () => {
    ...
    const turns = [{ agentId: 'opus', textParts: ['hi'] }];
    const ctx = { failed: false, errors: [] };
  ...
  it('does not throw when streaming hook fails', async () => {
    ...
    const turns = [{ agentId: 'opus', textParts: ['msg'] }];
    const ctx = { failed: false, errors: [] };
  ...
  it('resolves threadMeta from threadStore when available', async () => {
    ...
    const turns = [{ agentId: 'opus', textParts: ['hello'] }];
    const ctx = { failed: false, errors: [] };
