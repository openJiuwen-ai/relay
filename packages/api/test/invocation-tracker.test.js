 ...
 *
 * Also covers existing userId auth + agentId tracking (updated to slot-aware API).
 */
...
const { InvocationTracker } = await import('../src/domains/agents/services/agents/invocation/InvocationTracker.ts');
...
describe('InvocationTracker agentId tracking (slot-aware)', () => {
  it('start with agentIds stores them, cancel returns them', () => {
    const tracker = new InvocationTracker();
    tracker.start('thread-1', 'opus', 'alice', ['opus', 'gemini']);
    const result = tracker.cancel('thread-1', 'opus', 'alice');
    assert.equal(result.cancelled, true);
    assert.deepEqual(result.agentIds, ['opus', 'gemini']);
  });

  it('start without agentIds defaults to empty array', () => {
    const tracker = new InvocationTracker();
  ...
  it('cancel non-existent slot returns empty agentIds', () => {
    const tracker = new InvocationTracker();
  ...
  it('same cat new start in same thread overwrites previous agentIds', () => {
    const tracker = new InvocationTracker();
    tracker.start('thread-1', 'opus', 'alice', ['opus']);
    tracker.start('thread-1', 'opus', 'bob', ['gemini', 'codex']);
    const result = tracker.cancel('thread-1', 'opus', 'bob');
    assert.equal(result.cancelled, true);
    assert.deepEqual(result.agentIds, ['gemini', 'codex']);
  });
});
...
describe('SlotTracker: per-thread-per-cat isolation', () => {
  ...
  it('cancelAll only aborts slots owned by requestUserId when provided', () => {
    ...
    const cancelledAgentIds = tracker.cancelAll('t1', 'user1');

    assert.deepEqual(cancelledAgentIds, ['opus']);
    assert.equal(ctrl1.signal.aborted, true);
  ...
  it('getActiveSlots returns all active agentIds for thread', () => {
    const tracker = new InvocationTracker();
    tracker.start('t1', 'opus', 'user1', ['opus']);
    tracker.start('t1', 'codex', 'user1', ['codex']);
  ...
  it('has(threadId) without agentId returns true if any slot active', () => {
    const tracker = new InvocationTracker();
  ...
  it('getAgentIds returns target cats for specific slot', () => {
    const tracker = new InvocationTracker();
    tracker.start('t1', 'opus', 'user1', ['opus', 'gemini']);
    tracker.start('t1', 'codex', 'user1', ['codex']);
    assert.deepEqual(tracker.getAgentIds('t1', 'opus'), ['opus', 'gemini']);
    assert.deepEqual(tracker.getAgentIds('t1', 'codex'), ['codex']);
  });
