 ...
 * 3. threads.ts route validation (agentIdSchema on preferredCats)
 */
...
function createMockAgentService(agentId, responseText = 'Hello from mock') {
  const invoke = mock.fn(async function* (_prompt, options) {
    ...
    yield { type: 'session_init', agentId, sessionId, timestamp: Date.now() };
    yield { type: 'text', agentId, content: responseText, timestamp: Date.now() };
  ...
  return {
    ...
    addParticipants: (threadId, agentIds) => {
      ...
      for (const agentId of agentIds) {
        if (!participants[threadId].includes(agentId)) participants[threadId].push(agentId);
    ...
    updateLastActive: () => {},
    updatePreferredCats: (threadId, agentIds) => {
      if (agentIds.length > 0) {
        preferredCats[threadId] = agentIds;
      } else {
...
describe('ThreadStore preferredCats (memory)', () => {
  test('updatePreferredCats sets and retrieves preferred cats', async () => {
    ...
    store.updatePreferredCats(thread.id, [createAgentId('codex')]);

    const updated = store.get(thread.id);
    assert.deepEqual(updated.preferredCats, [createAgentId('codex')]);
  });
  ...
  test('updatePreferredCats supports multiple cats', async () => {
    ...
    const store = new ThreadStore();
    const thread = store.create('user-1', 'test');
    store.updatePreferredCats(thread.id, [createAgentId('codex'), createAgentId('gemini')]);

    const updated = store.get(thread.id);
    assert.deepEqual(updated.preferredCats, [createAgentId('codex'), createAgentId('gemini')]);
  });
});
...
describe('AgentRouter preferredCats routing', () => {
  ...
  // R5 P1-2 regression: duplicate agentIds must be deduped — cat invoked only once
  test('duplicate agentIds in preferredCats invokes cat only once', async () => {
    const { AgentRouter } = await import('../dist/domains/agents/services/agents/routing/AgentRouter.js');
