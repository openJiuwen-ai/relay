...
describe('WorklistRegistry', () => {
  ...
  test('cloud Codex P1: stale callback caller rejected by callerAgentId guard', async () => {
    ...
    try {
      ...
      // This simulates a stale callback from a preempted invocation whose agentId is codex
      ...
      // Without callerAgentId (legacy path from routeSerial text detection), always allowed.
      ...
      assert.deepEqual(pushToWorklist(threadId, ['codex']).added, ['codex']);
      assert.equal(fresh.a2aCount, 1, 'legacy path without callerAgentId must still work');
