...
import { InvocationRegistry } from '../dist/domains/agents/services/agents/invocation/InvocationRegistry.js';
import { InvocationTracker } from '../dist/domains/agents/services/agents/invocation/InvocationTracker.js';
...
function createFaultDrillRouter(modeRef) {
  ...
  return {
    ...
    async *routeExecution(_userId, _content, _threadId, _userMessageId, _targets, _intent, opts = {}) {
      ...
      yield { type: 'text', agentId: 'opus', content: 'fault-drill reply', timestamp: Date.now() };
      yield { type: 'done', agentId: 'opus', isFinal: true, timestamp: Date.now() };
    },
