...
async function createRealRouter() {
  ...
  const { GeminiAgentService } = await import(
    '../../dist/domains/agents/services/agents/providers/GeminiAgentService.js'
  );
...
describe('Multi-Cat Integration Tests', { skip: !shouldRunIntegrationTests }, () => {
  ...
  itOrSkip('routes to Claude (opus) when no @ mention is present', { skip: !hasClaude, timeout: 60_000 }, async () => {
    ...
    assert.ok(
      messages.some((m) => m.agentId === 'opus'),
  ...
  itOrSkip('routes to Codex when @assistant is mentioned', { skip: !hasCodex, timeout: 60_000 }, async () => {
    ...
    assert.ok(messages.length > 0, 'Should receive at least one message');
    assert.ok(
      messages.some((m) => m.agentId === 'codex'),
  ...
  itOrSkip('routes to Gemini when @design is mentioned', { skip: !hasGemini, timeout: 60_000 }, async () => {
    ...
    assert.ok(messages.length > 0, 'Should receive at least one message');
    assert.ok(
      messages.some((m) => m.agentId === 'gemini'),
  ...
  itOrSkip(
    ...
    async () => {
      ...
      const opusMessages = messages.filter((m) => m.agentId === 'opus');
      const codexMessages = messages.filter((m) => m.agentId === 'codex');
      ...
      const opusTextIndex = messages.findIndex((m) => m.agentId === 'opus' && m.type === 'text');
      const codexTextIndex = messages.findIndex((m) => m.agentId === 'codex' && m.type === 'text');
  ...
  itOrSkip(
    ...
    async () => {
      ...
      const agentIds = [...new Set(textMessages.map((m) => m.agentId))];

      assert.ok(agentIds.includes('opus'), 'Should have opus text');
      assert.ok(agentIds.includes('codex'), 'Should have codex text');
      assert.ok(agentIds.includes('gemini'), 'Should have gemini text');

      // Verify order: opus -> codex -> gemini
      const opusTextIndex = textMessages.findIndex((m) => m.agentId === 'opus');
      const codexTextIndex = textMessages.findIndex((m) => m.agentId === 'codex');
      const geminiTextIndex = textMessages.findIndex((m) => m.agentId === 'gemini');
...
describe('Individual Service Integration Tests', { skip: !shouldRunIntegrationTests }, () => {
  ...
  itOrSkip('CodexAgentService responds to prompt', { skip: !hasCodex, timeout: 60_000 }, async () => {
    const { CodexAgentService } = await import(
      '../../dist/domains/agents/services/agents/providers/CodexAgentService.js'
    );
