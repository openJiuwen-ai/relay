...
function createCapturingService(agentId, replyTextPrefix) {
  const capturedPrompts = [];
  let counter = 0;
  const invoke = mock.fn(async function* (prompt) {
    capturedPrompts.push(prompt);
    counter += 1;
    yield { type: 'session_init', agentId, sessionId: `${agentId}-sess`, timestamp: Date.now() };
    yield { type: 'text', agentId, content: `${replyTextPrefix}-${counter}`, timestamp: Date.now() };
