...
test('all messages have agentId opus', async () => {
  const proc = createMockProcess();
  const spawnFn = createMockSpawnFn(proc);
  const service = new ClaudeAgentService({ spawnFn });
  ...
  for (const msg of msgs) {
    assert.equal(msg.agentId, 'opus', `expected agentId opus, got ${msg.agentId}`);
  }
});
...
test('F32-b P1 regression: env var CAT_*_MODEL overrides default when model not passed', async () => {
  // Simulate index.ts pattern: pass agentId but NOT model → constructor resolves via getCatModel()
  ...
  try {
    const proc = createMockProcess();
    const spawnFn = createMockSpawnFn(proc);
    // NOTE: explicit agentId, no `model` param — matches the fixed index.ts pattern
    const service = new ClaudeAgentService({ agentId: 'opus', spawnFn });
