...
`);
    ...
    const parentOfficeClaw = join(realpathSync(parentRepo), '.office-claw');
    assert.notEqual(output, parentOfficeClaw, 'Must NOT write profiles to parent repo');
...
`);
    ...
    const outerOfficeClaw = join(realpathSync(outerRepo), '.office-claw');
    assert.notEqual(output, outerOfficeClaw, 'Must NOT write profiles to outer repo');
...
`);
    ...
    const victimOfficeClaw = join(realpathSync(victimRepo), '.office-claw');
    assert.notEqual(output, victimOfficeClaw, 'Must NOT write profiles to victim repo (forged .git)');
