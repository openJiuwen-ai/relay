...
describe('invocation-level hard timeout (F089)', () => {
  ...
  it('service that never yields done converges via invocation timeout', async () => {
    ...
    const hangingService = {
      async *invoke() {
        yield { type: 'text', agentId: 'codex', content: 'thinking...', timestamp: Date.now() };
  ...
  it('CLI_TIMEOUT_MS=0 does not produce instant invocation timeout', async () => {
    ...
    try {
      ...
      const freshMod = await import(
        `../dist/domains/agents/services/agents/invocation/invoke-single-cat.js?t=${Date.now()}`
      );
  ...
  it('user cancel (AbortSignal) still works alongside invocation timeout', async () => {
    ...
    const hangingService = {
      async *invoke() {
        yield { type: 'text', agentId: 'codex', content: 'hi', timestamp: Date.now() };
        await new Promise(() => {});
      },
    };
  ...
  it('active invocations with steady progress should not hit invocation_timeout', async () => {
    const progressiveService = {
      async *invoke() {
        yield { type: 'text', agentId: 'codex', content: 'tick-1', timestamp: Date.now() };
        await new Promise((r) => setTimeout(r, 150));
        yield { type: 'text', agentId: 'codex', content: 'tick-2', timestamp: Date.now() };
        await new Promise((r) => setTimeout(r, 150));
        yield { type: 'text', agentId: 'codex', content: 'tick-3', timestamp: Date.now() };
        await new Promise((r) => setTimeout(r, 150));
        yield { type: 'done', agentId: 'codex', isFinal: true, timestamp: Date.now() };
      },
    };
