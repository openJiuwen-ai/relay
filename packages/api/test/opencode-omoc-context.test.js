...
describe('Ralph Loop + Context Management (AC-11)', () => {
  ...
  // ── Ralph Loop continuation maintains consistent agentId ──

  test('all messages across Ralph Loop cycles have consistent agentId', async () => {
    const proc = createMockProcess();
    const spawnFn = mock.fn(() => proc);
    const service = new OpenCodeAgentService({
      agentId: 'opencode',
    ...
    for (const m of messages) {
      assert.strictEqual(
        m.agentId,
        'opencode',
        `all messages must have agentId=opencode, got "${m.agentId}" for type=${m.type}`,
      );
    }
  });
});
