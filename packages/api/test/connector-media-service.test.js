...
describe('ConnectorMediaService', () => {
  ...
  it('downloads weixin file media through late-bound download function', async () => {
    const { ConnectorMediaService } = await import('../dist/infrastructure/connectors/media/ConnectorMediaService.js');

    const tempDir = await mkdtemp(path.join(tmpdir(), 'media-test-'));
    const service = new ConnectorMediaService({ mediaDir: tempDir });
    const lateFn = mock.fn(async () => Buffer.from('weixin-file-data'));
    service.setWeixinDownloadFn(lateFn);

    const result = await service.download('weixin', {
      type: 'file',
      platformKey: '{"encryptQueryParam":"abc"}',
      fileName: 'report.pdf',
    });

    assert.equal(lateFn.mock.calls.length, 1);
    assert.equal(lateFn.mock.calls[0].arguments[0], '{"encryptQueryParam":"abc"}');
    assert.equal(lateFn.mock.calls[0].arguments[1], 'file');
    assert.ok(result.absPath.endsWith('.pdf'));
    assert.equal(result.mimeType, 'application/pdf');
