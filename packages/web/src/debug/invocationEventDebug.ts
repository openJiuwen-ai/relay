...
const LEGACY_STORAGE_KEY = 'cat-cafe:debug:ring-buffer';
...
function resetToDisabled() {
  ...
  clearPersistedDebugFlag(LEGACY_STORAGE_KEY);
  removeWindowApi();
}
...
export function bootstrapDebugFromStorage() {
  ...
  const applyPayload = (raw: string, source: DebugStorageEntry): boolean => {
    ...
    if (enabledValue && typeof parsed.expiresAt === 'number' && Number.isFinite(parsed.expiresAt)) {
      ...
      if (remaining <= 0) {
        ...
        clearStorageKey(source.storage, LEGACY_STORAGE_KEY);
        return false;
      }
  ...
  for (const source of storages) {
    let raw = safeReadStorage(source.storage, STORAGE_KEY);
    let usedLegacy = false;
    if (raw === null) {
      raw = safeReadStorage(source.storage, LEGACY_STORAGE_KEY);
      usedLegacy = raw !== null;
    }
    if (raw === null) continue;
    if (applyPayload(raw, source)) {
      if (usedLegacy) clearStorageKey(source.storage, LEGACY_STORAGE_KEY);
      return;
    }
  }
}
