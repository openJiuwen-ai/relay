...
import type { VoiceChunkEvent, VoiceStreamEndEvent, VoiceStreamStartEvent } from '@openjiuwen/relay-shared';
...
function matchesActiveSession(event: { threadId: string; agentId: string }): boolean {
  ...
  if (session.activeCatId !== event.agentId) return false;
  return true;
}
