...
function SendRunner({
  ...
  sendOptions?: { resumeAgentId?: string; clientDraftId?: string; mentionRefs?: Array<{ catId: string; mention: string }> };
  ...
  }, [deliveryMode, handleSend, interactiveAsk, onDone, overrideThreadId, sendOptions]);
...
describe('useSendMessage thread source', () => {
  ...
  it('includes resumeAgentId in the message payload when requested', async () => {
    await act(async () => {
      root.render(
        React.createElement(SendRunner, {
          activeThreadId: 'thread-route',
          overrideThreadId: undefined,
          sendOptions: { resumeAgentId: 'opus' },
          onDone: () => {},
        }),
      );
    });
    ...
    expect(mockApiFetch).toHaveBeenCalled();
    const payload = JSON.parse(String(mockApiFetch.mock.calls[0]?.[1]?.body));
    expect(payload.resumeAgentId).toBe('opus');
  });

  it('includes interactive_ask in the message payload when guided mode is enabled', async () => {
    await act(async () => {
      root.render(
        React.createElement(SendRunner, {
          activeThreadId: 'thread-route',
    ...
    expect(mockApiFetch).toHaveBeenCalled();
    const payload = JSON.parse(String(mockApiFetch.mock.calls[0]?.[1]?.body));
    expect(payload.interactive_ask).toBe(true);
  });

  it('does not send deprecated clientDraftId to /api/messages payload', async () => {
    await act(async () => {
      root.render(
        React.createElement(SendRunner, {
          activeThreadId: 'thread-route',
          ...
          deliveryMode: 'queue',
          sendOptions: {
            clientDraftId: 'draft-local-only',
    ...
    expect(mockApiFetch).toHaveBeenCalled();
    const payload = JSON.parse(String(mockApiFetch.mock.calls[0]?.[1]?.body));
    expect(payload.deliveryMode).toBe('queue');
    expect(payload.mentionRefs).toEqual([{ catId: 'expert-poetry', mention: '@古诗词创作专家' }]);
    expect(payload.clientDraftId).toBeUndefined();
  });
  ...
  it('queue-mode send degraded to immediate adds user bubble with persisted id', async () => {
    mockApiFetch.mockResolvedValue({
      ok: true,
      json: async () => ({ status: 'processing', userMessageId: 'msg-server-fallback' }),
    });
