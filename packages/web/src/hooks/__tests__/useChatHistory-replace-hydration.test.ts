...
function buildThreadBState(cachedAssistantTs: number) {
  return {
    ...
    targetAgents: ['opus'],
    agentStatuses: { opus: 'streaming' as const },
    agentInvocations: {},
    currentGame: null,
...
describe('useChatHistory replace hydration', () => {
  ...
  it('preserves a newer live bubble that arrived after thread switch', async () => {
    ...
    await history.resolve({
      messages: [{ id: 'b1', agentId: 'opus', content: 'cached assistant', timestamp: cachedAssistantTs }],
      hasMore: false,
    });
  ...
  it('keeps a richer local stream bubble instead of a stale draft duplicate', async () => {
    ...
    mountReplaceHydrationThread(
      makeThreadBState(cachedAssistantTs, {
        agentInvocations: { opus: { invocationId: 'inv-1', startedAt: cachedAssistantTs } },
      }),
    );
    ...
    await history.resolve({
      messages: [
        { id: 'b1', agentId: 'opus', content: 'cached assistant', timestamp: cachedAssistantTs },
        {
          id: 'draft-inv-1',
          agentId: 'opus',
          content: 'stale draft',
  ...
  it('clears stale isStreaming when local replaces a completed stream row (after stop / catch-up)', async () => {
    const history = installDeferredHistoryResponse();
    const cachedAssistantTs = Date.now() - 1000;
    mountReplaceHydrationThread(
      makeThreadBState(cachedAssistantTs, {
    ...
    const merged = useChatStore.getState().messages.find((m) => m.id === 'live-1');
    expect(merged).toBeDefined();
    expect(merged?.isStreaming).toBeFalsy();
  });

  it('clears isStreaming on preserved draft-* rows when invocation is not active (post-Stop catch-up)', async () => {
    const history = installDeferredHistoryResponse();
    const cachedAssistantTs = Date.now() - 1000;
    const staleInv = '35a9b196-1ede-45b7-b7a4-ae28f2631ab7';
    mountReplaceHydrationThread(
      makeThreadBState(cachedAssistantTs, {
        agentInvocations: {},
      }),
    ...
    act(() => {
      useChatStore.getState().addMessage({
        ...
        type: 'assistant',
        agentId: 'opus',
        content: 'draft shell with tools',
        timestamp: Date.now(),
        isStreaming: true,
        origin: 'stream',
        extra: { stream: { invocationId: staleInv } },
        toolEvents: [
          { id: 't1', type: 'tool_use' as const, label: 'tool', timestamp: Date.now() },
        ],
      });
    });
    ...
    await history.resolve({
      messages: [
        { id: 'b1', agentId: 'opus', content: 'cached assistant', timestamp: cachedAssistantTs },
        {
          id: '0001777292131480-000002-e4b3bf84',
          agentId: 'opus',
          content: 'persisted older turn',
          origin: 'stream',
          timestamp: Date.now() - 2000,
          extra: { stream: { invocationId: 'c5fa5683-9b54-4f26-9dbe-624344ce57ec' } },
        },
      ],
      hasMore: false,
    });

    const draft = useChatStore.getState().messages.find((m) => m.id === `draft-${staleInv}`);
    expect(draft).toBeDefined();
    expect(draft?.isStreaming).toBeFalsy();
  });
  ...
  it('reconciles a completed draft bubble with its formal message (Bug B: no duplicate)', async () => {
    const history = installDeferredHistoryResponse();
    const cachedAssistantTs = Date.now() - 1000;
    // No agentInvocations — simulates post-done state where invocationId is cleared
    mountReplaceHydrationThread(makeThreadBState(cachedAssistantTs));
      ...
      useChatStore.setState((state) => ({
        ...
        threadStates: {
          ...
          'thread-b': {
            ...
            messages: seededMessages,
            hasActiveInvocation: true,
            agentStatuses: { codex: 'streaming' },
            agentInvocations: { codex: { invocationId: 'inv-2', startedAt: now - 200 } },
          },
        },
      }));
    });
