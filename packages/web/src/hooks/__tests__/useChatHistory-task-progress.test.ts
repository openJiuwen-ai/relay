...
describe('useChatHistory task-progress hydration', () => {
  ...
  it('does not restore completed snapshots into targetAgents', async () => {
    ...
    expect(state.agentInvocations.codex?.taskProgress?.snapshotStatus).toBe('completed');
    expect(state.targetAgents).toEqual([]);
  });

  it('restores running snapshots into targetAgents even when all tasks are completed', async () => {
    apiFetchMock.mockImplementation((url: string) => {
      if (typeof url === 'string' && url.includes('/api/messages')) {
        return Promise.resolve(new Response(JSON.stringify({ messages: [], hasMore: false }), { status: 200 }));
      }
    ...
    const state = useChatStore.getState();
    expect(state.agentInvocations.codex?.taskProgress?.snapshotStatus).toBe('running');
