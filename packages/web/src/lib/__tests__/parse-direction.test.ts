...
// Minimal mock: alias (lowercase, no @) → agentId
const mockToAgent: Record<string, string> = {
  opus: 'opus',
  布偶猫: 'opus',
  codex: 'codex',
  缅因猫: 'codex',
...
const aliases = Object.keys(mockToAgent).sort((a, b) => b.length - a.length);
const mockRe = new RegExp(`@(${aliases.join('|')})(?=$|\\s|[,.:;!?])`, 'gi');

const getMocks = () => ({ toAgent: mockToAgent, re: mockRe });
...
describe('parseDirection', () => {
  ...
  it('uses extra.targetAgents when present (F098-C1)', () => {
    const msg = {
      origin: 'callback' as const,
      content: 'Review done',
  ...
  it('extra.targetAgents takes priority over content @mention parsing (F098-C1)', () => {
    const msg = {
      origin: 'callback' as const,
      content: '@codex some content',
      extra: { targetAgents: ['gpt52'] },
    };
    const result = parseDirection(msg, getMocks);
  ...
  it('filters out __co-creator__ pseudo-cat from @mention results (P1-2)', () => {
    // getMentionToAgentId maps @co-creator/@铲屎官 to __co-creator__ — must not leak into UI
    ...
    const getOwnerMocks = () => ({ toAgent: ownerToCat, re: ownerRe });
