...
const LEGACY_PROJECT_PIN_KEY = 'cat-cafe:sidebar:pinned-projects';
...
export function readPinnedProjects(storage: StorageLike): Set<string> {
  try {
    let raw = storage.getItem(PROJECT_PIN_KEY);
    if (!raw) raw = storage.getItem(LEGACY_PROJECT_PIN_KEY);
    if (raw) {
...
export function writePinnedProjects(projects: Set<string>, storage: StorageLike): void {
  try {
    ...
    try {
      storage.removeItem?.(LEGACY_PROJECT_PIN_KEY);
    } catch {
      /* ignore */
    }
  } catch {
