...
export type { Capabilities, AgentDeskConfig, ConfigData, ContextBudget } from './config-viewer-types';
...
export function AgentOverviewTab({
  ...
  onEditMember?: (agent: AgentData) => void;
  onToggleAvailability?: (agent: AgentData) => void;
      ...
      <div className="space-y-3">
        {members.map((memberData) => (
          <HubMemberOverviewCard
            key={memberData.id}
            member={memberData}
            configAgent={config.agents[memberData.id]}
            clientLabels={clientLabels}
            onEdit={onEditMember}
            onToggleAvailability={onToggleAvailability}
            togglingAvailability={togglingAgentId === memberData.id}
          />
        ))}
      </div>
      <p className="text-[13px] text-[#B59A88]">点击任意卡片进入成员配置 →</p>
      {members.length === 0 && <p className="text-sm text-gray-400">未找到成员配置数据</p>}
    </div>
  );
}
