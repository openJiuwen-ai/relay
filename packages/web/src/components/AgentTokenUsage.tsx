...
export interface AgentTokenUsageProps {
  agentId: string;
  usage: TokenUsage;
 ...
 */
export function AgentTokenUsage({ agentId, usage, contextHealth }: AgentTokenUsageProps) {
  ...
  const textColor = CAT_TEXT_COLORS[agentId] ?? 'text-gray-700';
  ...
  return (
    <div className="mt-1.5 space-y-1 animate-fade-in" data-testid={`token-usage-${agentId}`}>
      {/* Token counts row */}
      ...
      {cachePct > 0 && (
        <div>
          ...
          <TokenCacheBar percent={cachePct} agentId={agentId} />
        </div>
      )}
      ...
      {contextHealth && (
        <div>
          ...
          <ContextHealthBar agentId={agentId} health={contextHealth} />
        </div>
      )}
    </div>
  );
