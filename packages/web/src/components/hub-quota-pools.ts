...
function memberTag(agent: AgentData): string {
  return agent.mentionPatterns[0] ?? `@${agent.id}`;
}

function fallbackAccountRef(agent: AgentData): string | null {
...
function memberTagsForAccount(agents: AgentData[], accountId: string): string[] {
  return uniqueTags(
    agents
      .filter((row) => {
        const boundAccountRef = row.accountRef?.trim() || row.providerProfileId?.trim() || fallbackAccountRef(row);
...
export function buildAccountQuotaGroups(
  ...
  const builtinPools = builtinProfiles.map<AccountQuotaPool>((profile) => ({
    ...
    memberTags: memberTagsForAccount(agents, profile.id),
  ...
  const antigravityMemberTags = uniqueTags(agents.filter((row) => row.provider === 'antigravity').map(memberTag));
