...
const AGENT_USAGE_LABELS: Record<string, string> = {
  opus: '通用智能体 Opus',
  sonnet: '通用智能体 Sonnet',
...
export function DailyUsageSection() {
  ...
  return (
    <section className="rounded-xl border border-gray-200 bg-white p-4 space-y-3">
      ...
      {days.map((day) => {
        const perDayAgents = Object.entries(day.agents).sort(
        ...
        return (
          <div key={day.date} className="border-t border-gray-100 pt-2 space-y-1">
            ...
            {perDayAgents.map(([agentId, usage]) => (
              <AgentUsageRow key={agentId} agentId={agentId} usage={usage} />
            ))}
          </div>
        );
      })}
...
function AgentUsageRow({ agentId, usage }: { agentId: string; usage: AgentDailyUsage }) {
  return (
    <div className="flex items-center justify-between gap-2 text-xs">
      <div className="flex items-center gap-2 min-w-0">
        <span className="font-medium text-gray-700 truncate">{agentLabel(agentId)}</span>
