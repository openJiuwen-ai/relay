...
} from '../thread-sidebar/collapse-state';
...
const ALL_KEYS = ['pinned', '/proj/acme', '/proj/dare', 'favorites'];
...
describe('initCollapsedSet', () => {
  ...
  it('restores collapsed state from storage on init', () => {
    ...
    expect(set.has('/proj/acme')).toBe(false);
...
describe('persistence round-trip', () => {
  ...
  it('readCollapsedGroups returns what writeCollapsedGroups stored', () => {
    const storage = createMockStorage();
    writeCollapsedGroups(['/proj/acme'], storage);
    const result = readCollapsedGroups(storage);
    expect(result).toEqual(['/proj/acme']);
  });
...
describe('shouldCollapse', () => {
  ...
  it('returns true for collapsed groups when no search', () => {
    ...
    expect(shouldCollapse('/proj/acme', collapsed, '')).toBe(false);
  });
...
describe('findGroupKeyForThread', () => {
  const groups = [
    ...
    { groupKey: '/proj/acme', threadIds: ['t1', 't2'] },
  ...
  it('finds the group containing the given thread', () => {
    expect(findGroupKeyForThread('t1', groups)).toBe('/proj/acme');
  ...
  // Thread in both recent + project should prefer project ordering.
  it('prefers project group over recent when thread appears in both', () => {
    const groupsWithRecent = [
      ...
      { groupKey: '/proj/acme', threadIds: ['t1', 't2'], type: 'project' as const },
      { groupKey: '/proj/dare', threadIds: ['t3'], type: 'project' as const },
    ];
    // t1 is in both recent and /proj/acme → should return project
    expect(findGroupKeyForThread('t1', groupsWithRecent)).toBe('/proj/acme');
  ...
  it('falls back to recent if thread is only in recent', () => {
    const groupsWithRecent = [
      ...
      { groupKey: '/proj/acme', threadIds: ['t1'], type: 'project' as const },
    ];
...
describe('expandAllGroups / collapseAllGroups', () => {
  ...
  it('collapseAllGroups collapses ALL known keys, not just filtered subset', () => {
    const allKnown = ['pinned', '/proj/acme', '/proj/dare', 'favorites'];
    const filteredVisible = ['pinned', '/proj/acme']; // simulates search filter
...
describe('shouldCollapseBeforeInit', () => {
  it('returns true (collapsed) for any group before storage is read', () => {
    ...
    expect(shouldCollapseBeforeInit('/proj/acme')).toBe(true);
...
describe('resolveCollapse (hook-equivalent logic)', () => {
  ...
  it('delegates to shouldCollapse after init', () => {
    ...
    expect(resolveCollapse('/proj/acme', collapsed, '', true)).toBe(false);
...
describe('independent group collapse', () => {
  it('toggle one group does not affect others (via Set operations)', () => {
    ...
    // Simulate expanding acme
    collapsed.delete('/proj/acme');
    expect(collapsed.has('/proj/acme')).toBe(false);
    expect(collapsed.has('pinned')).toBe(true);
    expect(collapsed.has('/proj/dare')).toBe(true);
    expect(collapsed.has('favorites')).toBe(true);

    // Simulate re-collapsing acme
    collapsed.add('/proj/acme');
    expect(collapsed.has('/proj/acme')).toBe(true);
  });
});
