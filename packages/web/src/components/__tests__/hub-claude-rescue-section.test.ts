...
describe('HubClaudeRescueSection', () => {
  ...
  it('renders scanned broken sessions as checklist items', async () => {
    ...
    expect(container.textContent).toContain('检测到 2 只通用智能体 session 需要救援');
    ...
    expect(container.textContent).toContain('一键救活 2 只通用智能体');
  ...
  it('shows empty state when scan finds no broken sessions', async () => {
    ...
    expect(container.textContent).toContain('暂未发现坏掉的通用智能体 session');
  ...
  it('rescues selected sessions, refreshes scan, and shows success toast', async () => {
    ...
    const rescueButton = Array.from(container.querySelectorAll('button')).find((node) =>
      node.textContent?.includes('一键救活 2 只通用智能体'),
    ...
    expect(container.textContent).toContain('刚刚救活 2 只通用智能体');
    expect(container.textContent).toContain('暂未发现坏掉的通用智能体 session');

    const toasts = useToastStore.getState().toasts;
    expect(toasts.some((toast) => toast.type === 'success' && toast.title === '通用智能体已救活')).toBe(true);
  });
});
