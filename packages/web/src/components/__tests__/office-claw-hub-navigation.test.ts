...
// Mock heavy deps so OfficeClawHub can be imported without full React tree
vi.mock('@/hooks/useAgentData', () => ({
  useAgentData: () => ({ agents: [], getAgentById: () => undefined, refresh: () => Promise.resolve([]) }),
}));
vi.mock('@/utils/api-client', () => ({ apiFetch: vi.fn() }));

const { resolveRequestedHubTab, findGroupForTab } = await import('../OfficeClawHub');
