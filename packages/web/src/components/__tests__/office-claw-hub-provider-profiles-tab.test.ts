...
vi.mock('@/hooks/useAgentData', () => ({
  ...
  formatAgentName: (cat: { displayName: string; variantLabel?: string }) =>
...
import { OfficeClawHub } from '@/components/OfficeClawHub';
...
describe('OfficeClawHub provider profiles tab', () => {
  ...
  it('renders provider profiles tab label', () => {
    const html = renderToStaticMarkup(React.createElement(OfficeClawHub));
  ...
  it('keeps ragdoll rescue controls out of provider profiles after tab data loads', async () => {
    ...
    expect(container.textContent).not.toContain('通用智能体救援中心');
  ...
  it('renders ragdoll rescue section from the dedicated rescue tab', async () => {
    ...
    mockApiFetch.mockImplementation((path: string) => {
      ...
      if (path === '/api/config') {
        return Promise.resolve(
          jsonResponse({ config: { agents: {}, perAgentBudgets: {}, a2a: {}, memory: {}, hindsight: {}, governance: {} } }),
        );
      }
    ...
    await act(async () => {
      root.render(React.createElement(OfficeClawHub));
    });
    await flushEffects();

    expect(container.textContent).toContain('通用智能体救援中心');
    expect(container.textContent).toContain('检测到 1 只通用智能体 session 需要救援');
  });
});
