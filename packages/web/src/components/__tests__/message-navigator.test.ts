...
function makeMsg(id: string, type: 'user' | 'assistant' | 'system', agentId?: string): ChatMessageData {
  return {
    id,
    type,
...
describe('MessageNavigator', () => {
  ...
  it('tolerates variant agentIds before /api/agents loads', () => {
    const msgs = [makeMsg('m1', 'user'), makeMsg('m2', 'assistant', 'opus-45'), makeMsg('m3', 'assistant', 'spark')];
    const html = render(msgs);

    // base colors come from shared fallback OFFICE_CLAW_CONFIGS (opus/codex)
    expect(html).toContain('#9B7EBD');
    expect(html).toContain('#5B8C5A');
  ...
  it('resolves non-hyphen variant agentIds during fallback', () => {
    const msgs = [
      makeMsg('m1', 'user'),
    ...
    // base colors come from shared fallback OFFICE_CLAW_CONFIGS
  ...
  it('treats messages with agentId as assistant even when type is user', () => {
  ...
  it('applies dare fallback color and labels before /api/agents loads', () => {
