...
describe('ChatMessage image lightbox', () => {
    ...
    act(() => {
      root.render(
        React.createElement(ChatMessage, {
          ...
          getAgentById: (() => undefined) as never,
        }),
      );
    });
  ...
  it('downgrades legacy workspace image URLs to a sunset notice', async () => {
    const { ChatMessage } = await import('@/components/chat-message');
    ...
    expect(container.textContent).toContain('历史 workspace 图片，能力已下线');
    expect(container.textContent).toContain('images/cat.png');
