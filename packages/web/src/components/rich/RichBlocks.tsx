...
function RichBlockRenderer({ block, agentId, messageId }: { block: RichBlock; agentId?: string; messageId?: string }) {
  switch (block.kind) {
    case 'card':
    ...
    case 'audio':
      return <AudioBlock block={block} agentId={agentId} />;
    case 'interactive':
...
export function RichBlocks({ blocks, agentId, messageId }: { blocks: RichBlock[]; agentId?: string; messageId?: string }) {
    ...
    <div className="mt-2 space-y-2">
      {items.map((item) =>
        'grouped' in item ? (
          ...
          <RichBlockRenderer key={item.id} block={item} agentId={agentId} messageId={messageId} />
        ),
      )}
    </div>
  );
