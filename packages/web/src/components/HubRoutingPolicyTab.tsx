...
function buildPolicy(params: {
  ...
  if (params.reviewAvoidOpus) {
    scopes.review = { avoidAgentIds: ['opus'], reason: 'budget' };
  }
  if (params.architecturePreferOpus) {
    scopes.architecture = { preferAgentIds: ['opus'] };
  }
...
export function HubRoutingPolicyTab() {
  ...
  const fetchThread = useCallback(async () => {
    ...
    try {
      ...
      const avoid = policy?.scopes?.review?.avoidAgentIds ?? [];
      const prefer = policy?.scopes?.architecture?.preferAgentIds ?? [];
