...
interface A2ACollapsibleProps {
  ...
  /** F098: Resolve breed primary color from agentId for left border */
  getAgentBorderColor?: (agentId: string) => string | undefined;
}
 ...
 */
export function A2ACollapsible({ group, renderMessage, getAgentBorderColor }: A2ACollapsibleProps) {
  ...
  const agentIds = [...new Set(group.messages.filter((m) => m.agentId).map((m) => m.agentId!))];
  const agentLabel = agentIds.length > 0 ? agentIds.join(' ↔ ') : 'agents';
  const count = group.messages.length;

  // F098: Use first agent's breed color for left border (instead of static purple)
  const firstAgentId = group.messages[0]?.agentId;
  const borderColor = (firstAgentId && getAgentBorderColor?.(firstAgentId)) ?? '#9B7EBD';
  ...
  return (
    <div className="my-2">
      ...
      >
        ...
        <span>
          {expanded ? '收起内部讨论' : `查看内部讨论`} ({agentLabel}, {count} 条)
        </span>
      </button>
