...
import { buildAgentOptions, detectMenuTrigger } from '@/components/chat-input/chat-input-options';
import type { AgentData } from '@/hooks/useAgentData';

const MANY_MENTION_AGENTS: AgentData[] = [
  {
    id: 'opus',
    displayName: '布偶猫',
...
describe('mention filter matching', () => {
  const options = buildAgentOptions(MANY_MENTION_AGENTS);
