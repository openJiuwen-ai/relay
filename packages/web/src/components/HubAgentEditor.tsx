...
import { buildEditorLoadingNote, uploadAvatarAsset } from './hub-agent-editor.client';
import {
  buildAgentPayload,
...
import { AccountSection, EmbeddedAcpConfigSection, IdentitySection, RoutingSection } from './hub-agent-editor.sections';
import { AdvancedRuntimeSection } from './hub-agent-editor-advanced';
import { PersistenceBanner } from './hub-agent-editor-fields';
import type { ProfileItem, ProviderProfilesResponse } from './hub-provider-profiles.types';
import type { AgentStrategyEntry } from './hub-strategy-types';
import { useConfirm } from './useConfirm';

interface HubAgentEditorProps {
  member?: AgentData | null;
  configAgent?: ConfigData['agents'][string];
  draft?: HubAgentEditorDraft | null;
  open: boolean;
  onClose: () => void;
  onSaved: () => Promise<void> | void;
}

function resolveEditorMember(member?: AgentData | null, configAgent?: ConfigData['agents'][string]): AgentData | null | undefined {
  if (!member) return member;
  if (!configAgent) return member;
  const embeddedAcpRuntime = hasEmbeddedAcpRuntime(member);
  return {
    ...member,
    displayName: configAgent.displayName || member.displayName,
    accountRef: embeddedAcpRuntime
      ? member.accountRef || member.providerProfileId
      : configAgent.accountRef || configAgent.providerProfileId || member.accountRef || member.providerProfileId,
    providerProfileId: embeddedAcpRuntime
      ? member.providerProfileId || member.accountRef
      : configAgent.providerProfileId || configAgent.accountRef || member.providerProfileId || member.accountRef,
    provider: configAgent.provider || member.provider,
    defaultModel: configAgent.model || member.defaultModel,
    embeddedAcpConfig: configAgent.embeddedAcpConfig || member.embeddedAcpConfig,
    embeddedAcpExecutablePath: configAgent.embeddedAcpExecutablePath || member.embeddedAcpExecutablePath,
  };
}
...
export function HubAgentEditor({ member, configAgent, draft, open, onClose, onSaved }: HubAgentEditorProps) {
  ...
  const resolvedAgent = useMemo(() => resolveEditorMember(member, configAgent), [member, configAgent]);
  ...
  const [form, setForm] = useState<HubAgentEditorFormState>(() => initialState(resolvedAgent, draft));
  ...
  const embeddedAcpRuntime = useMemo(() => hasEmbeddedAcpRuntime(resolvedAgent), [resolvedAgent]);
  ...
  useEffect(() => {
    if (!open) return;
    setForm(initialState(resolvedAgent, draft));
    setFieldErrors({});
    setError(null);
  ...
  useEffect(() => {
    ...
    apiFetch('/api/config/session-strategy')
      .then(async (res) => {
        ...
        return (await res.json()) as { agents?: AgentStrategyEntry[] };
      })
      .then((body) => {
        if (cancelled) return;
        const entry = body.agents?.find((item) => item.agentId === resolvedAgent.id) ?? null;
  ...
  }, [open, resolvedAgent, showCodexSettings]);
  ...
  useEffect(() => {
    ...
    setForm((prev) => {
      if (prev.accountRef.trim().length === 0 && (member || !draft)) {
        return prev;
      }
  ...
  }, [availableProfiles, member, draft, form.client]);
  ...
  const patchForm = (patch: Partial<HubAgentEditorFormState>) => {
  ...
  const handleSave = async () => {
    ...
    try {
      const agentPayload = buildAgentPayload(form, resolvedAgent);
      const rollbackAgentPayload = resolvedAgent ? buildAgentPayload(initialState(resolvedAgent, null), resolvedAgent) : null;
      const nextStrategyPayload = resolvedAgent && strategyForm ? buildStrategyPayload(strategyForm) : null;
      const baselineStrategyPayload = resolvedAgent && strategyBaseline ? buildStrategyPayload(strategyBaseline) : null;
      const strategyChanged =
        resolvedAgent && nextStrategyPayload
          ? JSON.stringify(nextStrategyPayload) !== JSON.stringify(baselineStrategyPayload)
          : false;

      if (resolvedAgent && strategyChanged && nextStrategyPayload) {
        const strategyRes = await apiFetch(`/api/config/session-strategy/${resolvedAgent.id}`, {
          method: 'PATCH',
        ...
        if (strategyBaselineHasOverride && baselineStrategyPayload) {
          rollbackSteps.push(async () => {
            await apiFetch(`/api/config/session-strategy/${resolvedAgent.id}`, {
              method: 'PATCH',
              headers: { 'Content-Type': 'application/json' },
      ...
      const res = await apiFetch(resolvedAgent ? `/api/agents/${resolvedAgent.id}` : '/api/agents', {
        method: resolvedAgent ? 'PATCH' : 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      const persistedAgentBody = (await res.json().catch(() => ({}))) as { agent?: { id?: string } };
      const persistedCatId = persistedAgentBody.agent?.id ?? resolvedAgent?.id ?? null;
      if (persistedCatId) {
        if (resolvedAgent && rollbackAgentPayload) {
          rollbackSteps.push(async () => {
            await apiFetch(`/api/agents/${persistedCatId}`, {
              method: 'PATCH',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify(rollbackAgentPayload),
            });
          });
  ...
  const handleDelete = async () => {
    ...
    const ok = await confirm({
      title: '删除确认',
      message: `确认删除成员「${member.displayName}」吗？此操作不可撤销。`,
      variant: 'danger',
      confirmLabel: '删除',
    });
    if (!ok) return;
    ...
    setError(null);
    try {
      const res = await apiFetch(`/api/agents/${member.id}`, { method: 'DELETE' });
      if (!res.ok) {
        const payload = (await res.json().catch(() => ({}))) as Record<string, unknown>;
        ...
        <div className="flex shrink-0 items-start justify-between border-b border-[#F0DDCD] px-7 py-5">
          <div>
            <p className="text-[13px] font-semibold text-[#77A777]">
              成员协作 &gt; 总览 &gt; {member ? '编辑成员' : '添加成员'}
            </p>
          </div>
          <div className="flex items-center gap-2">
            {member && member.source === 'runtime' ? (
              <button
                type="button"
