 ...
 * - Bug 2: clearAgentStatuses leaves agentInvocations stale → status panel shows wrong state
...
describe('Issue #586 Bug 1: Store-level message dedup', () => {
  ...
  it('addMessageToThread deduplicates by exact id in background thread', () => {
    const store = useChatStore.getState();
    store.addMessageToThread('thread-B', makMsg('bg-1', { agentId: 'opus' }));
    store.addMessageToThread('thread-B', makMsg('bg-1', { agentId: 'opus' }));
...
describe('Issue #586 Bug 2: clearAgentStatuses also resets agentInvocations', () => {
  beforeEach(() => {
  ...
  it('clearAgentStatuses marks running taskProgress as completed', () => {
    const store = useChatStore.getState();

    // Simulate cat working with active task progress
    store.setAgentStatus('opus', 'streaming');
    ...
    // agentInvocations taskProgress should be marked completed
    expect(state.agentInvocations.opus?.taskProgress?.snapshotStatus).toBe('completed');
  });

  it('clearAgentStatuses preserves already-completed taskProgress', () => {
    const store = useChatStore.getState();
    store.setAgentInvocation('opus', {
      taskProgress: {
  ...
  it('clearAgentStatuses preserves interrupted taskProgress (cloud P1)', () => {
    const store = useChatStore.getState();
    ...
    expect(state.agentInvocations.codex?.taskProgress?.snapshotStatus).toBe('interrupted');
    expect(state.agentInvocations.codex?.taskProgress?.interruptReason).toBe('canceled');
  });

  it('clearAgentStatuses handles empty agentInvocations', () => {
    const store = useChatStore.getState();
    store.setAgentStatus('opus', 'done');
    store.clearAgentStatuses();
    const state = useChatStore.getState();
    expect(state.agentStatuses).toEqual({});
